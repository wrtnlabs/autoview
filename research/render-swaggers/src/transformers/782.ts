...
function visualizeData(
  ...
  const stateColor: Record<string, IAutoView.IAutoViewChipProps["color"]> = {
    open: "green",
    closed: "red",
    // fallback
    default: "gray",
  ...
  if (Array.isArray(input.labels) && input.labels.length > 0) {
    const chips: IAutoView.IAutoViewChipProps[] = input.labels.map((lbl) => {
      // Labels can be strings or objects
      const name =
        typeof lbl === "string" ? lbl : lbl.name ?? String(lbl.id ?? "");
      return {
        type: "Chip",
        label: name,
        variant: "outlined",
