...
namespace Schema {
    export namespace IApiSearchIssues {
        export type GetResponse = {
            ...
            items: Schema.issue_search_result_item[];
        };
    }
    /**
...
type IAutoViewTransformerInputType = Schema.IApiSearchIssues.GetResponse;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  for (const issue of input.items) {
    ...
    const avatar: IAutoView.IAutoViewAvatarProps | undefined = user
      ? {
          type: "Avatar",
          src: user.avatar_url,
          name: userLogin,
    ...
    const labelChips: IAutoView.IAutoViewChipProps[] = (issue.labels || []).map(
      (lbl) => ({
        type: "Chip",
        label: lbl.name ?? "",
        variant: "outlined",
    ...
    const listItem: IAutoView.IAutoViewListItemProps = {
      type: "ListItem",
      title: issue.title,
      description: `#${issue.number} opened on ${createdDate}`,
      // show avatar if available
      ...(avatar ? { startElement: avatar } : {}),
