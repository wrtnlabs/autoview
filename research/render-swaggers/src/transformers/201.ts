...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const mapColor = (variant?: string): IAutoView.IAutoViewChipProps["color"] => {
  ...
  if (input.session) {
    ...
    const sessionParts: IAutoView.IAutoViewPresentationComponentProps[] = [];
  ...
  if (input.chatTags && input.chatTags.length > 0) {
    const chips = input.chatTags.map<IAutoView.IAutoViewChipProps>(tag => ({
      type: "Chip",
      label: tag.name,
  ...
  if (input.oneTimeMsg) {
    ...
    const parts: IAutoView.IAutoViewPresentationComponentProps[] = [];
    if (otm.name) {
      parts.push({ type: "Text", content: [`Name: ${otm.name}`] });
    }
    if (otm.state) {
      parts.push(
        { type: "Chip", label: otm.state, color: "info", variant: "outlined", size: "small" }
      );
    }
    items.push({
      type: "DataListItem",
  ...
  if (input.supportBot) {
    ...
    const botParts: IAutoView.IAutoViewPresentationComponentProps[] = [];
    if (bot.botName) {
      botParts.push({ type: "Text", content: [`Name: ${bot.botName}`] });
    }
    if (bot.state) {
      botParts.push(
        { type: "Chip", label: bot.state, color: bot.state === "active" ? "success" : "gray", size: "small" }
      );
    }
    items.push({
      type: "DataListItem",
  ...
  for (const key of Object.keys(input) as Array<keyof typeof input>) {
    if (!handledKeys.has(key)) {
      (rest as any)[key] = (input as any)[key];
    }
  }
