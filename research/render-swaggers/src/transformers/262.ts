...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const header: IAutoView.IAutoViewCardHeaderProps = {
    ...
    title: user?.name ?? userChat?.title ?? "Chat",
    // Display the chat state if available
    description: userChat?.state ? `Status: ${userChat.state}` : undefined,
  ...
  if (campaign) {
    ...
    contentChildren.push({
      type: "Chip",
      label: campaign.name,
      color: stateColorMap[campaign.state ?? ""] ?? "gray",
      size: "small",
    });
  }
  ...
  if (chatTags.length > 0) {
    ...
    const normalizeColor = (c?: string): IAutoView.IAutoViewChipProps["color"] => {
      switch (c) {
        case "olive":
          return "yellow";
        case "cobalt":
        ...
        default:
          ...
          return (c as any) ?? "gray";
      }
    };
    const tagChips: IAutoView.IAutoViewChipProps[] = chatTags.map((t) => ({
      type: "Chip",
      label: t.name,
  ...
  if (message?.files && message.files.length > 0) {
    const fileItems: IAutoView.IAutoViewDataListItemProps[] = message.files.map((f) => ({
      type: "DataListItem",
      label: [
        {
          type: "Text",
  ...
  if (userSession) {
    const sessionItems: IAutoView.IAutoViewDataListItemProps[] = [];
