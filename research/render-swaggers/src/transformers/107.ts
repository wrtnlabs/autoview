...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const firstThumbnailUrl: string | undefined = input.content.thumbnails?.[0]?.url;
  ...
  const categoryChips: IAutoView.IAutoViewChipProps[] = (input.categories || []).map(cat => ({
    type: "Chip",
    label: cat.name,
    variant: "outlined"
  }));
  ...
  const tagChips: IAutoView.IAutoViewChipProps[] = (input.tags || []).map(t => ({
    type: "Chip",
    label: t,
    variant: "filled",
    color: "info"
  ...
  const cardMedia: IAutoView.IAutoViewCardMediaProps | undefined = firstThumbnailUrl
    ? {
        type: "CardMedia",
