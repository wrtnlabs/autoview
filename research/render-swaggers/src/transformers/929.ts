...
type IAutoViewTransformerInputType = Schema.IApiUserCodespaces.GetResponse;
...
You don't have any active codespaces at the moment.`
    ...
    const stateToColor: Record<string, IAutoView.IAutoViewChipProps["color"]> = {
        Available: "success",
    ...
    function makeListItem(cs: Schema.codespace): IAutoView.IAutoViewListItemProps {
        const owner = cs.owner;
        // Determine chip color by state, default to 'primary' if unknown
        const chipColor = stateToColor[cs.state] ?? "primary";

        return {
            type: "ListItem",
            title: cs.name,
            description: cs.display_name ?? undefined,
