...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const items: IAutoView.IAutoViewDataListItemProps[] = input.map((repo) => {
    // Owner avatar
    ...
    const titleLink: IAutoView.IAutoViewMarkdownProps = {
      type: "Markdown",
      content: `[**${repo.full_name}**](${repo.html_url})`,
    };

    // Optional description as Markdown for better readability
    const descriptionMd: IAutoView.IAutoViewMarkdownProps | undefined = repo.description
      ? {
          type: "Markdown",
