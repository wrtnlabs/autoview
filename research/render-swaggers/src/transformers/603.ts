...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const conclusionKey = input.conclusion ?? input.status ?? 'pending';
  const chipColor = conclusionColorMap[conclusionKey] ?? 'info';

  // Build the CardHeader: avatar + title + conclusion chip
  const header: IAutoView.IAutoViewCardHeaderProps = {
    type: 'CardHeader',
    title: input.display_title,
    description: `Run #${input.run_number}${
      input.run_attempt && input.run_attempt > 1 ? ` (attempt ${input.run_attempt})` : ''
    }`,
  ...
  const detailsList: IAutoView.IAutoViewDataListProps = {
    ...
    childrenProps: [
      ...
      },
      {
        ...
        value: [
          { type: 'Text', content: input.head_branch ?? 'unknown', variant: 'body2' },
        ],
      },
      {
        type: 'DataListItem',
      ...
      },
      {
        ...
        value: [
          {
            type: 'Text',
            content: new Date(input.run_started_at ?? input.created_at).toLocaleString(),
            variant: 'body2',
          },
        ],
      },
