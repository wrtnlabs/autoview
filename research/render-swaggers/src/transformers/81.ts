...
type IAutoViewTransformerInputType = Schema.IShoppingOrder;
...
function visualizeData(
  ...
  const customerChip: IAutoView.IAutoViewChipProps = {
    type: "Chip",
  ...
  const goodsItems: IAutoView.IAutoViewDataListItemProps[] = input.goods.map(
    (good) => ({
      type: "DataListItem",
      label: {
        type: "Text",
      ...
      value: {
        type: "Text",
        content: `Qty: ${good.volume}, ${formatCurrency(
          good.price.real
        )}`,
        variant: "body2",
      },
  ...
  const goodsComponent: IAutoView.IAutoViewComponentProps =
    goodsItems.length > 0
      ? {
          type: "DataList",
  ...
  const couponsUsed = input.price.ticket_payments?.length ?? 0;
  const couponChip: IAutoView.IAutoViewChipProps = {
    type: "Chip",
