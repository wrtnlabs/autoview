...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const items: IAutoView.IAutoViewListItemProps[] = input.map((repo) => {
    ...
    const startElement: IAutoView.IAutoViewAvatarProps = {
      type: "Avatar",
    ...
    const listItem: IAutoView.IAutoViewListItemProps = {
      type: "ListItem",
      title: repo.name,
      // Only include description if it's non-null and non-empty
      ...(repo.description ? { description: repo.description } : {}),
      startElement,
      // Attach repository page link so the item is clickable
      ...(repo.html_url ? { href: repo.html_url } : {}),
      // Attach the stats chips if any were created
      ...(statChips.length > 0 ? { endElement: statChips } : {}),
    };
