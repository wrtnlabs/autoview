...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const header: IAutoView.IAutoViewCardHeaderProps = {
    ...
    description: input.userChat?.state ? `State: ${input.userChat.state}` : undefined,
  };
  ...
  if (input.session) {
    listItems.push({
      ...
      value: [{ type: "Text", content: String(input.session.unread ?? 0) }],
    });
  }

  if (input.message) {
    // Use Markdown for message content to preserve formatting
    const text = input.message.plainText ?? "(no message text)";
    listItems.push({
      type: "DataListItem",
  ...
  const footerChips: IAutoView.IAutoViewChipProps[] = (input.chatTags ?? []).map((tag) => ({
    type: "Chip",
    label: tag.name,
