...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const customerInfoMarkdown = [
    ...
    `- **Name**: ${input.customer.citizen?.name || "Anonymous"}`,
    `- **Channel**: ${input.customer.channel.name}`,
    `- **Referrer**: ${input.customer.referrer || "N/A"}`,
  ].join("\n");
  const customerBlock: IAutoView.IAutoViewMarkdownProps = {
    type: "Markdown",
    content: customerInfoMarkdown,
  };

  // Build a data list of ordered goods
  const goodsList: IAutoView.IAutoViewDataListProps = {
    type: "DataList",
    childrenProps: input.goods.map((good) => {
      ...
      const volumeChip: IAutoView.IAutoViewChipProps = {
        type: "Chip",
        label: `${good.volume} pcs`,
        color: "secondary",
        size: "small",
      };
      const priceChip: IAutoView.IAutoViewChipProps = {
        type: "Chip",
  ...
  const summaryBlock: IAutoView.IAutoViewMarkdownProps = {
    type: "Markdown",
