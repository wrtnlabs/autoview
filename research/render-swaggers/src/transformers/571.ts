...
type IAutoViewTransformerInputType = Schema.full_repository;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const makeMetricItem = (
    label: string,
    value: number,
    iconId: string,
    iconColor: IAutoView.IAutoViewIconProps["color"]
  ...
  const metricsItems: IAutoView.IAutoViewDataListItemProps[] = [
  ...
  const topicChips: IAutoView.IAutoViewChipProps[] = (input.topics ?? []).map((topic) => ({
    type: "Chip",
    label: topic,
    variant: "outlined",
    size: "small",
  ...
  const chipGroup: IAutoView.IAutoViewChipGroupProps | undefined =
    topicChips.length > 0
      ? {
          type: "ChipGroup",
  ...
  const homepageButton: IAutoView.IAutoViewButtonProps | undefined =
    input.homepage
      ? {
          type: "Button",
          label: "Homepage",
  ...
  const contentChildren: IAutoView.IAutoViewPresentationComponentProps[] = [dataList];
  if (chipGroup) contentChildren.push(chipGroup);
  ...
  const footerButtons: IAutoView.IAutoViewPresentationComponentProps[] = [viewButton];
  if (homepageButton) footerButtons.push(homepageButton);
