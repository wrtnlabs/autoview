...
type IAutoViewTransformerInputType = Schema.commit_activity[];
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
    ...
    function getColorForCount(count: number): IAutoView.IAutoViewIconProps["color"] {
    ...
    const dataListItems: IAutoView.IAutoViewDataListItemProps[] = input.map(week => {
        ...
        const dayIcons: IAutoView.IAutoViewIconProps[] = week.days.map((count, idx) => ({
            type: "Icon",
            id: "circle",
            size: 8,
