...
type IAutoViewTransformerInputType = Schema.issue;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
    ...
    const header: IAutoView.IAutoViewCardHeaderProps = {
        type: "CardHeader",
        title: input.title,
        description: `#${input.number} opened on ${new Date(input.created_at).toLocaleDateString()} by ${input.user?.login || "unknown"}`,
        startElement: {
            type: "Avatar",
    ...
    const stateChips: IAutoView.IAutoViewChipProps[] = [];
    ...
    const labelChips: IAutoView.IAutoViewChipProps[] = [];
    ...
    const totalReactions = input.reactions?.total_count ?? 0;
    listItems.push({
        type: "DataListItem",
        label: [
            {
        ...
        childrenProps: [
            header,
            {
                ...
                childrenProps: [
                    ...
                    ...(bodyMarkdown ? [bodyMarkdown as IAutoView.IAutoViewMarkdownProps] : []),
                    // Show key metrics
                    dataList,
                ],
            },
