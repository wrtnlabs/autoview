...
type IAutoViewTransformerInputType = Schema.IApiOrgsActionsVariablesRepositories.GetResponse;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const repoItems: IAutoView.IAutoViewListItemProps[] = input.repositories.map(repo => {
    ...
    const starsChip: IAutoView.IAutoViewChipProps = {
      type: "Chip",
      label: String(repo.stargazers_count ?? 0),
      startElement: {
        type: "Icon",
        id: "star",
        color: "yellow",
    ...
    const forksCount = repo.forks_count ?? repo.forks ?? 0;
