...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const severityColor =
    ...
    }[value.severity as string] || "bg-gray-100 text-gray-800";
  ...
  const renderStateIcon = (): JSX.Element => {
    switch (value.state) {
      case "published":
      ...
      case "draft":
        return <LucideReact.Clock className="text-blue-500" size={16} />;
      case "triage":
        return <LucideReact.HelpCircle className="text-yellow-500" size={16} />;
      default:
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4">
      {/* Header: summary, severity badge, GHSA/CVE IDs */}
      <div className="flex items-start justify-between">
        <div className="flex-1 pr-4">
          ...
          <div
            className={`inline-flex items-center px-2 py-0.5 mt-1 text-xs font-medium ${severityColor} rounded-full`}
          >
            {severityLabel}
          </div>
        </div>
        <div className="flex flex-col items-end text-sm text-gray-500 space-y-0.5 whitespace-nowrap">
          <div>{value.ghsa_id}</div>
          {value.cve_id && <div>{value.cve_id}</div>}
        </div>
      </div>

      {/* Metadata: CVSS, state, published date */}
      <div className="flex flex-wrap gap-4 text-sm text-gray-600">
        {value.cvss?.score != null && (
          <div className="flex items-center">
            <LucideReact.Star className="text-yellow-500 mr-1" size={16} />
            <span>CVSS: {value.cvss.score}/10</span>
          </div>
        )}
        <div className="flex items-center">
          {renderStateIcon()}
        ...
        {value.published_at && (
          <div className="flex items-center">
            ...
            <span>Published: {formatDate(value.published_at)}</span>
          </div>
        )}
      </div>
      ...
      {/* Impacted packages count */}
      {value.vulnerabilities && value.vulnerabilities.length > 0 && (
        <div className="text-sm text-gray-600">
          Impacted packages: {value.vulnerabilities.length}
        </div>
      )}

      {/* CWE identifiers */}
      {value.cwes && value.cwes.length > 0 && (
      ...
      <div className="flex justify-between items-center">
        {value.author && (
          <div className="flex items-center space-x-2">
            ...
            />
            <span className="text-sm text-gray-700">{value.author.login}</span>
          </div>
        )}
        {value.publisher && (
          <div className="flex items-center space-x-2">
