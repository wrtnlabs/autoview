...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const writerRole = writer.type.charAt(0).toUpperCase() + writer.type.slice(1);
  const writerName = writer.member?.nickname ?? writer.id;
  const snapshots = value.snapshots;
  const latestSnapshot = snapshots[snapshots.length - 1];
  const editCount = snapshots.length > 1 ? snapshots.length - 1 : 0;

  const formattedCommentDate = new Date(value.created_at).toLocaleString(
    undefined,
    {
      dateStyle: "medium",
      timeStyle: "short",
  ...
  function formatFileName(file: (typeof latestSnapshot.files)[number]) {
    ...
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <span className="font-medium text-gray-900">{writerName}</span>
            <span className="text-gray-500 text-sm">{writerRole}</span>
          </div>
        </div>
        <div className="flex items-center gap-1 text-gray-400 text-xs">
          <LucideReact.Calendar size={14} />
          <time dateTime={value.created_at}>{formattedCommentDate}</time>
        </div>
      </div>
      ...
      {latestSnapshot && (
        <div className="space-y-2">
          <p className="text-gray-800 text-sm overflow-hidden line-clamp-3 whitespace-pre-wrap">
            {latestSnapshot.body}
          </p>
          <div className="flex items-center gap-1 text-gray-400 text-xs">
            <LucideReact.Calendar size={14} />
            <time dateTime={latestSnapshot.created_at}>
              {formattedSnapshotDate}
            </time>
            {latestSnapshot.format && (
              <span className="ml-2 px-1.5 py-0.5 bg-gray-100 text-gray-500 text-[10px] uppercase rounded">
                {latestSnapshot.format}
              </span>
            )}
          </div>
