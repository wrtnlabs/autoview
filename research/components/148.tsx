...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const contentAt = latestSnapshot ? formatDate(latestSnapshot.created_at) : '';

  // Determine writer type and display name
  type Writer = typeof value.writer;
  const writerType = (value.writer as Writer).type;
  let writerName = '';
  if (writerType === 'administrator' || writerType === 'seller') {
    // @ts-ignore
    writerName = value.writer.member.nickname;
  } else if (writerType === 'customer') {
    // @ts-ignore
    writerName =
      value.writer.member?.nickname ??
      value.writer.citizen?.name ??
      'Customer';
  }
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm border border-gray-200 max-w-full">
      ...
      {value.parent_id && (
        <div className="flex items-center gap-1 mt-2 text-sm text-blue-500">
          <LucideReact.CornerUpLeft size={16} />
          <span className="font-medium">Reply to comment</span>
        </div>
      )}

      {/* Main content */}
      <div className="mt-4 text-gray-700 text-sm overflow-hidden line-clamp-3">
      ...
      {editCount > 0 && (
        <div className="flex items-center gap-1 mt-3 text-sm text-gray-500">
          <LucideReact.History size={16} className="text-gray-400" />
          <span>Edited {editCount} {editCount === 1 ? 'time' : 'times'}</span>
        </div>
      )}
    </div>
  );
