...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Derived constants for writer info
  const writer = value.writer;
  const writerType = writer.type; // "administrator" | "seller" | "customer"
  const writerName = (() => {
    // Prefer real name if verified
    if ("citizen" in writer && writer.citizen) {
      return writer.citizen.name;
    }
    // Fallback to member nickname if available
    if ("member" in writer && writer.member && "nickname" in writer.member) {
      return writer.member.nickname;
    }
    // Last resort: shorten the ID
    return writer.id.slice(0, 8);
  })();
  const writerLabel = writerType.charAt(0).toUpperCase() + writerType.slice(1);
  const formattedDate = new Date(value.created_at).toLocaleString();

  // 2. Pick the latest snapshot as current content
  const latestSnapshot = value.snapshots[value.snapshots.length - 1];
  const contentFormat = latestSnapshot.format;
  const contentBody = latestSnapshot.body;
  const attachments = latestSnapshot.files;

  // 3. Render
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      {/* Header: writer avatar, name, role, timestamp */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center space-x-2">
          {writerType === "administrator" ? (
            <LucideReact.ShieldCheck size={20} className="text-blue-500" />
          ) : writerType === "seller" ? (
            <LucideReact.ShoppingCart size={20} className="text-green-500" />
          ) : (
            <LucideReact.User size={20} className="text-gray-500" />
          )}
          <span className="font-semibold text-gray-900">{writerName}</span>
      ...
      <div className="mb-3">
        {contentFormat === "html" ? (
          <div
            className="text-gray-800 text-sm line-clamp-3 overflow-hidden"
      ...
      {value.snapshots.length > 1 && (
        <div className="flex items-center text-gray-500 text-sm mt-4">
          ...
          <span className="ml-1">
            Edited {value.snapshots.length - 1} time
            {value.snapshots.length - 1 > 1 ? "s" : ""}
          </span>
        </div>
      )}
    </div>
