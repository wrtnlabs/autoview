...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Derive display name and user type
  ...
  if ("type" in writer) {
    if (writer.type === "administrator") {
      ...
      userType = "Customer";
      displayName =
        writer.member?.nickname ?? writer.citizen?.name ?? "Customer";
    }
  }
  const formattedDate = new Date(latestSnapshot.created_at).toLocaleString();
  const attachments = (latestSnapshot.files || []).map(
    (file) => file.name + (file.extension ? `.${file.extension}` : ""),
  );
      ...
      {attachments.length > 0 && (
        <ul className="flex flex-wrap gap-3">
          {attachments.map((fileName, idx) => (
            <li key={idx} className="flex items-center text-xs text-blue-600">
