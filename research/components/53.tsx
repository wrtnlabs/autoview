...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const hasMember = value.member !== null;
  const hasCitizen = value.citizen !== null;
  const hasExternal = value.external_user !== null;

  // Derive email display for member
  let emailDisplay = "";
  if (hasMember && value.member!.emails.length > 0) {
    const emails = value.member!.emails.map((e) => e.value);
    emailDisplay =
      emails.length > 2
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm max-w-md mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700">
        ...
        <div className="flex items-center">
          ...
          <span className="truncate">
            {value.referrer && value.referrer.length > 0 ? value.referrer : "—"}
          </span>
        </div>
        <div className="flex items-center">
          <LucideReact.Globe size={16} className="text-gray-500 mr-2" />
          <span>{value.ip}</span>
        </div>
      </div>
      ...
      <div className="mt-6 border-t pt-4 text-gray-700">
        ...
        <div className="space-y-2">
          {hasCitizen && (
            <div className="flex items-center">
              <LucideReact.Phone size={16} className="text-gray-500 mr-2" />
          ...
          {hasMember && (
            <div className="flex items-center">
              <LucideReact.UserCheck size={16} className="text-blue-500 mr-2" />
              <span>Member: {value.member!.nickname}</span>
            </div>
          )}
          {hasMember && emailDisplay && (
            <div className="flex items-center">
              <LucideReact.Mail size={16} className="text-gray-500 mr-2" />
              <span>{emailDisplay}</span>
            </div>
          )}
          {hasExternal && (
            <div className="flex items-center">
              <LucideReact.User size={16} className="text-indigo-500 mr-2" />
              <span>
                External: {value.external_user!.application} /{" "}
                {value.external_user!.uid}
              </span>
            </div>
          )}
          {!hasCitizen && !hasMember && !hasExternal && (
            <div className="flex items-center text-gray-500">
              <LucideReact.AlertCircle size={16} className="mr-2" />
              <span>No authenticated identity</span>
            </div>
          )}
        </div>
      </div>
