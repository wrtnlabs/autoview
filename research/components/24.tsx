...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const pageInfo = `Page ${pagination.current} of ${pagination.pages} (Total ${pagination.records} comments)`;
  const formatDate = (iso: string) => new Date(iso).toLocaleString();

  // Helper to render writer information with appropriate icon, name, and role badge.
  const renderWriterInfo = (
    writer: AutoViewInputSubTypes.IShoppingSaleInquiryComment["writer"]
  ) => {
    ...
    switch (writer.type) {
      case "seller":
        icon = <LucideReact.ShoppingCart className="text-blue-500" size={16} />;
        role = "Seller";
        name = writer.member.nickname;
        break;
      case "customer":
        icon = <LucideReact.User className="text-gray-500" size={16} />;
        role = "Customer";
        name = writer.member?.nickname ?? writer.citizen?.name ?? "Guest";
        break;
      case "administrator":
    ...
    return (
      <div className="flex items-center text-sm font-medium text-gray-800">
        ...
        <span className="ml-2 px-1.5 py-0.5 text-xs bg-gray-200 rounded">{role}</span>
      </div>
    );
  };

  // Render a single comment card, showing only the latest snapshot and metadata.
  const renderComment = (comment: AutoViewInputSubTypes.IShoppingSaleInquiryComment) => {
    ...
    return (
      ...
      >
        {renderWriterInfo(comment.writer)}
        <div className="mt-1 flex items-center text-xs text-gray-500">
          <LucideReact.Calendar size={14} />
          <span className="ml-1">{formatDate(latest.created_at)}</span>
          {comment.snapshots.length > 1 && (
            <span className="ml-2 italic text-gray-400">(edited)</span>
          )}
        </div>
        <p className="mt-2 text-gray-700 text-sm leading-relaxed overflow-hidden line-clamp-3">
          {latest.body}
        </p>
      </div>
    );
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-lg">
      <div className="flex items-center mb-4 text-sm text-gray-600">
        ...
        <span className="ml-2">{pageInfo}</span>
      </div>
