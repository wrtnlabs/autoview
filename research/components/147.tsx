...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const pageInfo = `Page ${current} of ${pages} (${records} comments)`;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-full">
      {/* Pagination Info */}
      <div className="text-sm text-gray-600 mb-4">{pageInfo}</div>
              ...
              {latest.files.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {latest.files.map((file, idx) => {
                    const name = file.name + (file.extension ? `.${file.extension}` : "");
                    return (
                      <a
                        key={idx}
                        href={file.url}
                        ...
                        className="text-blue-500 hover:underline text-xs bg-gray-100 px-2 py-1 rounded"
                      >
                        {name}
                      </a>
                    );
