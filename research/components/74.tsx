...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const pageInfo = `Page ${pagination.current} of ${pagination.pages} (${pagination.records} items)`;
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="space-y-4">
        {data.map((item) => {
          ...
          const thumbnailUrl =
            item.sale.content.thumbnails[0]?.url ||
            "https://placehold.co/300x200/f8fafc/475569?text=Image";

          // Derived display values
          const displayTitle = item.sale.content.title;
          const sectionName = item.sale.section.name;
          const sellerName = item.sale.seller.member.nickname;
          const totalReal = item.price.real * item.volume;
          const totalNominal = item.price.nominal * item.volume;

          return (
            <div
              key={item.id}
              className="flex flex-col md:flex-row bg-white rounded-lg shadow p-4"
            >
              {/* Thumbnail */}
              <div className="w-full md:w-1/4 h-32 md:h-24 rounded-md overflow-hidden bg-gray-100 flex-shrink-0">
                <img
                  src={thumbnailUrl}
                  alt={displayTitle}
              ...
              {/* Details */}
              <div className="flex-1 mt-2 md:mt-0 md:ml-4 grid grid-cols-1 sm:grid-cols-2 gap-x-4">
                {/* Left Section: Title, Section, Seller */}
                <div>
                ...
                {/* Right Section: Pricing and Dates */}
                <div className="text-sm text-gray-700 space-y-1">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium">
                      ${item.price.real.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex items-center gap-1">
                    <LucideReact.Layers
