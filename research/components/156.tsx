...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Flatten all stocks across units to compute price range
  const allStocks = value.units.flatMap((unit) => unit.stocks);
  const realPrices = allStocks.map((s) => s.price.real);
  const priceRange =
    realPrices.length > 0
      ? realPrices.length === 1
        ? realPrices[0].toLocaleString()
        : `${Math.min(...realPrices).toLocaleString()} – ${Math.max(
            ...realPrices,
          ).toLocaleString()}`
      : "N/A";

  // Prepare textual snippet from markdown/plain body
  const rawBody = value.content.body.replace(/[#_*>\-`~]/g, "");
  const descriptionSnippet =
    rawBody.length > 120 ? rawBody.slice(0, 120).trim() + "..." : rawBody;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md flex flex-col md:flex-row gap-4">
      <div className="w-full md:w-1/3 aspect-[4/3] bg-gray-100 overflow-hidden rounded">
        <img
          src={thumbnailUrl}
      ...
      <div className="flex-1 flex flex-col justify-between">
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between text-gray-600 text-sm">
