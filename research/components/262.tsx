...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const statusLabel = value.userChat?.state
    ? statusMap[value.userChat.state] ?? value.userChat.state
    : "Unknown";
  const online = Boolean(value.userOnline?.id);
  const priorityMap: Record<string, string> = {
    high: "High",
    medium: "Medium",
    low: "Low",
  };
  const priorityKey = value.userChat?.priority;
  const priorityLabel = priorityKey
    ? priorityMap[priorityKey] ?? priorityKey
    : "";
  const lastMessage = value.message?.plainText;
  const messageTime = value.message?.createdAt
  ...
  const campaignState = value.campaign?.state;
  const oneTimeMsgName = value.oneTimeMsg?.name;
  const oneTimeMsgState = value.oneTimeMsg?.state;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      {/* Header: Avatar, Name, Online/Status, Priority */}
      ...
      {/* Footer: Unread, Campaign, One-Time Msg */}
      {(unreadCount > 0 || campaignName || oneTimeMsgName) && (
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {campaignName && (
            <span
              className={`px-2 py-1 text-xs font-medium rounded ${
                statusColorMap[campaignState ?? ""] ?? "bg-gray-100 text-gray-800"
              }`}
            >
              {campaignName}
            </span>
          ...
          {oneTimeMsgName && (
            <span
              className={`px-2 py-1 text-xs font-medium rounded ${
                statusColorMap[oneTimeMsgState ?? ""] ??
                "bg-gray-100 text-gray-800"
              }`}
            >
              {oneTimeMsgName}
