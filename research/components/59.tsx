...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const issuedDate = new Date(value.created_at);
  const formattedIssued = issuedDate.toLocaleString(undefined, {
    year: "numeric",
    month: "short",
    day: "numeric",
    hour: "2-digit",
  ...
  if (!expiredAt) {
    expireLabel = "No expiration";
  } else if (expiredAt.getTime() <= now) {
    expireLabel = "Expired";
  } else {
    const formattedExp = expiredAt.toLocaleDateString(undefined, {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
    const daysLeft = Math.ceil(
      (expiredAt.getTime() - now) / (1000 * 60 * 60 * 24),
    );
    expireLabel = `${formattedExp} (in ${daysLeft} day${daysLeft > 1 ? "s" : ""})`;
  }

  // Customer display name
  const displayCustomerName =
    value.customer.member?.nickname ?? value.customer.citizen?.name ?? "Guest";

  // Discount display
  const { discount } = value.coupon;
  ...
  const thresholdLabel =
    discount.threshold !== null
      ? `Min. spend: $${discount.threshold.toLocaleString()}`
      : null;
  const limitLabel =
    isAmount && discount.limit !== null
      ? `Max discount: $${discount.limit.toLocaleString()}`
      : null;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="mt-4 border-t pt-3">
        ...
        <div className="space-y-1 text-sm text-gray-600">
          ...
          {value.customer.citizen?.mobile && (
            <div className="flex items-center gap-1">
              <LucideReact.Phone size={16} className="text-gray-500" />
              <span>{value.customer.citizen.mobile}</span>
            </div>
          )}
          <div className="flex items-center gap-1">
      ...
      {/* Discount Details */}
      <div className="mt-4 border-t pt-3">
        <h3 className="text-gray-700 font-medium mb-2">Discount</h3>
        <div className="flex items-center gap-2 text-sm text-gray-600">
          <LucideReact.Tag size={16} className="text-blue-500" />
          <span className="font-medium">{discountValue}</span>
        </div>
        <div className="mt-1 text-xs text-gray-500 space-y-0.5">
          {thresholdLabel && <div>{thresholdLabel}</div>}
          {limitLabel && <div>{limitLabel}</div>}
        </div>
      </div>
    </div>
  );
