...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const issuedAt = new Date(value.created_at).toLocaleString();
  const expiresAt = value.expired_at
    ? new Date(value.expired_at).toLocaleString()
    : "Never";

  const { coupon, customer } = value;
  const access = coupon.restriction.access;
  const exclusive = coupon.restriction.exclusive;
  ...
  const exclusiveBadgeClasses = exclusive
  ...
  const totalVolume =
    coupon.inventory.volume == null ? "Unlimited" : coupon.inventory.volume;
  const perCitizenVolume =
    coupon.inventory.volume_per_citizen == null
      ? "Unlimited"
      : coupon.inventory.volume_per_citizen;

  const openedAt = coupon.opened_at
    ? new Date(coupon.opened_at).toLocaleString()
    : "N/A";
  const closedAt = coupon.closed_at
    ? new Date(coupon.closed_at).toLocaleString()
    : "N/A";
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
        <h2 className="text-lg font-semibold text-gray-800">{coupon.name}</h2>
        <div className="flex space-x-2 mt-2 sm:mt-0">
          <span
            className={`px-2 py-1 text-xs font-medium rounded ${accessBadgeClasses}`}
          >
            {access === "public" ? "Public" : "Private"}
          </span>
          <span
            className={`px-2 py-1 text-xs font-medium rounded ${exclusiveBadgeClasses}`}
          >
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm text-gray-600">
        ...
        <div>
          ...
          {totalVolume} total, {perCitizenVolume} per user
        </div>
        <div className="sm:col-span-2">
          <span className="font-medium text-gray-800">Valid window:</span>{" "}
          {openedAt} – {closedAt}
        </div>
      </div>
    </div>
