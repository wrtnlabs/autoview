...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Safely truncate text to a given length
  const truncate = (text: string | undefined, max = 50): string =>
    text && text.length > max ? text.slice(0, max).trimEnd() + "…" : text || "";

  // Derive a human-friendly description for the event payload
  const getDescription = (evt: AutoViewInputSubTypes.event): string => {
    const p = evt.payload;
    const repoName = evt.repo?.name ?? "repository";
    if (p.action && p.issue) {
      return `${p.action.charAt(0).toUpperCase() + p.action.slice(1)} issue "${truncate(
        p.issue.title,
        40,
      )}" on ${repoName}`;
    }
    if (p.action && p.comment) {
      const snippet = truncate(p.comment.body_text ?? p.comment.body ?? "", 40);
      return `${p.action.charAt(0).toUpperCase() + p.action.slice(1)} comment "${snippet}" on ${repoName}`;
    }
    if (p.pages && p.pages.length > 0) {
      return `Updated ${p.pages.length} page${p.pages.length > 1 ? "s" : ""} on ${repoName}`;
    }
    if (evt.type) {
  ...
  const getAvatarPlaceholder = (login: string): string =>
  ...
  if (!value || value.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 bg-white rounded-lg shadow">
        <LucideReact.AlertCircle size={48} className="text-gray-300 mb-3" />
        <span className="text-gray-500">No events to display</span>
      </div>
    );
  }
    ...
    <ul className="space-y-4">
      {value.map((evt) => {
        ...
        return (
          <li
            key={evt.id}
            className="flex space-x-3 bg-white p-4 rounded-lg shadow-sm"
          >
            <div className="flex-shrink-0">
              <img
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.src = getAvatarPlaceholder(actor.login);
                }}
