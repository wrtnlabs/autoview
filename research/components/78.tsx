...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const orderDate = new Date(value.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' });
  let paymentStatus = 'Pending';
  let statusIcon: React.ReactNode = <LucideReact.Clock className="text-amber-500" size={16} />;
  if (value.publish) {
    if (value.publish.cancelled_at) {
      paymentStatus = 'Cancelled';
      statusIcon = <LucideReact.XCircle className="text-red-500" size={16} />;
    } else if (value.publish.paid_at) {
      paymentStatus = 'Paid';
  ...
  const goodsCount = value.goods.length;
  const formattedTotal = new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' }).format(value.price.real);
    ...
    <div className="p-6 bg-white rounded-lg shadow-md max-w-md mx-auto">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center space-x-1">
          {statusIcon}
          <span className="text-sm font-medium text-gray-700">{paymentStatus}</span>
        </div>
      </div>

      <div className="mb-4">
        <h3 className="text-md font-medium text-gray-800 mb-2">
          {goodsCount} {goodsCount > 1 ? 'Items' : 'Item'}
        </h3>
        <ul className="divide-y divide-gray-100">
          {value.goods.map((good) => {
            const title = good.commodity.sale.content.title;
            const thumb = good.commodity.sale.content.thumbnails[0]?.url || 'https://placehold.co/80x80?text=No+Image';
            const linePrice = good.price.real;
            const formattedLinePrice = new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' }).format(linePrice);
            return (
              <li key={good.id} className="flex items-center py-2">
                ...
                <div className="ml-4 flex-1">
                  ...
                  <p className="text-sm text-gray-500">Qty: {good.volume}</p>
                </div>
                <div className="ml-4 text-sm font-medium text-gray-800">{formattedLinePrice}</div>
              </li>
            );
          })}
        </ul>
      ...
      <div className="flex justify-between items-center border-t border-gray-200 pt-4">
        ...
        <span className="text-lg font-semibold text-gray-800">{formattedTotal}</span>
      </div>

      {value.publish && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          <h3 className="text-md font-medium text-gray-800 mb-2">Shipping Address</h3>
          <p className="text-sm text-gray-700">{value.publish.address.name}</p>
          <p className="text-sm text-gray-600 truncate">
            {value.publish.address.possession}, {value.publish.address.city}, {value.publish.address.country}
          </p>
        </div>
      )}
    </div>
