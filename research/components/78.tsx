...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const orderDate = new Date(value.created_at).toLocaleDateString(undefined, {
    year: "numeric",
    month: "short",
    day: "numeric",
  });

  const isPaid = Boolean(value.publish?.paid_at);
  const paidDate = value.publish?.paid_at
    ? new Date(value.publish.paid_at).toLocaleDateString(undefined, {
        year: "numeric",
        month: "short",
        day: "numeric",
      })
  ...
  const totalItems = value.goods.reduce((sum, good) => sum + good.volume, 0);
  const totalReal = value.price.real;
  const totalNominal = value.price.nominal;
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="text-right">
          ...
          <div className="flex items-center text-sm mt-1">
            ...
            <span className="ml-1">{isPaid ? "Paid" : "Pending"}</span>
          </div>
        </div>
      </div>
      ...
      <ul className="divide-y divide-gray-100 mb-4">
        {value.goods.map((good) => {
          ...
          const thumb =
            ...
            "https://placehold.co/80x80/f8fafc/475569?text=Product";
          const itemTotal = good.price.real;
          const unitPrice = itemTotal / good.volume;
          return (
            <li key={good.id} className="flex items-center py-2">
              ...
              <div className="flex-1 text-sm">
                ...
                <div className="text-gray-600">
                  Qty {good.volume} × {formatCurrency(unitPrice)}
                </div>
              </div>
      ...
      {/* Summary */}
      <div className="border-t border-gray-200 pt-4 mb-4 text-sm space-y-1">
      ...
      {value.publish?.address && (
        <div className="flex items-start text-sm text-gray-600 mb-2">
          <LucideReact.MapPin size={16} className="mr-1 mt-0.5" />
          <div className="leading-tight">
            <div className="font-medium text-gray-800">
              {value.publish.address.name}
            </div>
            <div>
              {value.publish.address.city}, {value.publish.address.country}
            </div>
          </div>
        </div>
      )}
