...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const orderName = value.name;
  const orderDate = formatDate(value.created_at);
  const channelName = value.customer?.channel?.name ?? '—';
  const itemCount = value.goods?.length ?? 0;
  const totalAmount = value.price?.real ?? 0;
    ...
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
          <h2 className="text-lg font-semibold text-gray-800 truncate">{orderName}</h2>
          <time
            dateTime={value.created_at}
            className="mt-1 text-sm text-gray-500 sm:mt-0 sm:ml-4"
          >
            {orderDate}
          </time>
        </div>
        <p className="mt-1 text-sm text-gray-600">Channel: {channelName}</p>
      </div>

      {/* Summary */}
      <div className="px-4 py-3 grid grid-cols-2 gap-4 text-sm text-gray-700">
        <div>
          <div className="font-medium">Items</div>
          <div className="mt-1">{itemCount}</div>
        </div>
        <div>
          <div className="font-medium">Total</div>
          <div className="mt-1">{formatCurrency(totalAmount)}</div>
        </div>
      </div>

      {/* Goods List */}
      <ul className="divide-y divide-gray-100">
        {value.goods?.map((good, idx) => {
          const title = good.commodity?.sale?.content?.title ?? 'Unnamed item';
          const thumbUrl =
            good.commodity?.sale?.content?.thumbnails?.[0]?.url ?? '';
          const quantity = good.volume ?? 0;
          const price = good.price?.real ?? 0;

          return (
            <li key={idx} className="flex items-center px-4 py-3 hover:bg-gray-50">
              {/* Thumbnail */}
              {thumbUrl ? (
                <img
                  src={thumbUrl}
                ...
                <div className="w-12 h-12 flex-shrink-0 rounded bg-gray-200 flex items-center justify-center text-gray-400">
                  📦
                </div>
              )}
              {/* Details */}
