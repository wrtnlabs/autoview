...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Data aggregation and formatting functions
  const { pagination, data: orders } = value;
  const { current, pages, records } = pagination;

  const pageSummary = `Page ${current} of ${pages} (${orders.length} shown, ${records} total)`;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow">
      ...
      <ul className="divide-y divide-gray-200">
        {orders.map((order) => {
          const itemCount = order.goods.reduce(
            (sum, g) => sum + (g.volume ?? 0),
            0,
          );
          const { nominal, real } = order.price;
          const isDiscounted = nominal > real;
          const paid = order.publish?.paid_at != null;
          ...
          return (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 flex flex-wrap items-center text-gray-500 text-sm gap-4">
                  ...
                  <div className="flex items-center">
                    {paid ? (
                      ...
                      <LucideReact.Clock size={14} className="text-amber-500" />
                    )}
                    <span className="ml-1">{paid ? "Paid" : "Pending"}</span>
                  </div>
                </div>
              </div>
              <div className="mt-3 sm:mt-0 sm:ml-6 flex items-baseline space-x-2">
                {isDiscounted && (
