...
export namespace AutoViewInputSubTypes {
  export namespace IPageIShoppingDelivery {
    ...
    export type IInvert = {
       ...
       */
      data: AutoViewInputSubTypes.IShoppingDelivery.IInvert[];
    };
  }
  ...
  export namespace IShoppingDelivery {
    ...
    export type IInvert = {
       ...
       */
      orders: AutoViewInputSubTypes.IShoppingOrder.IInvertFromDelivery[];
      /**
       * Primary Key.
       *
       * @title Primary Key
  ...
  export namespace IShoppingOrder {
    ...
    export type IInvertFromDelivery = {
       ...
       */
      publish: null | AutoViewInputSubTypes.IShoppingOrderPublish.IInvertFromDelivery;
      /**
       * Creation time of the record.
       *
       * @title Creation time of the record
...
export type AutoViewInput =
  AutoViewInputSubTypes.IPageIShoppingDelivery.IInvert;

// The component name must always be "VisualComponent"
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Define data aggregation/transformation functions or derived constants if necessary.
  const { pagination, data: deliveries } = value;
  ...
  const renderState = (state: string): React.ReactNode => {
    const config: Record<
      string,
    ...
    > = {
      none: {
        ...
        icon: <LucideReact.MinusCircle className="text-gray-500" size={16} />,
      },
      underway: {
        label: "In Transit",
      ...
      preparing: {
        ...
        icon: <LucideReact.Hammer className="text-blue-500" size={16} />,
      },
      manufacturing: {
        label: "Manufacturing",
        color: "bg-indigo-100 text-indigo-800",
        icon: <LucideReact.Building className="text-indigo-500" size={16} />,
      },
      shipping: {
        label: "Shipping",
        color: "bg-purple-100 text-purple-800",
        icon: <LucideReact.Truck className="text-purple-500" size={16} />,
      },
      delivering: {
        label: "Delivering",
        color: "bg-teal-100 text-teal-800",
        icon: <LucideReact.MapPin className="text-teal-500" size={16} />,
      },
      arrived: {
        label: "Arrived",
    ...
    <section className="p-4">
      ...
      <ul className="space-y-4 md:hidden">
        {deliveries.map((delivery) => (
          <li key={delivery.id} className="bg-white p-4 rounded-lg shadow-sm">
            <div className="flex justify-between items-center mb-2">
              <span className="font-medium text-gray-800 truncate">
                {delivery.id}
              </span>
              {renderState(delivery.state)}
            </div>
            <div className="flex flex-wrap text-sm text-gray-600">
              <div className="flex items-center mr-4">
                <LucideReact.User size={16} className="mr-1" />
                <span>{delivery.seller.id}</span>
              </div>
              <div className="flex items-center mr-4">
              ...
              <div className="flex items-center mr-4">
                ...
                <span>{delivery.orders.length} orders</span>
              </div>
              <div className="flex items-center mr-4">
                <LucideReact.Truck size={16} className="mr-1" />
                <span>{delivery.journeys.length} journeys</span>
              </div>
              <div className="flex items-center mr-4">
                <LucideReact.Package size={16} className="mr-1" />
                <span>{delivery.pieces.length} pieces</span>
              </div>
              <div className="flex items-center">
                <LucideReact.Users size={16} className="mr-1" />
                <span>{delivery.shippers.length} shippers</span>
              </div>
            </div>
          </li>
        ))}
        ...
        <table className="min-w-full text-sm text-left">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 w-1/6">Seller</th>
              <th className="px-4 py-2 w-1/6">Created</th>
              <th className="px-4 py-2 w-1/6">State</th>
              <th className="px-4 py-2">Orders</th>
              <th className="px-4 py-2">Journeys</th>
              <th className="px-4 py-2">Pieces</th>
              <th className="px-4 py-2">Shippers</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {deliveries.map((delivery) => (
              <tr key={delivery.id}>
                <td className="px-4 py-2 truncate max-w-xs">{delivery.id}</td>
                <td className="px-4 py-2">{delivery.seller.id}</td>
                <td className="px-4 py-2">{formatDate(delivery.created_at)}</td>
                <td className="px-4 py-2">{renderState(delivery.state)}</td>
                <td className="px-4 py-2">{delivery.orders.length}</td>
                <td className="px-4 py-2">{delivery.journeys.length}</td>
                <td className="px-4 py-2">{delivery.pieces.length}</td>
                <td className="px-4 py-2">{delivery.shippers.length}</td>
              </tr>
            ))}
          </tbody>
        </table>
