...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { name, created_at, customer, goods, price } = value;
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <time className="text-sm text-gray-500">{orderDate}</time>
      </div>

      {/* Customer & item count */}
      <div className="flex flex-wrap text-sm text-gray-600 mb-4 space-x-4">
        <span>Channel: {customer.channel.name}</span>
        <span>Items: {goods.length}</span>
      </div>
      ...
      <ul className="divide-y divide-gray-200">
        {goods.map((good) => {
          ...
          const thumb = good.commodity.sale.content.thumbnails?.[0]?.url;
          const qty = good.volume;
          const itemNominal = good.price.nominal;
          const itemReal = good.price.real;

          return (
            <li key={good.id} className="py-3 flex items-center">
              {thumb && (
                <img
                  src={thumb}
                  alt={title}
              ...
              <div className="text-right ml-4">
                <p className="text-gray-800 font-medium">
                  {formatAmount(itemReal)}
                </p>
                {itemNominal > itemReal && (
                  <p className="text-gray-500 text-xs line-through">
                    {formatAmount(itemNominal)}
                  </p>
                )}
              </div>
            </li>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 flex justify-between items-center">
        ...
        <div className="text-right">
          <p className="text-lg font-bold text-gray-900">{formatAmount(price.real)}</p>
          {price.nominal > price.real && (
            <p className="text-gray-500 text-sm line-through">
              {formatAmount(price.nominal)}
            </p>
          )}
        </div>
      </div>
