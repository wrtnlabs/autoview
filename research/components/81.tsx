...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalItems = value.goods.reduce((sum, good) => sum + (good.volume ?? 0), 0);
  ...
  const orderStatusInfo = (() => {
    ...
    const map: Record<string, { label: string; icon: JSX.Element }> = {
      ...
      underway: {
        label: 'Underway',
        icon: <LucideReact.Truck size={16} className="text-blue-500" />,
      },
      preparing: {
        label: 'Preparing',
      ...
      delivering: {
        label: 'Delivering',
        icon: <LucideReact.Truck size={16} className="text-green-500" />,
      },
      arrived: {
        label: 'Delivered',
    ...
    return map[value.publish.state] || {
      label: value.publish.state || 'Unknown',
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md space-y-6">
      ...
      <div className="flex items-center text-sm space-x-2">
        ...
        <span className="text-gray-700">{orderStatusInfo.label}</span>
      </div>
      ...
      <ul className="space-y-4">
        {value.goods.map((good) => {
          ...
          const qty = good.volume;
          const price = good.price.real;
          const stateKey = good.state ?? 'none';
          const itemState = {
            none: {
              label: 'Pending',
              icon: <LucideReact.Clock size={14} className="text-gray-400" />,
            },
            underway: {
              label: 'Underway',
              icon: <LucideReact.Truck size={14} className="text-gray-400" />,
            },
            preparing: {
              label: 'Preparing',
              icon: <LucideReact.Settings size={14} className="text-blue-400" />,
            },
            manufacturing: {
              label: 'Manufacturing',
              icon: <LucideReact.Settings size={14} className="text-blue-400" />,
            ...
            shipping: {
              label: 'Shipping',
              icon: <LucideReact.Truck size={14} className="text-blue-400" />,
            },
            delivering: {
              label: 'Delivering',
              icon: <LucideReact.Truck size={14} className="text-green-400" />,
            },
            arrived: {
              label: 'Delivered',
              icon: <LucideReact.CheckCircle size={14} className="text-green-400" />,
            },
          }[stateKey] || {
            label: stateKey,
            icon: <LucideReact.AlertCircle size={14} className="text-gray-400" />,
          };

          return (
            <li key={good.id} className="flex items-center space-x-4">
              {/* Thumbnail */}
      ...
      {/* Summary */}
      <div className="flex justify-end border-t pt-4 space-x-6">
        <div className="text-sm text-gray-600">Items: {totalItems}</div>
        <div className="text-sm font-medium text-gray-800">
          Total: {formatPrice(value.price.real)}
        </div>
      </div>
    </div>
  );
