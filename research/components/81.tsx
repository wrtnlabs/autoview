...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm space-y-6">
      {/* Header: Order name, ID, created date */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-center">
        <div>
          <h2 className="text-xl font-semibold text-gray-800">{value.name}</h2>
          <p className="mt-1 text-sm text-gray-500">Order ID: {value.id}</p>
        </div>
      ...
      <div className="border-t border-gray-200 pt-4">
        <h3 className="flex items-center text-gray-700 font-medium">
          <LucideReact.User className="mr-2 text-gray-400" size={18} />
          Customer
        </h3>
      ...
      <div>
        <h3 className="flex items-center text-gray-700 font-medium">
          <LucideReact.Package className="mr-2 text-gray-400" size={18} />
            ...
            >
              <div className="flex-1">
                <p className="text-gray-800 font-medium truncate">
                  {good.commodity.sale.content.title}
                </p>
                <p className="mt-1 text-sm text-gray-500">Qty: {good.volume}</p>
                {good.state && (
                  <p className="mt-1 inline-flex items-center text-sm text-amber-600">
                    <LucideReact.Clock className="mr-1" size={14} />
                    {good.state.charAt(0).toUpperCase() + good.state.slice(1)}
                  </p>
                )}
              </div>
              <div className="mt-2 sm:mt-0 text-right">
                <p className="text-gray-800 font-semibold">
                  {formatPrice(good.price.real)}
                </p>
                {good.price.nominal > good.price.real && (
                  <p className="text-sm text-gray-400 line-through">
                    {formatPrice(good.price.nominal)}
                  </p>
                )}
              </div>
            </li>
      ...
      <div className="border-t border-gray-200 pt-4">
        <h3 className="flex items-center text-gray-700 font-medium">
          <LucideReact.DollarSign className="mr-2 text-gray-400" size={18} />
          Order Summary
        </h3>
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600">
          <div className="flex justify-between">
            <span>Subtotal</span>
            <span>{formatPrice(value.price.nominal)}</span>
          </div>
          <div className="flex justify-between">
            <span>Total Paid</span>
            <span className="font-semibold">
              {formatPrice(value.price.real)}
            </span>
          </div>
          <div className="flex justify-between">
            <span>Cash</span>
            <span>{formatPrice(value.price.cash)}</span>
          </div>
          <div className="flex justify-between">
            <span>Deposit</span>
            <span>{formatPrice(value.price.deposit)}</span>
          </div>
          <div className="flex justify-between">
            <span>Mileage</span>
            <span>{formatPrice(value.price.mileage)}</span>
          </div>
          <div className="flex justify-between">
            <span>Coupons</span>
            <span>{formatPrice(value.price.ticket)}</span>
          </div>
        </div>
      </div>
      ...
      {value.publish && (
        <div className="border-t border-gray-200 pt-4">
          <h3 className="flex items-center text-gray-700 font-medium">
            {value.publish.paid_at ? (
              <LucideReact.CheckCircle
                className="mr-2 text-green-500"
                size={18}
              />
            ) : (
              <LucideReact.Clock className="mr-2 text-amber-500" size={18} />
            )}
            Payment Status
          </h3>
          <div className="mt-2 space-y-1 text-sm text-gray-600">
            <div className="flex justify-between">
              ...
              <span>
                {value.publish.paid_at
                  ? formatDate(value.publish.paid_at)
                  : "Pending"}
              </span>
            </div>
            {value.publish.cancelled_at && (
              <div className="flex justify-between">
                <span>Cancelled At</span>
                <span>{formatDate(value.publish.cancelled_at)}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span>Delivery State</span>
              <span className="capitalize">{value.publish.state ?? "N/A"}</span>
            </div>
          </div>
        </div>
      )}
