...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const rawMsg =
    ...
    "";
  const truncatedMsg =
    rawMsg.length > 100 ? rawMsg.slice(0, 100) + "…" : rawMsg;
  ...
  // 2. Compose visual structure with JSX and Tailwind CSS
  return (
    <div className="p-4 bg-white rounded-lg shadow-md flex items-start space-x-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          ...
          <div title={`Status: ${userChat?.state || "unknown"}`}>
            {stateIcon}
          </div>
        </div>
        <div className="flex items-center text-sm text-gray-500 space-x-4 mt-1">
          ...
          <div className="flex items-center space-x-1">
            ...
            <span>{updatedAt || lastMsgTime}</span>
          </div>
        </div>
        {truncatedMsg && (
