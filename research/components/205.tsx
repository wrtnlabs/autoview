...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const lastSeen = user?.lastSeenAt
    ? new Date(user.lastSeenAt).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })
    : null;
  ...
  const messageText =
    message?.plainText ||
    message?.blocks?.map(block => block.value).filter(Boolean).join(' ') ||
  ...
  const oneTimeMsgName = value.oneTimeMsg?.name;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-sm w-full bg-white rounded-lg shadow p-4 space-y-4">
      {(avatarUrl || userName) && (
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-lg font-medium text-gray-900 truncate">{userName}</div>
            {lastSeen && (
              <div className="text-sm text-gray-500 truncate">Last seen: {lastSeen}</div>
            )}
          </div>
        </div>
      )}
      ...
      {session && (
        <div className="flex items-center text-sm text-gray-600 space-x-4">
          {chatState && <span className="capitalize">{chatState}</span>}
          {unreadCount > 0 && (
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap">
          {tags.map((tag, idx) => (
            <span
              key={idx}
              className="text-xs bg-gray-200 text-gray-700 px-2 py-0.5 rounded mr-1 mb-1 truncate"
            >
              {tag.name}
            </span>
          ))}
      ...
      {(campaignName || oneTimeMsgName) && (
        <div className="text-sm text-blue-600">
          {campaignName
            ? `Campaign: ${campaignName}`
            : `One-time Msg: ${oneTimeMsgName}`}
        </div>
      )}

      <div>
        <div className="text-sm text-gray-700 line-clamp-2">{messageText}</div>
        {messageTime && (
