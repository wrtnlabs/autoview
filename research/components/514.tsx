...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const enforcement = enforcementMap[value.enforcement];

  const ruleCounts = (value.rules || []).reduce<Record<string, number>>(
    (acc, rule) => {
  ...
  if (cond) {
    if ("ref_name" in cond && cond.ref_name) {
      const inc = cond.ref_name.include?.length || 0;
      const exc = cond.ref_name.exclude?.length || 0;
      condList.push(`Ref includes ${inc}, excludes ${exc}`);
    }
    if ("repository_name" in cond && cond.repository_name) {
      const inc = cond.repository_name.include?.length || 0;
      const exc = cond.repository_name.exclude?.length || 0;
      condList.push(`Repos inc ${inc}, exc ${exc}`);
    }
    if ("repository_id" in cond && cond.repository_id) {
      const ids = cond.repository_id.repository_ids?.length || 0;
      condList.push(`Repo IDs ${ids}`);
    }
    if ("repository_property" in cond && cond.repository_property) {
      const inc = cond.repository_property.include?.length || 0;
      const exc = cond.repository_property.exclude?.length || 0;
      condList.push(`Props inc ${inc}, exc ${exc}`);
    }
  }
  const conditionsSummary = condList.join("; ") || "None";

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-md w-full bg-white rounded-lg shadow p-4 mx-auto">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-1">
          {React.createElement(enforcement.icon, {
            size: 16,
            className: enforcement.color,
          })}
          <span className={`text-sm font-medium ${enforcement.color}`}>
            {enforcement.label}
          </span>
        </div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-600">
        ...
        {value.bypass_actors && value.bypass_actors.length > 0 && (
          <div className="flex items-center space-x-1">
            ...
            <span>{value.bypass_actors.length} Bypass</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-x-4 text-xs text-gray-500">
        {createdAt && (
          <div className="flex items-center space-x-1">
