...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const summaryConditions = (() => {
    ...
    if ('ref_name' in cond && cond.ref_name) {
      const inc = cond.ref_name.include;
      const exc = cond.ref_name.exclude;
      if (inc?.length)
        parts.push(
          `Refs incl: ${inc.slice(0, 3).join(', ')}${inc.length > 3 ? '…' : ''}`
        );
      if (exc?.length)
        parts.push(
          `Refs excl: ${exc.slice(0, 3).join(', ')}${exc.length > 3 ? '…' : ''}`
        );
    }
    if ('repository_name' in cond) {
      const inc = cond.repository_name.include;
      const exc = cond.repository_name.exclude;
      if (inc?.length)
        parts.push(
          `Repos incl: ${inc.slice(0, 3).join(', ')}${inc.length > 3 ? '…' : ''}`
        );
      if (exc?.length)
        parts.push(
          `Repos excl: ${exc.slice(0, 3).join(', ')}${exc.length > 3 ? '…' : ''}`
        );
    }
    if ('repository_id' in cond) {
      const ids = cond.repository_id.repository_ids;
      if (ids?.length)
        parts.push(
          `Repo IDs: ${ids.slice(0, 3).join(', ')}${ids.length > 3 ? '…' : ''}`
        );
    }
    if ('repository_property' in cond) {
      const inc = cond.repository_property.include;
      const exc = cond.repository_property.exclude;
      if (inc?.length)
        parts.push(
          `Prop incl: ${inc.map((p) => p.name).slice(0, 3).join(', ')}${
            inc.length > 3 ? '…' : ''
          }`
        );
      if (exc?.length)
        parts.push(
          `Prop excl: ${exc.map((p) => p.name).slice(0, 3).join(', ')}${
            exc.length > 3 ? '…' : ''
          }`
        );
    }
  ...
  const ruleTypes = Array.from(new Set(value.rules?.map((r) => r.type) ?? []));
  const ruleCount = value.rules?.length ?? 0;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-6 max-w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-700">
            Source:{' '}
            {value.source_type
              ? `${value.source_type} / ${value.source}`
              : value.source}
          </span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div>
        <h3 className="text-md font-medium text-gray-800 mb-1">Conditions</h3>
        <p className="text-sm text-gray-600">{summaryConditions}</p>
      </div>
      <div>
