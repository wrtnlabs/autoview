...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { user, userChat, message, session, chatTags, campaign, oneTimeMsg, supportBot } = value;

  // User info
  const userName = user?.name ?? "Unknown User";
  const avatarUrl = user?.avatarUrl;

  // Chat state and color mapping
  const chatState = userChat?.state ?? "unknown";
  ...
  const chatStateColor = stateColors[chatState] ?? stateColors.unknown;
  const chatStateLabel = chatState.charAt(0).toUpperCase() + chatState.slice(1);

  // Message snippet
  const rawText =
    message?.plainText ??
    message?.blocks?.map((b) => b.value || "").join(" ") ??
    "";
  ...
  const formatDate = (ts?: number) =>
    ts
      ...
      : "";
  const openedAt = formatDate(userChat?.openedAt ?? userChat?.createdAt);
  ...
  // Campaign badge colors
  const campaignStateColors: Record<string, string> = {
  ...
  const campaignColor =
    campaignStateColors[campaign?.state ?? "draft"] ??
    campaignStateColors.draft;

  // One-time message badge colors
  const otmStateColors: Record<string, string> = {
    draft: "bg-gray-100 text-gray-800",
  ...
  const otmColor = otmStateColors[oneTimeMsg?.state ?? "draft"] ??
    otmStateColors.draft;

  // Support bot badge colors
  const sbStateColors: Record<string, string> = {
    draft: "bg-gray-100 text-gray-800",
    active: "bg-green-100 text-green-800",
    stopped: "bg-red-100 text-red-800",
  };
  const sbColor =
    sbStateColors[supportBot?.state ?? "draft"] ?? sbStateColors.draft;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="p-4 bg-white rounded-lg shadow-md flex flex-col sm:flex-row sm:items-center sm:space-x-4">
      {avatarUrl ? (
        <img
          src={avatarUrl}
          alt={userName}
      ...
      <div className="flex-1 flex flex-col mt-2 sm:mt-0">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`px-2 py-0.5 rounded-full text-xs font-medium ${chatStateColor}`}
          >
            {chatStateLabel}
          </span>
        </div>
        {openedAt && (
          <div className="text-sm text-gray-500 mt-1">{openedAt}</div>
        )}
        <p className="text-sm text-gray-600 mt-2 line-clamp-2">{lastMessage}</p>
        <div className="flex flex-wrap items-center gap-2 mt-3">
          {chatTags?.map((tag) => (
            <span
              key={tag.key}
              className={`px-2 py-1 rounded-full text-xs font-medium ${
                tagColors[tag.colorVariant ?? "green"]
              }`}
            >
              {tag.name}
            </span>
          ...
          {campaign?.name && (
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${campaignColor}`}
            >
          ...
          {oneTimeMsg?.name && (
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${otmColor}`}
            >
              One-time Msg: {oneTimeMsg.name}
            </span>
          )}
          {supportBot?.botName && (
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${sbColor}`}
            >
              Bot: {supportBot.botName}
            </span>
          )}
        </div>
      </div>
