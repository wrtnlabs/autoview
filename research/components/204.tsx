...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Derived data: user display name and avatar handling
  ...
  const [avatarSrc, setAvatarSrc] = React.useState<string>(
    value.user?.avatarUrl || fallbackAvatar,
  );
  React.useEffect(() => {
    setAvatarSrc(value.user?.avatarUrl || fallbackAvatar);
  }, [value.user?.avatarUrl, fallbackAvatar]);
  ...
  // 2. Message preview text and timestamp
  const messageText =
  ...
  const messageTime = value.message?.createdAt
    ? new Date(value.message.createdAt).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "";

  // 3. Chat status icon mapping
  ...
  > = {
    ...
    queued: {
      icon: LucideReact.Hourglass,
      color: "text-gray-500",
      label: "Queued",
    },
  };
  const statusInfo = value.userChat?.state && statusMap[value.userChat.state];
  ...
  // 5. Render the chat overview card
  return (
    <div className="flex items-start p-4 bg-white rounded-lg shadow">
      ...
      <div className="ml-4 flex-1 min-w-0">
        ...
        {/* Message Preview */}
        <p className="text-sm text-gray-600 mt-1 line-clamp-2">{messageText}</p>
        {/* Tags and Timestamp */}
            ...
            {value.chatTags?.map((tag) => (
              <span
                ...
                className={`px-2 py-0.5 rounded text-xs font-medium ${
                  tagColorClasses[tag.colorVariant ?? ""] ||
