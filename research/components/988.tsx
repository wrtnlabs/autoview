  ...
  return (
    <ul role="list" className="space-y-4">
      {events.map((event) => {
        ...
        // Derive a concise summary from payload
        const summary: string = (() => {
          const { action, issue, comment, pages } = event.payload;
          if (action) return action;
          if (issue) return `Issue: ${issue.title}`;
          if (comment) return comment.body_text ?? comment.body ?? "";
        ...
        return (
          <li key={event.id} className="bg-white p-4 rounded-lg shadow-sm">
