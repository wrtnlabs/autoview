...
export type AutoViewInput = AutoViewInputSubTypes.IShoppingDelivery.IInvert;
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalOrders = value.orders.length;
  const totalPieces = value.pieces.reduce((sum, p) => sum + p.quantity, 0);
  const shippersCount = value.shippers.length;
  ...
  > = {
    ...
    manufacturing: {
      label: "Manufacturing",
      icon: <LucideReact.Settings size={16} className="text-blue-500" />,
      color: "text-blue-500",
    },
    shipping: {
      label: "Shipping",
      icon: <LucideReact.Truck size={16} className="text-indigo-500" />,
      color: "text-indigo-500",
    },
    delivering: {
      label: "Delivering",
      icon: <LucideReact.MapPin size={16} className="text-purple-500" />,
    ...
    underway: {
      label: "Underway",
      icon: <LucideReact.Activity size={16} className="text-blue-500" />,
      color: "text-blue-500",
    },
  };
  const stateInfo = stateMap[value.state] || stateMap.none;

  const journeyIconMap: Record<string, JSX.Element> = {
    preparing: <LucideReact.Clock size={14} className="text-amber-500" />,
    manufacturing: <LucideReact.Settings size={14} className="text-blue-500" />,
    shipping: <LucideReact.Truck size={14} className="text-indigo-500" />,
    delivering: <LucideReact.MapPin size={14} className="text-purple-500" />,
  };
    ...
    <div className="p-4 bg-white rounded-lg shadow-md space-y-6">
      <header className="flex justify-between items-center">
        <h2 className="flex items-center text-xl font-semibold text-gray-800">
          <LucideReact.Truck size={20} className="text-gray-600 mr-2" />
          Delivery Details
        </h2>
      ...
      <div className="flex flex-wrap items-center gap-6">
        <div className="flex items-center text-sm">
          <span className="mr-1 font-medium text-gray-700">State:</span>
          {stateInfo.icon}
          <span className={`ml-1 ${stateInfo.color} font-medium`}>
            {stateInfo.label}
          </span>
        </div>
        ...
        <div className="flex items-center text-sm text-gray-700">
          ...
          <span className="ml-1">
            {totalPieces} Item{totalPieces !== 1 ? "s" : ""} Delivered
          </span>
        </div>
        <div className="flex items-center text-sm text-gray-700">
          <LucideReact.Users size={16} className="text-gray-500" />
          <span className="ml-1">
            {shippersCount} Shipper{shippersCount !== 1 ? "s" : ""}
          </span>
        </div>
      </div>
      ...
      {shippersCount > 0 && (
        <section>
          <h3 className="text-sm font-medium text-gray-800 mb-2">Shippers</h3>
          <div className="flex flex-wrap gap-2">
            {value.shippers.slice(0, 3).map((s) => (
              <div
                key={s.id}
      ...
      {value.journeys.length > 0 && (
        <section>
          ...
          <div className="border-l border-gray-200 ml-1 pl-4 space-y-4">
            {value.journeys.map((j) => {
              ...
              const end = j.completed_at
                ...
                : "In Progress";
              const icon = journeyIconMap[j.type] || (
                <LucideReact.Clock size={14} className="text-gray-500" />
              );
