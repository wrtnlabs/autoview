...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const currentSnapshot = React.useMemo(
    () =>
      value.snapshots.reduce((prev, curr) =>
  ...
  const reviewerName =
    ...
    'Guest';
  const reviewDate = new Date(currentSnapshot.created_at).toLocaleDateString(
    undefined,
    { year: 'numeric', month: 'short', day: 'numeric' }
  );
  const score = Math.max(0, Math.min(5, currentSnapshot.score));

  let answerSection: React.ReactNode = null;
  if (value.answer?.snapshots.length) {
    const ans = value.answer.snapshots.reduce((prev, curr) =>
      new Date(prev.created_at) > new Date(curr.created_at) ? prev : curr
    );
    const answerDate = new Date(ans.created_at).toLocaleDateString(undefined, {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
    });
    answerSection = (
      <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex items-center text-gray-600 text-sm">
          ...
          <LucideReact.Calendar size={16} className="ml-auto text-gray-400" />
          <span className="ml-1">{answerDate}</span>
        </div>
        <h4 className="mt-2 text-base font-medium text-gray-800 truncate">
          {ans.title}
        </h4>
        <p className="mt-1 text-gray-700 line-clamp-3">{ans.body}</p>
      </div>
    );
  }
      ...
      <div className="flex items-center mt-2">
        {Array.from({ length: 5 }, (_, i) => (
          <LucideReact.Star
            key={i}
            size={16}
            className={i < score ? 'text-amber-400' : 'text-gray-300'}
          />
        ))}
        <span className="ml-3 text-gray-500 text-xs">{reviewDate}</span>
      </div>

      <h3 className="mt-3 text-lg font-semibold text-gray-800 truncate">
        {currentSnapshot.title}
      </h3>
      <p className="mt-1 text-gray-700 line-clamp-3">{currentSnapshot.body}</p>
