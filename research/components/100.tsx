...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const latestSnapshot = value.snapshots.reduce((prev, cur) =>
  ...
  const starsCount = Math.round(latestSnapshot.score);
  const stars = Array.from({ length: starsCount }, (_, i) => (
    <LucideReact.Star key={i} size={16} className="text-amber-400" />
  ));
  const unread = !value.read_by_seller;
  const hasAnswer = Boolean(value.answer);
  const latestAnswerSnapshot = value.answer
    ? value.answer.snapshots.reduce((prev, cur) =>
        new Date(prev.created_at) > new Date(cur.created_at) ? prev : cur,
      )
    : null;
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm space-y-4">
      ...
      {hasAnswer && latestAnswerSnapshot && (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h4 className="flex items-center text-gray-800 font-semibold text-sm gap-1">
            <LucideReact.MessageSquare size={16} />
            Seller Response
          </h4>
          <p className="mt-2 text-gray-700 text-sm truncate font-medium">
          ...
          <div className="mt-2 flex items-center gap-1 text-xs text-gray-500">
            ...
            <span>
              {new Date(latestAnswerSnapshot.created_at).toLocaleDateString(
                undefined,
                {
                  year: "numeric",
                  month: "short",
