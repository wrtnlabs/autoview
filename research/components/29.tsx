...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const getWriterName = (writer: Writer): string => {
    if (writer.type === 'administrator' || writer.type === 'seller') {
    ...
    return (
      ...
      writer.citizen?.name ??
      writer.external_user?.nickname ??
      'Anonymous'
    );
  };

  const getWriterIcon = (writer: Writer): JSX.Element => {
    if (writer.type === 'administrator') {
      return <LucideReact.Shield size={16} className="text-blue-500" />;
    }
    if (writer.type === 'seller') {
      return <LucideReact.ShoppingCart size={16} className="text-green-500" />;
    }
    // customer
    return <LucideReact.User size={16} className="text-gray-500" />;
  };
    ...
    <div className="p-4 bg-white rounded-lg shadow-md">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-sm text-gray-500">
          {records} total &middot; Page {current}/{pages}
        </div>
      </div>
            ...
            <div key={comment.id} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {getWriterIcon(comment.writer)}
                  <span className="font-medium text-gray-800">
                    {getWriterName(comment.writer)}
                  </span>
                  {comment.parent_id && (
              ...
              <p className="mt-2 text-gray-700 line-clamp-3">{latest.body}</p>
              {comment.snapshots.length > 1 && (
                <div className="mt-2 text-sm text-gray-500 flex items-center gap-1">
                  <LucideReact.Edit2 size={16} />
                  <span>Edited {comment.snapshots.length - 1} times</span>
                </div>
              )}
            </div>
          );
