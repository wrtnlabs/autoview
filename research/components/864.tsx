...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Map enforcement to a semantically appropriate icon and color
  let EnforcementIcon: React.ComponentType<any> = LucideReact.XCircle;
  let enforcementColor = "text-gray-500";
  switch (value.enforcement) {
    case "active":
      ...
      break;
    case "evaluate":
      EnforcementIcon = LucideReact.Clock;
      enforcementColor = "text-blue-500";
      break;
    default:
      EnforcementIcon = LucideReact.XCircle;
      enforcementColor = "text-gray-500";
      break;
  }
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center">
          <EnforcementIcon className={`${enforcementColor}`} size={20} />
          <span className={`ml-1 ${enforcementColor} text-sm font-medium`}>
      ...
      <div className="mt-2 text-sm text-gray-600">
        <span className="capitalize">{value.target || "repository"}</span>{" "}
        ruleset on{" "}
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 text-sm text-gray-700">
        <div className="flex items-center">
          ...
          <span>{value.rules?.length ?? 0} rules</span>
        </div>
        <div className="flex items-center">
          <LucideReact.Users className="text-gray-500 mr-2" size={16} />
          <span>{value.bypass_actors?.length ?? 0} bypass actors</span>
        </div>
        <div className="flex items-center">
            ...
            {value.conditions.ref_name && (
              <div>
                ...
                {value.conditions.ref_name.include && (
                  <span>
                    Include: {value.conditions.ref_name.include.join(", ")}
                  </span>
                )}
                {value.conditions.ref_name.exclude && (
                  <span className="ml-2">
                    Exclude: {value.conditions.ref_name.exclude.join(", ")}
                  </span>
                )}
              </div>
            )}
            {"repository_name" in value.conditions &&
              value.conditions.repository_name && (
                <div>
                  <span className="font-medium">Repositories:</span>{" "}
                  {value.conditions.repository_name.include && (
                    <span>
                      Include:{" "}
                      {value.conditions.repository_name.include.join(", ")}
                    </span>
                  )}
                  {value.conditions.repository_name.exclude && (
                    <span className="ml-2">
                      Exclude:{" "}
                      {value.conditions.repository_name.exclude.join(", ")}
                    </span>
                  )}
                </div>
              )}
            {"repository_id" in value.conditions &&
              value.conditions.repository_id && (
                <div>
                  <span className="font-medium">Repository IDs:</span>{" "}
                  <span>
                    {value.conditions.repository_id.repository_ids
                      ?.map(String)
                      .join(", ") || "—"}
                  </span>
                </div>
