...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Determine writer display name and icon based on their type
  const writer = value.writer as any;
  let writerLabel: string = "";
  let WriterIcon: React.ComponentType<any> = LucideReact.User;

  if (writer.type === "administrator") {
    writerLabel = writer.member.nickname;
    WriterIcon = LucideReact.Shield;
  } else if (writer.type === "seller") {
    writerLabel = writer.member.nickname;
    WriterIcon = LucideReact.ShoppingCart;
  } else {
    // customer
    writerLabel = writer.member?.nickname ?? writer.citizen?.name ?? "Customer";
    WriterIcon = LucideReact.User;
  }

  // Extract the latest snapshot and revision count
  const snapshots = value.snapshots ?? [];
  const revisionCount = snapshots.length;
  const latestSnapshot = revisionCount ? snapshots[snapshots.length - 1] : null;

  // Format dates for display
  const commentDate = new Date(value.created_at).toLocaleString();
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-full">
      {/* Header: writer, date, revisions */}
      <div className="flex items-center text-sm text-gray-600 mb-2">
        <WriterIcon
          className="text-gray-500 mr-2 flex-shrink-0"
          size={16}
          aria-label={writer.type}
        />
        <span className="font-medium text-gray-800">{writerLabel}</span>
        <LucideReact.Calendar
          className="text-gray-400 mx-2 flex-shrink-0"
          size={16}
        />
        <time dateTime={value.created_at} className="text-gray-600">
          {commentDate}
        </time>
        {revisionCount > 1 && (
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center text-xs text-gray-500">
            <LucideReact.Clock
              ...
              aria-label="Updated"
            />
            <time dateTime={latestSnapshot.created_at}>{snapshotDate}</time>
          </div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {latestSnapshot.files.map((file) => {
                ...
                return (
                  <div
                    key={file.url}
                    className="flex items-center text-xs text-blue-600 truncate max-w-full"
                  >
                    ...
                    />
                    <span title={file.url}>{fileName}</span>
                  </div>
                );
              })}
            </div>
