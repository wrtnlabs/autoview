...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const writerType = value.writer.type;
  let writerName: string;
  if (writerType === "customer") {
    const customer = value.writer as AutoViewInputSubTypes.IShoppingCustomer;
    writerName =
      customer.member?.nickname ??
      customer.citizen?.name ??
      "Customer";
  } else {
    const invert = value.writer as
      | AutoViewInputSubTypes.IShoppingAdministrator.IInvert
      | AutoViewInputSubTypes.IShoppingSeller.IInvert;
    writerName = invert.member.nickname;
  }

  const latestSnapshot = value.snapshots[value.snapshots.length - 1];
  const commentDate = new Date(latestSnapshot.created_at).toLocaleString(undefined, {
    year: "numeric",
    month: "short",
    day: "numeric",
    hour: "numeric",
  ...
  const remainingCount = files.length - displayedFiles.length;
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <span className="ml-2 px-1 text-xs text-gray-500 bg-gray-100 rounded">
            {writerType.charAt(0).toUpperCase() + writerType.slice(1)}
          </span>
        </div>
      ...
      <p className="mt-3 text-gray-700 break-words line-clamp-3">
        {latestSnapshot.body}
      </p>
          ...
          <ul className="space-y-1">
            {displayedFiles.map((file) => {
              const filename = file.extension
                ...
                : file.name;
              return (
                <li key={file.url} className="flex items-center text-blue-500">
            ...
            {remainingCount > 0 && (
              <li className="text-sm text-gray-500">+{remainingCount} more</li>
            )}
          </ul>
        </div>
      )}
