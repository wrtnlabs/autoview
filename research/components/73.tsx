...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { content, seller, section, categories, tags } = sale;
  const title = content.title;
  const thumbnailUrl =
    content.thumbnails[0]?.url ||
    `https://placehold.co/300x300/f8fafc/475569?text=${encodeURIComponent(title)}`;
  const placeholder = `https://placehold.co/300x300/f8fafc/475569?text=${encodeURIComponent(title)}`;

  const totalReal = price.real * volume;
  const unitReal = price.real;
  const unitNominal = price.nominal;
  const formatCurrency = (n: number) =>
    `$${n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

  const formattedUnitReal = formatCurrency(unitReal);
  const formattedUnitNominal = formatCurrency(unitNominal);
  const formattedTotalReal = formatCurrency(totalReal);

  const formattedAddedDate = new Date(created_at).toLocaleDateString(
    undefined,
    {
      year: "numeric",
      month: "short",
  ...
  return (
    <div className="flex flex-col md:flex-row bg-white rounded-lg shadow-md overflow-hidden">
      {/* Thumbnail */}
      <div className="w-full md:w-32 aspect-square flex-shrink-0">
        <img
          src={thumbnailUrl}
          alt={title}
      ...
      <div className="flex flex-col p-4 flex-1">
        ...
        <div className="flex items-center text-xs text-gray-500 mt-1">
          ...
          <span className="ml-1">Added {formattedAddedDate}</span>
        </div>
        ...
        {/* Price & Quantity */}
        <div className="mt-auto flex flex-col sm:flex-row sm:items-center sm:justify-between pt-4 border-t border-gray-100">
          <div className="flex items-baseline space-x-2">
            {unitNominal > unitReal + 0.001 && (
              <span className="text-sm text-gray-400 line-through">
                {formattedUnitNominal}
              </span>
            )}
