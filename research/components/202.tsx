...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const chatState =
    (value.userChat?.state && chatStateMap[value.userChat.state]) || null;
  ...
  // Extract a snippet from message
  let lastMessageSnippet = "";
  if (value.message) {
    if (value.message.plainText) {
      lastMessageSnippet = value.message.plainText;
    } else if (value.message.blocks && value.message.blocks.length > 0) {
      const firstBlock = value.message.blocks[0];
      lastMessageSnippet =
        firstBlock.value || firstBlock.blocks?.[0]?.value || "";
    }
  }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow flex items-start space-x-4">
        ...
        {(tags.length > 0 || campaignName) && (
          <div className="mt-2 flex flex-wrap gap-2">
            {tags.map((tag) => (
              ...
              >
                <LucideReact.Tag size={12} className="mr-1 inline-block" />
                {tag.name}
              </span>
            ))}
            {campaignName && (
              <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded">
                <LucideReact.Target size={12} className="mr-1 inline-block" />
                {campaignName}
              </span>
            )}
          </div>
      ...
      {value.session?.unread && value.session.unread > 0 && (
