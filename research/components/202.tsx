...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Derive a message snippet
  const snippet =
    value.message?.plainText?.trim() ||
    value.message?.blocks?.[0]?.value?.trim() ||
    "No message";
  // Choose a timestamp from message or chat open time
  const rawTime =
  ...
  const statusText = state ? statusMap[state] : "";
  const statusClass = state
    ? statusColorMap[state]
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow flex items-start space-x-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Snippet */}
        <p className="mt-1 text-gray-700 truncate">{snippet}</p>
        {/* Tags & Status */}
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          {statusText && (
            <span
              className={`text-xs font-medium px-2 py-1 rounded ${statusClass}`}
            >
              {statusText}
            </span>
          )}
