...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const variantColors: Record<string, { bg: string; text: string }> = {
    ...
    olive: { bg: "bg-lime-100", text: "text-lime-800" },
    purple: { bg: "bg-purple-100", text: "text-purple-800" },
    pink: { bg: "bg-pink-100", text: "text-pink-800" },
    navy: { bg: "bg-blue-100", text: "text-blue-800" },
  };
  const stateIcon = (() => {
    switch (value.userChat?.state) {
      case "opened":
        return (
  ...
  return (
    <div className="flex items-start p-4 bg-white rounded-lg shadow transition-shadow hover:shadow-md">
      <img
      ...
      <div className="ml-4 flex flex-col items-end">
        ...
        {stateIcon && <div className="mt-2">{stateIcon}</div>}
      </div>
    </div>
  );
}
