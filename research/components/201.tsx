...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const displayName =
    value.user?.name || value.user?.memberId || "Unknown User";
  const avatarSrc = value.user?.avatarUrl;
  ...
  // Extract a simple text preview from blocks if plainText is missing
  const messageText =
  ...
  // Map colorVariant to Tailwind classes
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            ...
            {formattedDate && (
              <time
                ...
                className="text-gray-500 text-sm flex items-center gap-1 whitespace-nowrap"
              >
          ...
          <p className="mt-2 text-gray-700 whitespace-pre-wrap overflow-hidden text-ellipsis line-clamp-3">
            {messageText}
          </p>
          {tags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {tags.map((tag) => {
                ...
                const variant = tag.colorVariant || "gray";
                const classes =
                  tagColorClasses[variant] || tagColorClasses.gray;
                return (
                  <div
                    key={key}
                    className={`flex items-center gap-1 px-2 py-0.5 rounded text-xs ${classes}`}
                  >
