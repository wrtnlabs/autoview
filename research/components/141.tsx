...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const commentDate = new Date(latestSnapshot.created_at).toLocaleDateString(undefined, {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
  ...
  // Determine writer type label and icon
  let writerLabel = '';
  let WriterIcon: JSX.Element;
  switch (value.writer.type) {
    case 'administrator':
      writerLabel = 'Administrator';
      WriterIcon = <LucideReact.ShieldCheck size={16} className="text-blue-500" aria-label="Administrator" />;
      break;
    case 'seller':
      writerLabel = 'Seller';
      WriterIcon = <LucideReact.ShoppingCart size={16} className="text-green-500" aria-label="Seller" />;
      break;
    case 'customer':
      writerLabel = 'Customer';
      WriterIcon = <LucideReact.User size={16} className="text-gray-500" aria-label="Customer" />;
      break;
    default:
      writerLabel = 'User';
      WriterIcon = <LucideReact.User size={16} className="text-gray-500" aria-label="User" />;
  }

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-xl mx-auto p-4 bg-white rounded-lg shadow-md divide-y divide-gray-200">
      {/* Header: Writer and Date */}
      <div className="flex items-center justify-between pb-3">
        ...
        <div className="flex items-center gap-1 text-xs text-gray-500">
          ...
          <time dateTime={latestSnapshot.created_at}>{commentDate}</time>
        </div>
      </div>
        ...
        <div className="pt-4">
          ...
          <ul className="space-y-2">
            {latestSnapshot.files.map((file) => {
              ...
              return (
                <li key={file.url} className="flex items-center gap-2 text-sm text-gray-600">
