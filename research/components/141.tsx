...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Derive writer type label
  const typeLabel = (() => {
    if ((writer as any).type === "customer") return "Customer";
    if ((writer as any).type === "seller") return "Seller";
    return "Administrator";
  })();
  // Derive writer display name: prefer member.nickname, then citizen.name, fallback to id
  const writerName = (() => {
    // @ts-ignore
    const member = writer.member;
    // Member invert or member object
    if (member && typeof member.nickname === "string") return member.nickname;
    // Try citizen name
    // @ts-ignore
    if (writer.citizen && typeof writer.citizen.name === "string")
      return writer.citizen.name;
    // Fallback to id
    // @ts-ignore
    return writer.id || "Unknown";
  })();

  // Latest snapshot: last in the array
  const snapshots = value.snapshots || [];
  const latestSnapshot = snapshots[snapshots.length - 1];
  const bodyText = latestSnapshot ? latestSnapshot.body : "";
  const snapshotFiles = latestSnapshot ? latestSnapshot.files : [];
  // Format the snapshot timestamp
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm max-w-full">
      ...
      <p className="mt-3 text-gray-800 text-sm line-clamp-3 whitespace-pre-wrap">
        {bodyText}
      </p>

      {snapshotFiles.length > 0 && (
        <div className="mt-2 flex items-center text-gray-500 text-sm">
          <LucideReact.Paperclip size={16} aria-label="Attachments" />
          <span className="ml-1">
            {snapshotFiles.length} attachment
      ...
      {formattedDate && (
        <div className="mt-4 flex items-center text-gray-400 text-xs">
          <LucideReact.Calendar size={14} aria-label="Updated at" />
