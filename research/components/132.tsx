...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Determine sale status and corresponding icon
  const { statusText, statusIcon } = (() => {
    ...
    if (value.opened_at && new Date(value.opened_at) > now) {
      return {
        statusText: "Not Opened",
        statusIcon: <LucideReact.Clock className="text-gray-500" size={16} />,
      };
    }
    if (value.closed_at && new Date(value.closed_at) < now) {
      return {
        statusText: "Closed",
        statusIcon: <LucideReact.XCircle className="text-gray-500" size={16} />,
      };
    }
    return {
      statusText: "Active",
  ...
  // Compute lowest nominal and real prices across all unit stocks
  const allNominal = value.units.flatMap((u) =>
    u.stocks.map((s) => s.price.nominal),
  );
  const allReal = value.units.flatMap((u) => u.stocks.map((s) => s.price.real));
  const minNominal = allNominal.length ? Math.min(...allNominal) : null;
  const minReal = allReal.length ? Math.min(...allReal) : null;

  // Select primary thumbnail or fallback placeholder
  const primaryThumbnail =
    value.content.thumbnails[0]?.url ||
    `https://placehold.co/300x300/f8fafc/475569?text=${encodeURIComponent(value.content.title)}`;
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            {statusIcon}
            <span className="ml-1 text-sm text-gray-600">{statusText}</span>
          </div>
        </div>
        ...
        <div className="flex items-baseline space-x-2">
          {minNominal !== null && minReal !== null && minNominal > minReal && (
            <span className="text-sm text-gray-500 line-through">
              ${minNominal.toLocaleString()}
            </span>
          )}
          {minReal !== null && (
            <span className="text-xl font-bold text-gray-900">
              ${minReal.toLocaleString()}
            </span>
          )}
        </div>
        ...
        {value.categories.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {value.categories.slice(0, 3).map((cat) => (
              <span
                key={cat.id}
                className="flex items-center text-xs text-blue-600 bg-blue-100 rounded-full px-2 py-1"
              >
            ...
            {value.categories.length > 3 && (
              <span className="text-xs text-gray-500">
                +{value.categories.length - 3} more
              </span>
            )}
          </div>
        )}
        ...
        {value.tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {value.tags.map((t) => (
              <span
                key={t}
                className="text-xs text-gray-700 bg-gray-100 rounded-full px-2 py-1"
              >
                #{t}
              </span>
            ))}
        ...
        <div className="flex items-center text-sm text-gray-500 space-x-4">
          <div className="flex items-center">
            ...
            <span>{value.seller.member.nickname}</span>
          </div>
          <div className="flex items-center">
            <LucideReact.Calendar size={14} className="mr-1" />
            <span>
              {formatDate(value.opened_at) || formatDate(value.created_at)}
            </span>
          </div>
        </div>
      </div>
