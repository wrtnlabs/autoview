...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const thumbnailUrl = value.content.thumbnails[0]?.url ?? null;

  const rawDescription = value.content.body.replace(/\s+/g, ' ').trim();
  const description = rawDescription.length > 120
    ? rawDescription.slice(0, 120) + '…'
    : rawDescription;

  const categoryNames = value.categories.map(cat => cat.name);
  const tagList = value.tags;

  // Flatten all stocks across units to compute price range and total stock
  const allStocks = value.units.flatMap(unit => unit.stocks);
  const prices = allStocks.map(s => s.price.real);
  const minPrice = Math.min(...prices);
  const maxPrice = Math.max(...prices);

  const stockCounts = allStocks.map(s => s.inventory.income - s.inventory.outcome);
  ...
  const closedAt = formatDate(value.closed_at);

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden sm:flex sm:space-x-4">
      {thumbnailUrl && (
        <div className="sm:flex-shrink-0 h-48 sm:h-auto sm:w-48 bg-gray-100">
          <img
            src={thumbnailUrl}
      ...
      <div className="p-4 flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">{value.content.title}</h2>
            {value.latest && (
          ...
          <div className="mt-1 flex flex-wrap items-center space-x-2">
            ...
            <span className="text-xs text-gray-500">by {sellerName}</span>
          </div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 flex flex-wrap">
            {categoryNames.map((name, idx) => (
              <span
                key={`cat-${idx}`}
                className="text-xs bg-gray-100 text-gray-800 px-2 py-0.5 rounded mr-2 mt-2"
              >
                {name}
              </span>
            ))}
          ...
          <div className="mt-2 flex flex-wrap">
            {tagList.map((tag, idx) => (
              <span
                key={`tag-${idx}`}
                className="text-xs bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded mr-2 mt-2"
              >
                #{tag}
              </span>
            ))}
          ...
          <div className="mt-3 text-xs text-gray-500">
            <div>
              <span className="font-medium">Sale Period:</span> {openedAt} – {closedAt}
            </div>
            <div>Last Updated: {formatDate(value.updated_at)}</div>
          </div>
        </div>
      </div>
    </div>
