...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const categoryNames = value.sale.categories.map((c) => c.name);
  const tags = value.sale.tags;
  const quantity = value.volume;
  const nominalPrice = value.price.nominal;
  const realPrice = value.price.real;
  const isDiscount = nominalPrice > realPrice;
  const formattedNominal = nominalPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const formattedReal = realPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const createdDate = new Date(value.created_at).toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" });
      ...
      <div className="flex-1 flex flex-col justify-between">
        <div>
          ...
          <div className="mt-1 text-sm text-gray-500">
            <span>{sectionName}</span> · <span>Seller: {sellerName}</span>
          </div>
          ...
          <div className="mt-2 flex items-center space-x-2">
            ...
            >
              {value.orderable ? "Orderable" : "Not Orderable"}
            </span>
            {value.pseudo && (
