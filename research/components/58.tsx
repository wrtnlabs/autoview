...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const isAmount = value.discount.unit === "amount";
  const discountLabel = isAmount
    ? `$${(value.discount as AutoViewInputSubTypes.IShoppingCouponDiscount.IAmount).value.toFixed(2)}`
    : `${(value.discount as AutoViewInputSubTypes.IShoppingCouponDiscount.IPercent).value}%`;
  const threshold =
    value.discount.threshold !== null
      ? `$${value.discount.threshold.toFixed(2)}`
      : null;
  const limit =
    value.discount.limit !== null
      ? `$${value.discount.limit.toFixed(2)}`
      : null;
  const multiplicative = isAmount
    ? (value.discount as AutoViewInputSubTypes.IShoppingCouponDiscount.IAmount).multiplicative
    : false;
  ...
  const openedAt = formatDate(value.opened_at);
  ...
  const expiredInDays =
    value.restriction.expired_in !== null
      ? `${value.restriction.expired_in} days after issue`
      : null;
  const expiredAtDate =
    value.restriction.expired_at !== null
      ? formatDate(value.restriction.expired_at)
      : null;

  const conditionCount = value.criterias.length;
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="flex items-baseline space-x-2 mb-4">
        {isAmount ? (
          <LucideReact.DollarSign className="text-green-500" size={24} />
        ) : (
          <LucideReact.Percent className="text-indigo-500" size={24} />
        )}
        <span className="text-2xl font-bold text-gray-900">{discountLabel}</span>
        {multiplicative && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600">
        <div className="flex items-center">
          <LucideReact.Globe className="mr-2 text-gray-400" size={16} />
          <span>{value.restriction.access === "public" ? "Public" : "Private"}</span>
        </div>

        <div className="flex items-center">
          <LucideReact.Lock className="mr-2 text-gray-400" size={16} />
          <span>{value.restriction.exclusive ? "Exclusive" : "Non-exclusive"}</span>
        </div>

        <div className="flex items-center">
          <LucideReact.Package className="mr-2 text-gray-400" size={16} />
          <span>Remaining: {inventoryVolume}</span>
        </div>

        <div className="flex items-center">
          <LucideReact.Users className="mr-2 text-gray-400" size={16} />
          <span>Per User: {inventoryPerCitizen}</span>
        </div>

        <div className="col-span-full flex items-center">
          <LucideReact.Calendar className="mr-2 text-gray-400" size={16} />
          <span>
            Valid: {openedAt} – {closedAt ?? "∞"}
          </span>
        </div>

        {threshold && (
          <div className="flex items-center">
            <LucideReact.ShoppingCart className="mr-2 text-gray-400" size={16} />
            <span>Min. Purchase: {threshold}</span>
          </div>
        )}

        {limit && isAmount && (
          <div className="flex items-center">
            <LucideReact.Shield className="mr-2 text-gray-400" size={16} />
        ...
        {expiredInDays && (
          <div className="col-span-full flex items-center">
            ...
            <span>Expires In: {expiredInDays}</span>
          </div>
        )}
        ...
        {expiredAtDate && (
          <div className="col-span-full flex items-center">
            <LucideReact.Calendar className="mr-2 text-gray-400" size={16} />
            <span>Ticket Expires: {expiredAtDate}</span>
          </div>
        )}

        <div className="col-span-full flex items-center">
          <LucideReact.Filter className="mr-2 text-gray-400" size={16} />
          <span>
        ...
        <div className="col-span-full flex items-center">
          <LucideReact.Info className="mr-2 text-gray-400" size={16} />
