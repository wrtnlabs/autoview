...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const status =
    openedDate && now < openedDate
      ? "upcoming"
      : closedDate && now > closedDate
        ? "expired"
        : "active";
  const statusIcon = {
    upcoming: <LucideReact.Clock className="text-amber-500" size={16} />,
    active: <LucideReact.CheckCircle className="text-green-500" size={16} />,
    expired: <LucideReact.XCircle className="text-red-500" size={16} />,
  }[status];
  const statusLabel = {
    upcoming: "Upcoming",
    active: "Active",
  ...
  const discountLabel =
    value.discount.unit === "amount"
      ? `$${value.discount.value}${value.discount.multiplicative ? "×" : ""} off`
      : `${value.discount.value}% off`;
  const thresholdLabel =
    value.discount.threshold !== null
      ? `Min spend: $${value.discount.threshold}`
      : "";
  const limitLabel =
    value.discount.limit !== null
      ? `Max discount: $${value.discount.limit}`
      : "";

  const criteriaCount = value.criterias.reduce(
    (acc, c) => {
  ...
  const restrictionAccessIcon =
    value.restriction.access === "public" ? (
      <LucideReact.Globe className="text-blue-500" size={16} />
    ) : (
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4">
      ...
      <div className="flex items-center gap-2 text-gray-700">
        {value.discount.unit === "amount" ? (
          <LucideReact.DollarSign size={20} className="text-indigo-500" />
        ) : (
          <LucideReact.Percent size={20} className="text-indigo-500" />
        )}
        <div>
          <p className="text-xl font-bold text-gray-900">{discountLabel}</p>
          <p className="text-sm text-gray-500">
            {thresholdLabel} {limitLabel}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="text-sm text-gray-600">
            <p>Inventory</p>
            <p>
              {value.inventory.volume ?? "∞"} left / per user:{" "}
        ...
        {/* Access */}
        <div className="flex items-center space-x-2">
          {restrictionAccessIcon}
          <div className="text-sm text-gray-600">
            Access: {value.restriction.access}
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="text-sm text-gray-600">
            ...
            <p>
              {value.opened_at
                ? new Date(value.opened_at).toLocaleDateString()
                : "Any"}{" "}
              –{" "}
              {value.closed_at
                ? new Date(value.closed_at).toLocaleDateString()
                : "∞"}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="text-sm text-gray-600">
            <p>Designer since</p>
            <p>{new Date(value.designer.created_at).toLocaleDateString()}</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        {value.criterias.length === 0 ? (
          <p className="text-sm text-gray-500">No specific criteria</p>
        ) : (
          <div className="flex flex-wrap gap-2 mt-2">
            {Object.entries(criteriaCount).map(([type, count]) => {
              const icons: Record<string, JSX.Element> = {
                section: (
                  <LucideReact.Tag size={14} className="text-pink-500" />
                ),
                seller: (
                  <LucideReact.Users size={14} className="text-green-500" />
                ),
                sale: (
                ...
                funnel: (
                  <LucideReact.Filter size={14} className="text-amber-500" />
                ),
              };
