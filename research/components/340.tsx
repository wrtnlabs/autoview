  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-gray-500">
        ...
        <p className="mt-4 text-lg">No events to display.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      <ul className="divide-y divide-gray-200">
        {events.map((event) => (
          <li key={event.id} className="flex space-x-4 py-4">
            <img
              src={event.actor.avatar_url}
              alt={event.actor.display_login || event.actor.login}
            ...
            <div className="flex-1 space-y-1">
              ...
              {event.payload.action && (
                <div className="text-sm text-gray-600">
                  Action:&nbsp;
                  <span className="font-medium">{event.payload.action}</span>
                </div>
              )}
              {event.payload.issue && (
              ...
              <div className="flex items-center text-sm text-gray-400">
                ...
                <time className="ml-1">{formatDate(event.created_at)}</time>
              </div>
            </div>
          </li>
        ))}
