...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const referrerText = value.referrer ?? 'None';
  const citizen = value.citizen;
  const member = value.member;
  const external = value.external_user;
  const citizenDate = citizen ? new Date(citizen.created_at).toLocaleDateString() : '';
  const memberDate = member ? new Date(member.created_at).toLocaleDateString() : '';
  const externalDate = external ? new Date(external.created_at).toLocaleDateString() : '';
  // Prepare emails display for member
  let emailsDisplay = '';
  if (member) {
    const emails = member.emails.map((e) => e.value);
    emailsDisplay =
      emails.length > 2
        ? `${emails.slice(0, 2).join(', ')} +${emails.length - 2}`
        : emails.join(', ');
    if (!emailsDisplay) emailsDisplay = 'None';
  }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4 max-w-full">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-800">Customer Connection</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700 text-sm">
        <div className="flex items-center gap-1">
          ...
          <span className="ml-1 text-gray-500">({value.channel.code})</span>
        </div>
        <div className="flex items-center gap-1">
          <LucideReact.Link size={16} className="text-gray-400" />
          <span className="truncate break-all">{value.href}</span>
        </div>
        <div className="flex items-center gap-1">
          <LucideReact.Link size={16} className="text-gray-400" />
          <span className="truncate break-all">{referrerText}</span>
        </div>
        <div className="flex items-center gap-1">
          ...
          {citizen && (
            <div className="bg-gray-50 p-3 rounded">
              ...
              <div className="mt-2 text-sm text-gray-600 space-y-1">
                ...
                <div>Mobile: {citizen.mobile}</div>
                <div>Verified: {citizenDate}</div>
              </div>
            </div>
          )}
          {member && (
            <div className="bg-gray-50 p-3 rounded">
              ...
              <div className="mt-2 text-sm text-gray-600 space-y-1">
                ...
                <div>Emails: {emailsDisplay}</div>
                <div>Joined: {memberDate}</div>
              </div>
            </div>
          )}
          {external && (
            <div className="bg-gray-50 p-3 rounded">
              <div className="flex items-center gap-2">
                <LucideReact.ExternalLink size={16} className="text-gray-500" />
                <span className="font-medium text-gray-800">External User</span>
              </div>
              <div className="mt-2 text-sm text-gray-600 space-y-1">
                <div>App: {external.application}</div>
                <div>UID: {external.uid}</div>
                <div>Nickname: {external.nickname}</div>
                <div>Connected: {externalDate}</div>
              </div>
            </div>
          )}
        </div>
