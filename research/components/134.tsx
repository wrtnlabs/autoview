...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const allPrices = value.units.flatMap((unit) =>
    unit.stocks.map((s) => s.price.real),
  );
  ...
  // Choose a thumbnail or fallback placeholder
  ...
  const updatedDate = new Date(value.updated_at).toLocaleDateString();

  // Truncate description
  const description = value.content.body;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-md w-full bg-white rounded-lg shadow-md overflow-hidden flex flex-col md:flex-row">
      <div className="md:w-48 w-full h-48 bg-gray-100 flex-shrink-0">
        <img
          src={imageUrl}
      ...
      <div className="p-4 flex flex-col justify-between flex-1">
        ...
        <div className="mt-4 space-y-2">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-gray-800">{priceDisplay}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <StatusIcon size={16} className={statusClass} />
            <span className={`uppercase font-medium ${statusClass}`}>
              {statusText}
            </span>
          </div>
