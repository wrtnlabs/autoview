...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  //    Get the latest question snapshot
  const latestQuestionSnapshot = value.snapshots.reduce((prev, curr) =>
  ...
  //    If there's an answer, get its latest snapshot and formatted date
  const hasAnswer = value.answer !== null;
  let latestAnswerSnapshot: AutoViewInputSubTypes.IBbsArticle.ISnapshot | null =
    null;
  let formattedAnswerDate = "";
  if (hasAnswer && value.answer) {
    latestAnswerSnapshot = value.answer.snapshots.reduce((prev, curr) =>
      new Date(prev.created_at) > new Date(curr.created_at) ? prev : curr,
    );
    formattedAnswerDate = new Date(value.answer.created_at).toLocaleString(
      undefined,
      {
        year: "numeric",
        month: "short",
  ...
  return (
    <div className="max-w-md mx-auto bg-white shadow rounded-lg p-4 space-y-4">
      {/* Question Header */}
      ...
      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
        <div className="flex items-center">
          ...
          <span>
            {latestQuestionSnapshot.files.length} attachment
            {latestQuestionSnapshot.files.length !== 1 ? "s" : ""}
          </span>
        </div>
        <div className="flex items-center">
        ...
        <div className="flex items-center">
          ...
          <span className="ml-1">
            {value.read_by_seller ? "Viewed by seller" : "Not viewed"}
          </span>
        </div>
      </div>
      ...
      {hasAnswer && latestAnswerSnapshot && (
        <div className="border-t pt-4 space-y-2">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-500">{formattedAnswerDate}</span>
          </div>
          <p className="text-gray-700 text-sm line-clamp-3">
