...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Data transformation: sort issues by creation date (newest first) and date formatter
  ...
  if (sortedIssues.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-gray-400">
        ...
        <p className="mt-2 text-lg">No issues to display</p>
      </div>
    );
  }

  // 3. Compose visual structure: list of issue cards
  return (
    <div className="space-y-4">
      {sortedIssues.map((issue) => (
        ...
        >
          {/* Issue preview: avatar, title, number, date */}
          ...
          {/* Metadata: comments count and state */}
          <div className="mt-3 flex items-center space-x-6 sm:mt-0">
            ...
            <div className="flex items-center text-sm">
              {issue.state === "open" ? (
                <LucideReact.Circle size={16} className="text-green-500 mr-1" />
              ) : (
          ...
          {issue.labels.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2 sm:mt-0 sm:ml-6">
