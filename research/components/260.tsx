...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const lastMessageText = value.message?.plainText ?? "";
  ...
  return (
    <article className="p-4 bg-white rounded-lg shadow-md flex items-start space-x-3">
      ...
      <div className="flex-1 min-w-0">
        {/* Header: Name, unread badge, time */}
        <div className="flex items-center">
          <h3 className="font-medium text-gray-900 truncate">{userName}</h3>
          {unreadCount > 0 && (
            <span className="ml-2 bg-blue-500 text-white text-xs font-semibold rounded-full px-2">
              {unreadCount}
            </span>
          )}
          {formattedTime && (
            <time
              dateTime={value.message?.createdAt?.toString() ?? ""}
          ...
          <div className="mt-2 flex flex-wrap">
            {tags.map((tag) => (
              <span
                ...
                className="mr-2 mb-1 bg-gray-100 text-gray-700 text-xs font-medium rounded px-2 py-0.5"
              >
                {tag.name}
              </span>
            ))}
