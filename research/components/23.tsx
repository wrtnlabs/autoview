...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const creationDate = new Date(value.created_at).toLocaleString(undefined, {
    dateStyle: "medium",
    timeStyle: "short",
  });
  ...
  const editedCount = snapshotCount > 1 ? snapshotCount - 1 : 0;
  const editedDate = editedCount
  ...
  const commentBody = latestSnapshot.body;
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {editedCount > 0 && (
              <div className="flex items-center text-gray-500 text-sm">
                ...
                <span>
                  Edited {editedCount} time{editedCount > 1 ? "s" : ""}
                </span>
              </div>
            )}
          </div>
          <div className="mt-2 text-gray-700 text-sm whitespace-pre-wrap line-clamp-3">
            {commentBody}
          </div>
          {files.length > 0 && (
