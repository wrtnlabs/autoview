...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const roleLabel = writer.type.charAt(0).toUpperCase() + writer.type.slice(1);
  ...
  if (latestSnapshot.format === 'html') {
    bodyContent = (
      <div
        className="text-gray-700 text-sm prose prose-sm line-clamp-4"
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm border border-gray-200 max-w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center text-gray-500 text-xs">
          ...
          <time dateTime={latestSnapshot.created_at} className="ml-1">
            {formattedDate}
          </time>
        </div>
      </div>
      ...
      {latestSnapshot.files.length > 0 && (
        <div className="mt-4">
          <span className="font-medium text-gray-800 text-sm">Attachments:</span>
          <ul className="mt-2 flex flex-col space-y-2">
            {latestSnapshot.files.map((file) => (
              <li key={file.url} className="flex items-center space-x-1">
                <LucideReact.FileText size={16} className="text-indigo-500 flex-shrink-0" />
                <a
                  href={file.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-indigo-600 hover:underline truncate text-sm"
                >
                  {file.name}
