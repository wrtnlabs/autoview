...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const writerDisplay: string = (() => {
    if (typeof value.writer === 'string') return value.writer;
    if (value.writer && typeof value.writer === 'object') {
      const obj = value.writer as any;
      return obj.name ?? obj.username ?? JSON.stringify(obj);
    }
    return String(value.writer);
  })();

  const createdAt: string = new Date(value.created_at).toLocaleString(undefined, {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
  ...
  const snapshotDate: string = latestSnapshot
  ...
  const rawBody: string = latestSnapshot?.body ?? '';
  const maxLength = 200;
  const truncatedBody: string =
  ...
  return (
    <div className="w-full max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-gray-900">{writerDisplay}</span>
        <span className="text-xs text-gray-500">{snapshotDate || createdAt}</span>
      </div>

      <p className="text-gray-800 text-sm whitespace-pre-wrap line-clamp-4 mb-2">
        {truncatedBody}
      </p>
