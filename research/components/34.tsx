...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Reviewer name resolution: prefer member nickname, then citizen name, then external user nickname.
  ...
  // Format the main review date
  const formattedReviewDate = new Date(value.created_at).toLocaleDateString(
    undefined,
    {
      year: "numeric",
      month: "short",
  ...
  // Choose the latest review snapshot by created_at
  const latestSnapshot = value.snapshots.reduce((prev, curr) => {
    return new Date(curr.created_at) > new Date(prev.created_at) ? curr : prev;
  }, value.snapshots[0]);

  // Truncate long bodies for performance and mobile-friendliness
  const truncate = (text: string, max = 100) =>
    text.length > max
      ? text.replace(/\s+/g, " ").slice(0, max).trim() + "…"
      : text;

  const reviewSnippet = truncate(latestSnapshot.body, 120);

  // Prepare answer data if available
  const hasAnswer = value.answer !== null;
  let answerSnapshot: AutoViewInputSubTypes.IBbsArticle.ISnapshot | undefined;
  let formattedAnswerDate = "";
  let answerSnippet = "";

  if (hasAnswer && value.answer) {
    formattedAnswerDate = new Date(value.answer.created_at).toLocaleDateString(
      undefined,
      { year: "numeric", month: "short", day: "numeric" },
    );
    answerSnapshot = value.answer.snapshots.reduce((prev, curr) => {
      return new Date(curr.created_at) > new Date(prev.created_at)
        ? curr
        : prev;
    }, value.answer.snapshots[0]);
    answerSnippet = truncate(answerSnapshot.body, 120);
  }
  ...
  if (!latestSnapshot) {
    return (
      <div className="p-4 bg-white rounded-lg shadow flex items-center justify-center text-gray-500">
        ...
        <span className="ml-2">No review available.</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4">
      ...
      {/* Body Snippet */}
      <p className="text-sm text-gray-700 line-clamp-3">{reviewSnippet}</p>
      ...
      {/* Seller's Answer */}
      {hasAnswer && answerSnapshot && (
        <div className="mt-4 p-3 bg-gray-50 rounded-lg border-l-4 border-blue-500 space-y-2">
          <div className="flex items-center space-x-2">
          ...
          <p className="text-sm text-gray-700 line-clamp-3">{answerSnippet}</p>
        </div>
      )}

      {/* Read Status */}
      <div className="flex items-center text-sm">
        <span className="text-gray-500">Seller viewed:</span>
        {value.read_by_seller ? (
          <LucideReact.CheckCircle className="ml-2 text-green-500" size={16} />
        ) : (
          <LucideReact.Clock className="ml-2 text-amber-500" size={16} />
        )}
      </div>
    </div>
  );
