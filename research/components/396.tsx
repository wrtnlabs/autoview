...
export type AutoViewInput =
  AutoViewInputSubTypes.IApiOrgsActionsRunnerGroupsHostedRunners.GetResponse;
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-lg">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-800">Hosted Runners</h2>
      ...
      {value.runners.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {value.runners.map((runner) => {
            const lastActive =
              runner.last_active_on && runner.last_active_on !== null
              ...
              >
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{runner.machine_size_details.memory_gb} GB RAM</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <LucideReact.Database size={16} className="text-gray-500" />
                    <span>{runner.machine_size_details.storage_gb} GB SSD</span>
                  </div>
                  <div className="flex items-center gap-1">
                  ...
                  {runner.image_details ? (
                    ...
                    <div className="flex items-center gap-1">
                      <LucideReact.ImageOff
                        size={16}
                        className="text-gray-400"
                      />
                      <span>No image info</span>
                    </div>
                  )}
                  <div className="flex items-center gap-1">
                    {runner.public_ip_enabled ? (
                      <LucideReact.CheckCircle
                        size={16}
                        className="text-green-500"
                        aria-label="Public IP enabled"
                      />
                    ) : (
                      <LucideReact.XCircle
                        size={16}
                        className="text-gray-400"
                        aria-label="Public IP disabled"
                      />
                    )}
                    <span>
                      {runner.public_ip_enabled
                        ? `Public IPs: ${runner.public_ips?.length ?? 0}`
                        : "No Public IP"}
                    </span>
                  </div>
                </div>
