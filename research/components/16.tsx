...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const formattedValue = `${value.value.toLocaleString()} points`;
  const donor = value.citizen;
  const admin = value.administrator;
  const adminNickname = admin.member.nickname;
  const adminEmail = admin.member.emails[0]?.value ?? "N/A";
  const adminMobile = admin.citizen.mobile;
  const donorJoined = new Date(donor.created_at).toLocaleDateString();
  const adminJoined = new Date(admin.created_at).toLocaleDateString();
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-6">
      ...
      <div className="flex items-center space-x-2">
        <LucideReact.Gift className="text-indigo-500" size={20} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="flex items-center space-x-2">
          <LucideReact.Gift className="text-indigo-500" size={16} />
          <span className="text-sm text-gray-600">
            Amount:{" "}
            <span className="font-medium text-gray-800">{formattedValue}</span>
          </span>
        </div>
        <div className="sm:col-span-2">
          <h3 className="text-sm font-medium text-gray-800">Reason</h3>
          <p className="mt-1 text-gray-700 line-clamp-3">{value.reason}</p>
        </div>
      </div>

      {/* Donor & Administrator Details */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Donor Card */}
        <div className="border border-gray-200 p-4 rounded-md">
          <h3 className="flex items-center text-sm font-medium text-gray-800">
            <LucideReact.User className="text-gray-500 mr-1" size={16} />
            Donor
          </h3>
          <p className="mt-1 text-gray-700">{donor.name}</p>
          <div className="flex items-center text-gray-600 text-sm mt-2">
            <LucideReact.Phone className="text-gray-500" size={14} />
            <span className="ml-1">{donor.mobile}</span>
          </div>
          <div className="flex items-center text-gray-600 text-sm mt-2">
            <LucideReact.Calendar className="text-gray-500" size={14} />
            <span className="ml-1">Joined {donorJoined}</span>
          </div>
        </div>
        ...
        <div className="border border-gray-200 p-4 rounded-md">
          <h3 className="flex items-center text-sm font-medium text-gray-800">
            <LucideReact.UserCheck className="text-gray-500 mr-1" size={16} />
            Administrator
          </h3>
          <p className="mt-1 text-gray-700">{adminNickname}</p>
          <div className="flex items-center text-gray-600 text-sm mt-2">
            <LucideReact.Mail className="text-gray-500" size={14} />
            <span className="ml-1">{adminEmail}</span>
          </div>
          <div className="flex items-center text-gray-600 text-sm mt-2">
            <LucideReact.Phone className="text-gray-500" size={14} />
            <span className="ml-1">{adminMobile}</span>
          </div>
          <div className="flex items-center text-gray-600 text-sm mt-2">
            <LucideReact.Calendar className="text-gray-500" size={14} />
            <span className="ml-1">Joined {adminJoined}</span>
          </div>
        </div>
      </div>
    </div>
