...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const actorName = actor?.login ?? "Unknown";
  const formattedDate = new Date(value.created_at).toLocaleString(undefined, {
    dateStyle: "medium",
    timeStyle: "short",
  });
  const commitSha = value.commit_id ? value.commit_id.slice(0, 7) : null;
  const avatarSrc =
    actor?.avatar_url ??
  ...
  // Visual structure
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm flex flex-col space-y-3 max-w-md w-full">
      {/* Header: Actor, Event, Date */}
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-gray-800">
            {actorName} <span className="text-gray-500">performed</span>{" "}
            <span className="capitalize">{value.event.replace(/_/g, " ")}</span>
          </p>
      ...
      <div className="flex flex-col space-y-2 text-sm text-gray-700">
        ...
        {value.milestone && (
          <div className="flex items-center space-x-2">
            <LucideReact.Flag size={16} className="text-gray-500" />
            <span>Milestone: {value.milestone.title}</span>
          </div>
        )}
        ...
        {value.project_card && (
          <div className="flex items-center space-x-2">
            <LucideReact.Columns size={16} className="text-gray-500" />
            <span>Project column: {value.project_card.column_name}</span>
          </div>
        )}

        {value.assigner && value.assignee && (
          <div className="flex items-center space-x-2">
        ...
        {value.dismissed_review && (
          <div className="flex items-center space-x-2">
            ...
            <span>Dismissed review #{value.dismissed_review.review_id}</span>
          </div>
        )}

        {value.lock_reason !== undefined && (
          <div className="flex items-center space-x-2">
            <LucideReact.Lock size={16} className="text-gray-500" />
            <span>
              Locked{value.lock_reason ? `: ${value.lock_reason}` : ""}
            </span>
          </div>
        )}
      </div>
