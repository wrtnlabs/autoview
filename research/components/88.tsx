...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const categoryNames = value.categories.map((c) => c.name).join(", ");
  const tags = value.tags;
  const formattedCreatedAt = new Date(value.created_at).toLocaleDateString(
    "en-US",
    { month: "short", day: "numeric", year: "numeric" },
  );
  const formattedOpenedAt = value.opened_at
    ? new Date(value.opened_at).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      })
  ...
  const formattedClosedAt = value.closed_at
    ? new Date(value.closed_at).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      })
  ...
  const priceValues = allStocks.map((stock) => stock.price.real);
  const hasPrices = priceValues.length > 0;
  const minPrice = hasPrices ? Math.min(...priceValues) : 0;
  const maxPrice = hasPrices ? Math.max(...priceValues) : 0;
  ...
  const availabilityCount = allStocks.reduce(
    (sum, stock) => sum + (stock.inventory.income - stock.inventory.outcome),
    0,
  );
  const stockStatus = availabilityCount > 0 ? "In Stock" : "Out of Stock";

  // 2. Compose the visual structure
  return (
    <div className="p-4 bg-white rounded-lg shadow-md flex flex-col md:flex-row gap-4">
      <div className="w-full md:w-1/3 aspect-video bg-gray-100 rounded overflow-hidden">
        <img
          src={thumbnailUrl}
          alt={title}
      ...
      <div className="flex-1 flex flex-col gap-2">
        ...
        <div className="flex items-center text-sm text-gray-500">
          ...
          <span className="ml-1">{sectionName}</span>
        </div>
        ...
        {formattedOpenedAt && (
          <div className="flex items-center text-sm text-gray-500">
            <LucideReact.Calendar size={16} />
            <span className="ml-1">Opened: {formattedOpenedAt}</span>
          </div>
        )}
        {formattedClosedAt && (
          <div className="flex items-center text-sm text-gray-500">
            <LucideReact.Calendar size={16} />
            <span className="ml-1">Closed: {formattedClosedAt}</span>
          </div>
        )}

        <div className="flex items-center text-sm text-gray-500">
          <LucideReact.Tag size={16} />
          <span className="ml-1">{categoryNames}</span>
        </div>
        ...
        <div className="flex items-center text-sm text-gray-700 mt-2">
          ...
          <span className="ml-1">Price: {priceText}</span>
        </div>
        <div className="flex items-center text-sm text-gray-700">
          ...
          <span className="ml-1">{stockStatus}</span>
        </div>
      </div>
    </div>
  );
