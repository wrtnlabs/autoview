...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const description = value.content.body.replace(/\s+/g, ' ').trim();
  const truncatedDescription =
    description.length > 120 ? description.slice(0, 120) + '…' : description;
  const imageUrl = value.content.thumbnails[0]?.url || '';
  const categoryNames = value.categories.map((c) => c.name);
  const tagNames = value.tags;

  // Gather all real & nominal prices
  const allPrices = value.units.flatMap((unit) =>
    unit.stocks.map((stock) => stock.price)
  );
  const realPrices = allPrices.map((p) => p.real);
  const nominalPrices = allPrices.map((p) => p.nominal);
  const minReal = realPrices.length ? Math.min(...realPrices) : 0;
  const maxReal = realPrices.length ? Math.max(...realPrices) : 0;
  const minNominal = nominalPrices.length ? Math.min(...nominalPrices) : 0;
  const maxNominal = nominalPrices.length ? Math.max(...nominalPrices) : 0;
  ...
  let status = 'Open';
  if (value.opened_at === null) status = 'Scheduled';
  if (value.suspended_at) status = 'Suspended';
  else if (value.paused_at) status = 'Paused';
  else if (value.closed_at) status = 'Closed';
    ...
    <div className="max-w-sm bg-white rounded-lg shadow-md overflow-hidden flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={`text-xs font-medium uppercase px-2 py-1 rounded ${statusClass}`}
          >
            {status}
          </span>
        </div>
        ...
        {tagNames.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">
            {tagNames.map((tag) => (
              <span
                key={tag}
                className="text-xs text-blue-700 bg-blue-100 px-2 py-0.5 rounded"
              >
                {tag}
              </span>
            ))}
          ...
          {minNominal > minReal && (
            <div className="text-sm text-gray-500 line-through">
              {minNominal === maxNominal
                ...
                : `${formatCurrency(minNominal)} – ${formatCurrency(
                    maxNominal
                  )}`}
            </div>
          )}
