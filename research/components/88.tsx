...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const allPrices = value.units.flatMap((unit) =>
    unit.stocks.map((stock) => stock.price.real)
  );
  ...
  const excerpt =
    value.content.body.length > 150
      ? value.content.body.slice(0, 150) + '…'
      : value.content.body;
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      {/* Section & Seller */}
      <div className="flex flex-wrap text-sm text-gray-600 space-x-3 mb-3">
        <div className="flex items-center">
          <LucideReact.MapPin size={16} className="mr-1" aria-label="Section" />
          <span className="truncate">{value.section.name}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.User size={16} className="mr-1" aria-label="Seller" />
          <span className="truncate">{sellerName}</span>
        </div>
      </div>

      {/* Price & Date */}
      <div className="flex flex-wrap items-center text-gray-700 text-sm space-x-4 mb-3">
        <div className="flex items-center">
          <LucideReact.Tag size={16} className="mr-1" aria-label="Price" />
          <span>{priceDisplay}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.Calendar size={16} className="mr-1" aria-label="Created at" />
      ...
      {value.tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
          {value.tags.map((tag) => (
            <span
              key={tag}
              className="flex items-center space-x-1 px-2 py-1 bg-blue-50 text-blue-600 text-xs rounded"
            >
              <LucideReact.Tag size={12} aria-label="Tag" />
              <span>{tag}</span>
            </span>
          ))}
        </div>
