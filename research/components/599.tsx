...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const startedAt = value.run_started_at
    ? new Date(value.run_started_at)
    : createdAt;
  ...
  let StatusIcon = LucideReact.HelpCircle;
  let statusColor = "text-gray-500";
  let statusText = (value.status ?? "unknown")
    .replace(/_/g, " ")
    ...
    case "completed":
      switch (value.conclusion) {
        ...
        case "neutral":
          ...
          statusText = value.conclusion === "neutral" ? "Neutral" : "Success";
          break;
        case "failure":
        case "timed_out":
        case "cancelled":
          ...
          statusText =
            value.conclusion === "cancelled" ? "Cancelled" : "Failed";
          break;
        default:
          StatusIcon = LucideReact.HelpCircle;
  ...
  // Fallback avatar URL if actor image fails
  const actorName = value.actor?.name ?? value.actor?.login ?? "";
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      {/* Subtitle: Workflow name or path */}
      ...
      <div className="mt-3 flex items-center space-x-4">
        <div className="flex items-center">
          <img
            src={value.actor?.avatar_url || avatarFallback}
            alt={value.actor?.login || "actor avatar"}
            onError={(e) => {
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-gray-600">
        <div className="flex items-center">
          ...
          <span>{value.head_branch || "N/A"}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.Code size={16} className="mr-1 text-gray-400" />
          <span className="font-mono">{shortSha}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.Hash size={16} className="mr-1 text-gray-400" />
          <span>
            Run #{value.run_number}
            {value.run_attempt ? ` (Attempt ${value.run_attempt})` : ""}
          </span>
        </div>
        <div className="flex items-center">
      ...
      {/* Footer: Run URL */}
      <div className="mt-4 border-t pt-3 text-xs text-gray-500 truncate">
        <LucideReact.Link
          size={14}
