...
export type AutoViewInput =
  AutoViewInputSubTypes.codespace_with_full_repository;
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const memoryGB = value.machine
    ? (value.machine.memory_in_bytes / 1024 ** 3).toFixed(1)
    : null;

  // Choose an icon based on codespace state
  const stateIcon = (() => {
    switch (value.state) {
      case "Available":
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="mt-3 space-y-1 text-gray-700 text-sm">
        ...
        {value.idle_timeout_minutes != null && (
          <div className="flex items-center gap-1">
            <LucideReact.Timer size={16} className="text-gray-400" />
        ...
        <div className="mt-2 space-y-2 text-gray-700 text-sm">
          ...
          <div className="flex items-center gap-4 text-gray-600">
            <div className="flex items-center gap-1">
              ...
              <span>{value.repository.stargazers_count}</span>
            </div>
            <div className="flex items-center gap-1">
              <LucideReact.GitBranch size={16} />
              <span>{value.repository.forks_count}</span>
            </div>
            <div className="flex items-center gap-1">
              <LucideReact.Eye size={16} />
              <span>{value.repository.watchers_count}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {value.machine && (
        <div className="mt-4 border-t pt-4">
          <h3 className="text-lg font-semibold text-gray-800">Machine</h3>
          <div className="mt-2 space-y-1 text-gray-700 text-sm">
            ...
            <div>
              <span className="font-medium">CPUs:</span> {value.machine.cpus}
            </div>
            {memoryGB && (
              <div>
                <span className="font-medium">Memory:</span> {memoryGB} GB
              </div>
            )}
          </div>
        </div>
      ...
      {value.git_status && (
        <div className="mt-4 border-t pt-4">
          <h3 className="text-lg font-semibold text-gray-800">Git Status</h3>
          <div className="mt-2 space-y-1 text-gray-700 text-sm">
            {typeof value.git_status.ahead === "number" && (
              <div>Ahead: {value.git_status.ahead}</div>
            )}
            {typeof value.git_status.behind === "number" && (
              <div>Behind: {value.git_status.behind}</div>
            )}
            {value.git_status.has_unpushed_changes && (
              <div>Unpushed changes</div>
            )}
            {value.git_status.has_uncommitted_changes && (
              <div>Uncommitted changes</div>
            )}
            {value.git_status.ref && <div>Ref: {value.git_status.ref}</div>}
          </div>
        </div>
      )}
    </div>
