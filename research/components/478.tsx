...
export type AutoViewInput =
  AutoViewInputSubTypes.IApiOrgsMembersCodespaces.GetResponse;
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Map codespace state to semantically appropriate icons
  const getStateIcon = (
    state: AutoViewInputSubTypes.codespace["state"],
  ): JSX.Element => {
    const commonProps = { size: 16 as const, className: "flex-shrink-0" };
    switch (state) {
      case "Available":
        return (
  ...
  // Format ISO date strings into a user-friendly format
  ...
  // Mobile-first responsive card grid, with an empty state when no codespaces exist.
  return (
    <div className="p-4">
      ...
      {codespaces.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 text-gray-500">
          <LucideReact.AlertCircle
            ...
            aria-label="No codespaces"
          />
          <p className="text-sm">No codespaces available.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {codespaces.map((cs) => {
            ...
            const machineName = cs.machine?.display_name;
            const lastUsed = formatDate(cs.last_used_at);
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-start">
                  <h3 className="text-md font-medium text-gray-800 truncate flex-1">
                    {displayName}
                  </h3>
                  <div className="ml-2">{getStateIcon(cs.state)}</div>
                </div>
                {/* Repository */}
                <div className="flex items-center text-gray-600 text-sm mt-2 truncate">
                  <LucideReact.GitBranch size={16} className="text-gray-500" />
                  <span className="ml-1 truncate">{repoName}</span>
                </div>
                {/* Machine */}
                {machineName && (
                  <div className="flex items-center text-gray-600 text-sm mt-1 truncate">
                    <LucideReact.Cpu size={16} className="text-gray-500" />
                    <span className="ml-1 truncate">{machineName}</span>
                  </div>
                )}
                {/* Dates */}
