...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const sessionUnread = value.session?.unread ?? 0;
  ...
  const chatState = chatStateMap[value.userChat?.state || ""] || "Unknown";
  // Extract a plain text snippet from the last message
  let messageText = "";
  if (value.message?.plainText) {
    messageText = value.message.plainText;
  } else if (value.message?.blocks) {
    messageText = value.message.blocks
      .map(b => b.value || "")
      .join(" ")
      .trim();
  }
  const snippet = messageText.length > 100 ? messageText.slice(0, 100) + "…" : messageText;
    ...
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md flex flex-col space-y-4">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1">
          <h2 className="text-lg font-medium text-gray-900 truncate">{userName}</h2>
          <p className="text-sm text-gray-500">{chatState}</p>
        </div>
        {sessionUnread > 0 && (
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span
              key={tag.key}
              className="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded-full truncate"
            >
              {tag.name}
            </span>
          ))}
