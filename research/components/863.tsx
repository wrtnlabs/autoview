...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const enforcementMappings: Record<
  ...
  > = {
    disabled: {
      ...
      icon: <LucideReact.Slash size={16} className="text-gray-500" />
    },
    active: {
      label: "Active",
    ...
    evaluate: {
      ...
      icon: <LucideReact.Eye size={16} className="text-amber-500" />
    }
  };
  const enforcement = enforcementMappings[value.enforcement];
  ...
  // Conditions summary
  let conditionsJSX: React.ReactNode = (
    <span className="text-gray-500">None</span>
  );
  if (value.conditions) {
    const cond = value.conditions as
    ...
    if ("ref_name" in cond && cond.ref_name) {
      if (cond.ref_name.include?.length)
        parts.push(`Ref include: ${cond.ref_name.include.join(", ")}`);
      if (cond.ref_name.exclude?.length)
        parts.push(`Ref exclude: ${cond.ref_name.exclude.join(", ")}`);
    }
    if ("repository_name" in cond) {
      if (cond.repository_name.include?.length)
        parts.push(`Repo include: ${cond.repository_name.include.join(", ")}`);
      if (cond.repository_name.exclude?.length)
        parts.push(`Repo exclude: ${cond.repository_name.exclude.join(", ")}`);
    } else if ("repository_id" in cond) {
      if (cond.repository_id.repository_ids?.length)
        parts.push(
          `Repo IDs: ${cond.repository_id.repository_ids.join(", ")}`
        );
    } else if ("repository_property" in cond) {
      const rp = cond.repository_property;
      if (rp.include?.length)
        parts.push(
          `Prop include: ${rp.include.map(i => i.name).join(", ")}`
        );
      if (rp.exclude?.length)
        parts.push(
          `Prop exclude: ${rp.exclude.map(i => i.name).join(", ")}`
        );
    }
    if (parts.length) {
      conditionsJSX = (
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
        ...
        <span
          className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded ${enforcement.className}`}
        >
          {enforcement.icon}
          <span className="ml-1">{enforcement.label}</span>
        </span>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-2 text-sm">
        <span className="flex items-center text-gray-600">
          ...
          <span className="ml-1 capitalize">{value.target ?? "All"}</span>
        </span>
        <span className="flex items-center text-gray-600">
          <LucideReact.Users size={16} className="text-gray-400" />
          <span className="ml-1">
            {value.source_type ?? "Repository"}: {value.source}
          </span>
        </span>
        {value.current_user_can_bypass && (
          <span className="flex items-center text-gray-600">
            ...
            <span className="ml-1">
              {value.current_user_can_bypass.replace(/_/g, " ")}
            </span>
          </span>
        )}
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          <h3 className="font-medium text-gray-700 mb-1">Conditions</h3>
          {conditionsJSX}
        </div>
          ...
          {rulesCount > 0 ? (
            <div className="flex flex-wrap gap-2">
              {ruleTypes.map(type => (
                <span
                  key={type}
                  className="px-2 py-0.5 bg-gray-100 text-gray-800 rounded text-xs capitalize"
                >
                  {type.replace(/_/g, " ")}
                </span>
              ))}
        ...
        <div>
          <h3 className="font-medium text-gray-700 mb-1">Bypass Actors</h3>
          {bypassCount > 0 ? (
            <span className="text-gray-600">
              {bypassCount} actor{bypassCount > 1 ? "s" : ""}
            </span>
          ) : (
            <span className="text-gray-500">None</span>
          )}
        ...
        <div>
          <h3 className="font-medium text-gray-700 mb-1">Timestamps</h3>
