...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const displayedItems = value.items.slice(0, MAX_DISPLAY);
  const remainingCount = value.items.length - displayedItems.length;
        ...
        {displayedItems.map((item) => (
          <li key={item.id} className="p-4 bg-gray-50 rounded-lg">
            <div className="flex items-start justify-between">
              ...
              <span className="ml-2 text-xs text-gray-500">#{item.number}</span>
            </div>
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {item.labels
                  ...
                  .map((label) => {
                    ...
                    const textClass = label.color
                      ? "text-white"
                      : "text-gray-800";
                    const bgClass = label.color ? "" : "bg-gray-200";
                    return (
                      <span
                        key={label.name}
                        className={`text-xs font-medium px-2 py-0.5 rounded ${textClass} ${bgClass}`}
                        style={
