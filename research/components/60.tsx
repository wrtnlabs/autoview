...
export namespace AutoViewInputSubTypes {
    ...
    */
    export interface IPageIShoppingCouponTicket {
        /**
         * Page information.
         *
        ...
        */
        data: AutoViewInputSubTypes.IShoppingCouponTicket[];
    }
...
export type AutoViewInput = AutoViewInputSubTypes.IPageIShoppingCouponTicket;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-gray-600 text-sm">
        Page {pagination.current} of {pagination.pages} &bull; {pagination.records} ticket
      ...
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-8 text-gray-400">
          ...
          <span>No coupon tickets found.</span>
        </div>
      ) : (
        <div className="space-y-4">
          {data.map((ticket) => {
            const expired = isExpired(ticket.expired_at);
            return (
              <div
                key={ticket.id}
                className="p-4 bg-white rounded-lg shadow flex flex-col sm:flex-row sm:justify-between"
              >
                {/* Left: Coupon and status */}
                <div className="flex items-center mb-3 sm:mb-0">
                  <LucideReact.Tag className="text-indigo-500 mr-2" size={20} />
                  <h3 className="text-lg font-semibold text-gray-800 truncate">
                    {ticket.coupon.name}
                  </h3>
                  <span role="img" aria-label={expired ? "Expired" : "Active"}>
                    {expired ? (
                      <LucideReact.XCircle className="ml-3 text-red-500" size={18} />
                    ) : (
                      <LucideReact.CheckCircle className="ml-3 text-green-500" size={18} />
                    )}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm text-gray-600">
                  <div className="flex items-center">
                    ...
                    <span className="truncate">Customer: {ticket.customer.id}</span>
                  </div>
                  <div className="flex items-center">
                    <LucideReact.Hash className="text-gray-400 mr-1" size={16} />
                    <span className="truncate">
                      Ticket: {ticket.id.slice(0, 8)}…{ticket.id.slice(-4)}
                    </span>
                  </div>
                  <div className="flex items-center">
                    <LucideReact.Calendar className="text-gray-400 mr-1" size={16} />
                    <span>Issued: {formatDateTime(ticket.created_at)}</span>
                  </div>
                  <div className="flex items-center">
                    <LucideReact.Calendar className="text-gray-400 mr-1" size={16} />
                    <span>Expires: {formatDateTime(ticket.expired_at)}</span>
                  </div>
                </div>
              </div>
            );
