...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const discountLabel =
    value.discount.unit === "amount"
      ? `$${value.discount.value.toLocaleString()}`
  ...
  const validFrom = formatDate(value.opened_at);
  const validTo = formatDate(value.closed_at);
  const createdAt = formatDate(value.created_at);

  const inventoryTotal =
    value.inventory.volume !== null
      ? value.inventory.volume.toLocaleString()
      : "∞";
  const inventoryPer =
    value.inventory.volume_per_citizen !== null
      ? value.inventory.volume_per_citizen.toLocaleString()
      : "∞";

  const issueTotal =
    value.restriction.volume !== null
      ? value.restriction.volume.toLocaleString()
      : "∞";
  const issuePer =
    value.restriction.volume_per_citizen !== null
      ? value.restriction.volume_per_citizen.toLocaleString()
      : "∞";

  const criteriaLabels = value.criterias.map((c) => {
    const dir = c.direction[0].toUpperCase() + c.direction.slice(1);
    switch (c.type) {
      case "section":
        return `${dir} ${c.sections.length} section${c.sections.length !== 1 ? "s" : ""}`;
      case "seller":
        return `${dir} ${c.sellers.length} seller${c.sellers.length !== 1 ? "s" : ""}`;
      case "sale":
        return `${dir} ${c.sales.length} sale${c.sales.length !== 1 ? "s" : ""}`;
      case "funnel":
        return `${dir} ${c.funnels.length} funnel${c.funnels.length !== 1 ? "s" : ""}`;
      default:
        return "";
    }
  });
    ...
    <div className="max-w-md w-full mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center text-sm text-blue-600">
          <LucideReact.Tag size={20} className="mr-1" />
      ...
      <div className="space-y-3 text-gray-700 text-sm">
        <div className="flex items-center">
          <LucideReact.Box size={16} className="text-gray-400 mr-2" />
          <span>
            Inventory: {inventoryTotal}
            <span className="ml-1 text-gray-500">
              / {inventoryPer} per user
            </span>
          </span>
        </div>
        ...
        <div className="flex items-center">
          {value.restriction.access === "public" ? (
            <LucideReact.Globe size={16} className="text-gray-400 mr-2" />
          ) : (
            <LucideReact.Lock size={16} className="text-gray-400 mr-2" />
          )}
          <span>
            Access:{" "}
            <span className="capitalize">{value.restriction.access}</span>
          </span>
        </div>
        ...
        <div className="flex items-center">
          ...
          <span>Exclusive: {value.restriction.exclusive ? "Yes" : "No"}</span>
        </div>
        ...
        <div className="flex items-center">
          <LucideReact.CalendarClock size={16} className="text-gray-400 mr-2" />
      ...
      <div className="mt-4 text-gray-700 text-sm border-t pt-4">
        <div className="flex items-center mb-1">
          ...
          <span>
            Issue limit: {issueTotal} total, {issuePer} per person
          </span>
        </div>
        {value.restriction.expired_in !== null && (
          <div className="flex items-center">
            <LucideReact.Clock size={16} className="text-gray-400 mr-2" />
            <span>Expires in: {value.restriction.expired_in} days</span>
          </div>
        )}
        {value.restriction.expired_at && (
          <div className="flex items-center">
            <LucideReact.Calendar size={16} className="text-gray-400 mr-2" />
            <span>Expires at: {formatDate(value.restriction.expired_at)}</span>
          </div>
        )}
      </div>
          ...
          <ul className="list-disc list-inside space-y-1">
            {criteriaLabels.map((label, idx) => (
              <li key={idx} className="flex items-center">
                <LucideReact.Filter size={14} className="text-gray-400 mr-2" />
                <span>{label}</span>
              </li>
            ))}
          </ul>
