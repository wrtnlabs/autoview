...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const thresholdLabel =
    discount.threshold != null ? `$${discount.threshold}` : "None";
  const limitLabel =
    discount.limit != null ? `$${discount.limit}` : "None";

  const isAmountDiscount = discount.unit === "amount";
  const multiplicative =
    isAmountDiscount &&
    (discount as AutoViewInputSubTypes.IShoppingCouponDiscount.IAmount)
      .multiplicative;
  ...
  const criteriaCount = value.criterias.length;
  ...
  const exclusiveBadge = restriction.exclusive ? (
  ...
  const expiredIn =
    restriction.expired_in != null
      ? `${restriction.expired_in} days after issue`
      : null;
  const expiredAt =
    restriction.expired_at != null
      ? new Date(restriction.expired_at).toLocaleDateString()
      : null;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600">
        ...
        <div className="flex items-center gap-1">
          ...
          <span>Min purchase: {thresholdLabel}</span>
        </div>

        <div className="flex items-center gap-1">
          <LucideReact.DollarSign size={16} />
          <span>Max discount: {limitLabel}</span>
        </div>
        ...
        {(expiredIn || expiredAt) && (
          <div className="col-span-full flex flex-col">
            <span className="text-gray-500 text-xs">Usage expiry:</span>
            {expiredIn && (
              <span className="text-gray-700 text-sm">{expiredIn}</span>
            )}
            {expiredAt && (
              <span className="text-gray-700 text-sm">On {expiredAt}</span>
            )}
          </div>
        )}
      </div>
