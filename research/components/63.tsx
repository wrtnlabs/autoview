...
export namespace AutoViewInputSubTypes {
    ...
    */
    export interface IPageIShoppingDepositCharge {
        /**
         * Page information.
         *
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const pageStart = (current - 1) * limit + 1;
  const pageEnd = Math.min(current * limit, records);
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4 text-sm text-gray-600">
        Showing{' '}
        <span className="font-semibold">{pageStart}</span>–<span className="font-semibold">{pageEnd}</span> of{' '}
        <span className="font-semibold">{records}</span> records (Page{' '}
        <span className="font-semibold">{current}</span> of <span className="font-semibold">{pages}</span>)
      </div>
      ...
      <ul className="divide-y divide-gray-200">
        {data.map((item) => {
          ...
          if (item.publish) {
            if (item.publish.paid_at) {
              status = 'Paid';
              StatusIcon = LucideReact.CheckCircle;
              statusColor = 'text-green-500';
              borderColor = 'border-green-500';
            } else if (item.publish.cancelled_at) {
              status = 'Cancelled';
          ...
          return (
            <li
              key={item.id}
              className={`py-4 flex items-center justify-between border-l-4 ${borderColor} pl-4`}
            >
              {/* Main info */}
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-gray-900 truncate">{formattedValue}</div>
                ...
                <div className="flex items-center gap-1 text-xs text-gray-500 mt-1">
                  ...
                  <span className="truncate">{item.customer.channel.name}</span>
                </div>
              </div>

              {/* Status badge */}
              <div className="flex items-center text-xs font-medium ml-4">
