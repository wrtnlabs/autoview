...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const pageInfo = `${pagination.current}/${pagination.pages} • ${pagination.records} items`;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <section className="space-y-6 p-4 bg-gray-50">
      <header className="flex justify-between items-center">
        <h1 className="text-xl font-semibold text-gray-800">Orders Overview</h1>
      ...
      <ul className="space-y-4">
        {orders.map((order) => {
          ...
          const savings = nominal - real;
          const channelName = order.customer.channel?.name ?? '—';

          return (
            <li
              key={order.id}
              className="bg-white rounded-lg shadow-md p-4 flex flex-col md:flex-row md:justify-between md:items-center"
            >
              <div className="flex-1">
                <h2 className="text-lg font-medium text-gray-800 truncate">{order.name}</h2>
              ...
              <div className="mt-3 md:mt-0 md:text-right flex flex-col space-y-1">
                <span className="text-sm text-gray-600">
                  Items: <span className="font-semibold text-gray-800">{itemCount}</span>
                </span>
                <div className="flex items-baseline justify-end space-x-2">
                  <span className="text-lg font-bold text-gray-900">{formatCurrency(real)}</span>
                  {savings > 0 && (
                    <>
                      <span className="text-sm text-gray-500 line-through">
                        {formatCurrency(nominal)}
                      </span>
                      <span className="text-sm text-green-600">Saved {formatCurrency(savings)}</span>
                    </>
                  )}
                </div>
              </div>
