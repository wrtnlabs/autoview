...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Format a timestamp string into a readable local datetime
  const formatDate = (iso: string): string =>
    new Date(iso).toLocaleString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });

  // Determine display status for an order, returning an icon and label
  const getStatus = (order: AutoViewInputSubTypes.IShoppingOrder) => {
    const pub = order.publish;
    if (!pub) {
      return {
    ...
    if (!pub.paid_at) {
      return {
        icon: <LucideReact.CreditCard size={16} className="text-gray-500" />,
        label: 'Pending Payment',
      };
    }
    // Paid – map detailed state to friendly label
    switch (pub.state) {
      case 'preparing':
        return {
      ...
      case 'delivering':
        return {
          icon: <LucideReact.Truck size={16} className="text-purple-500" />,
          label: 'Delivering',
        };
      case 'arrived':
        return {
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <p className="mb-4 text-sm text-gray-500">
        Showing {orders.length} of {records} orders
      </p>
      ...
      <ul className="divide-y divide-gray-200">
        {orders.map(order => {
          ...
          const itemCount = order.goods.length;
          const priceStr = order.price.real.toLocaleString();
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center space-x-2">
                  <h3 className="text-gray-900 font-medium truncate">{order.name}</h3>
                  {status.icon}
                  <span className="text-sm text-gray-500 truncate">{status.label}</span>
                </div>
                <div className="mt-1 flex flex-wrap text-sm text-gray-500 space-x-4">
                  <div className="flex items-center space-x-1">
                    <LucideReact.Calendar size={14} className="text-gray-400" />
                    <span>{created}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <LucideReact.Package size={14} className="text-gray-400" />
                    <span>{itemCount} items</span>
                  </div>
                </div>
              </div>
              <div className="mt-3 sm:mt-0 sm:ml-4 text-right">
                <span className="text-lg font-semibold text-gray-800">{priceStr}</span>
              </div>
            </li>
          );
        })}
