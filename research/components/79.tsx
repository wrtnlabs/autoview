...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const to = Math.min(current * limit, records);

  const getStatusInfo = (
    publish: AutoViewInputSubTypes.IShoppingOrderPublish | null,
  ) => {
    if (!publish) {
      return {
    ...
    switch (publish.state) {
      ...
      case "underway":
        return {
          label: publish.state.charAt(0).toUpperCase() + publish.state.slice(1),
          icon: <LucideReact.Clock className="text-amber-500" size={16} />,
        };
      case "shipping":
      case "delivering":
        return {
          label:
            publish.state === "delivering" ? "Out for Delivery" : "Shipping",
  ...
  return (
    <div className="space-y-4">
      {orders.map((order) => {
        const status = getStatusInfo(order.publish);
        const real = order.price.real;
        const nominal = order.price.nominal;
        return (
          <article
            key={order.id}
          ...
          >
            ...
            <div className="flex items-center gap-3 text-sm text-gray-500 mb-3">
              ...
              {order.customer.channel && (
                <div className="flex items-center gap-1">
                  ...
                  <span>{order.customer.channel.name}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between text-sm text-gray-600 mb-2">
              <div className="flex items-center gap-1">
                ...
                <span>
                  {order.goods.length} item
                  {order.goods.length !== 1 ? "s" : ""}
                </span>
              </div>
            ...
            {/* Footer: pagination range (hidden per order) */}
          </article>
        );
      })}
      ...
      <div className="flex items-center justify-between text-sm text-gray-500">
        <span>
          Showing {from}-{to} of {records} orders
        </span>
        <span>
