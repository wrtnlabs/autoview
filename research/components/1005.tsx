...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Transform and format event dates
  const formattedEvents = value.map((event): { event: AutoViewInputSubTypes.event; formattedDate: string } => {
  ...
  if (formattedEvents.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-gray-500">
        ...
        <span className="mt-4 text-lg">No events to display</span>
      </div>
    );
  }
  ...
  return (
    <div className="divide-y divide-gray-200 bg-white rounded-lg shadow-sm overflow-hidden">
      {formattedEvents.map(({ event, formattedDate }) => {
        const actor = event.actor;
        const actorName = actor.display_login || actor.login;
        ...
        const readableEventType = eventType.replace(/([A-Z])/g, ' $1').trim();

        return (
          <div key={event.id} className="flex items-start space-x-4 p-4">
            <img
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-2 text-sm">
                <span className="font-medium text-gray-900 truncate">{actorName}</span>
                <span className="text-gray-500">•</span>
                <span className="text-gray-700 truncate">{readableEventType}</span>
              ...
              <div className="mt-1 flex items-center text-sm text-gray-600 truncate">
                ...
                <span className="ml-1">{event.repo.name}</span>
              </div>
            </div>
          </div>
        );
