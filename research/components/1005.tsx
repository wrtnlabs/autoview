...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Data transformation: prepare events array and date formatter
  const events = value;
  ...
  return (
    <div className="space-y-4">
      {events.map((event) => {
        ...
        const pagesCount = event.payload.pages?.length;
        return (
          <div
            key={event.id}
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-gray-800">
                ...
                <span className="ml-1 font-medium">{event.repo.name}</span>
                {action && (
                  <span className="ml-1 text-indigo-600">({action})</span>
                )}
                {pagesCount ? (
