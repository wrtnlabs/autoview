...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const issueDate = new Date(value.created_at);
  const issueDateStr = issueDate.toLocaleString();

  const expiryDateStr = value.expired_at
    ? new Date(value.expired_at).toLocaleDateString()
    : 'No expiration';

  const discount = value.coupon.discount;
  const discountSummaryParts: string[] = [];
  if (discount.unit === 'amount') {
    discountSummaryParts.push(`$${discount.value} off`);
    if (discount.threshold !== null) {
      discountSummaryParts.push(`Min $${discount.threshold}`);
    }
    if (discount.multiplicative && discount.limit !== null) {
      discountSummaryParts.push(`Max $${discount.limit}`);
    }
  } else {
    discountSummaryParts.push(`${discount.value}% off`);
    if (discount.threshold !== null) {
  ...
  const discountSummary = discountSummaryParts.join(' • ');
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm max-w-md mx-auto">
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center text-sm text-gray-500 mt-1">
          ...
          <span className="ml-1">Issued: {issueDateStr}</span>
        </div>
        <div className="flex items-center text-sm text-gray-500 mt-1">
          <LucideReact.Calendar size={16} />
          <span className="ml-1">Expires: {expiryDateStr}</span>
        </div>
      </div>
      ...
      <div className="flex items-center text-sm text-gray-700 mb-4">
        ...
        <span className="ml-1">{discountSummary}</span>
      </div>
      ...
      <div className="border-t pt-4 text-sm text-gray-600 space-y-2">
        <div className="flex items-center">
          ...
          <span className="ml-1">Channel: {value.customer.channel.name}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.Server size={16} className="text-gray-500" />
          <span className="ml-1">IP: {value.customer.ip}</span>
        </div>
        {value.customer.citizen && (
          <div className="flex items-center">
            <LucideReact.User size={16} className="text-gray-500" />
            <span className="ml-1">{value.customer.citizen.name}</span>
          </div>
        )}
      </div>
    </div>
