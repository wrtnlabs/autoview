...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const customerName =
    ...
    "Guest";
  const channelName = customer.channel?.name ?? "Unknown Channel";
  ...
  const thresholdDisplay =
    discount.threshold != null ? `Min: $${discount.threshold}` : null;

  const { volume, volume_per_citizen } = value.coupon.inventory;
  const volumeDisplay = volume != null ? volume : "Unlimited";
  const perCitizenDisplay =
    volume_per_citizen != null ? volume_per_citizen : "Unlimited";

  const accessLevel =
    value.coupon.restriction.access === "public" ? "Public" : "Private";
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
          ...
          <div className="flex items-center space-x-1">
            ...
            <span>Remaining: {volumeDisplay}</span>
          </div>
          <div className="flex items-center space-x-1">
            <LucideReact.Users size={16} className="text-gray-500" />
            <span>Per Customer: {perCitizenDisplay}</span>
          </div>
        </div>
      </div>
