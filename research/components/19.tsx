...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const start = Math.min((current - 1) * limit + 1, records);
    ...
    <section className="w-full max-w-screen-lg mx-auto p-4">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="text-sm text-gray-600">
          Showing {start}–{end} of {records} orders (Page {current} of {pages})
        </div>
      </div>
      ...
      <div className="space-y-4">
        {value.data.length === 0 ? (
          <div className="flex items-center justify-center text-gray-500 py-8">
            ...
            <span className="ml-2">No orders found</span>
          </div>
        ) : (
          value.data.map((order) => {
            ...
            const statusLabel = order.publish
              ...
              >
                ...
                <div className="mt-4 sm:mt-0 flex items-center space-x-6">
                  <div className="flex items-center text-sm text-gray-700 space-x-1">
                    ...
                    <span>{order.goods.length} items</span>
                  </div>
                  <div className="flex items-center text-sm text-gray-700 space-x-1">
                    ...
                    <div className="flex items-baseline">
                      ...
                      <span
                        className={`ml-${hasDiscount ? "1" : "0"} font-semibold text-gray-900`}
                      >
                        ${real.toFixed(2)}
                      </span>
                    </div>
