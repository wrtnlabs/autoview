...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Data transformation and formatting utilities
  ...
  const statusMap: Record<string, { icon: JSX.Element; label: string }> = {
    ...
    underway: {
      icon: <LucideReact.Truck className="text-blue-500" size={16} />,
      label: "Underway",
    },
    preparing: {
      icon: <LucideReact.Cog className="text-amber-500" size={16} />,
      label: "Preparing",
    },
    manufacturing: {
      icon: <LucideReact.HardDrive className="text-purple-500" size={16} />,
      label: "Manufacturing",
    },
    shipping: {
      icon: <LucideReact.Send className="text-blue-500" size={16} />,
      label: "Shipping",
    },
    delivering: {
      icon: <LucideReact.Package className="text-green-500" size={16} />,
      label: "Delivering",
    },
    arrived: {
  ...
  return (
    <div className="w-full p-4 bg-white rounded-lg shadow-sm sm:p-6">
      {/* Pagination Summary */}
      ...
      <ul className="space-y-4">
        {orders.map((order) => {
          ...
          const customerName =
            ...
            "Guest";
          const stateKey = order.publish?.state ?? "none";
          const { icon, label } = statusMap[stateKey] || statusMap.none;
          const { nominal, real } = order.price;

          return (
            <li key={order.id} className="p-4 bg-gray-50 rounded-lg">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center text-sm text-gray-600 mt-2 gap-4">
                    ...
                    <div className="flex items-center gap-1">
                      <LucideReact.ShoppingCart size={16} />
                      <span>{order.goods.length}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      {icon}
                      <span>{label}</span>
