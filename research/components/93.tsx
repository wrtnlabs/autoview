...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const rawQuestionBody = latestSnapshot.body ?? "";
  const formattedQuestionBody =
    rawQuestionBody.length > 200
      ? `${rawQuestionBody.slice(0, 200)}…`
      : rawQuestionBody;
  const hasAttachments = latestSnapshot.files.length > 0;

  const answer = value.answer;
  const hasAnswer = Boolean(answer);
  let latestAnswerSnapshot: AutoViewInputSubTypes.IBbsArticle.ISnapshot | null = null;
  let answerDate = "";
  let formattedAnswerBody = "";
  if (hasAnswer && answer) {
    latestAnswerSnapshot =
      answer.snapshots[answer.snapshots.length - 1] ?? null;
    answerDate = new Date(answer.created_at).toLocaleString();
    const rawAnswerBody = latestAnswerSnapshot?.body ?? "";
    formattedAnswerBody =
      rawAnswerBody.length > 200
        ? `${rawAnswerBody.slice(0, 200)}…`
        : rawAnswerBody;
  }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-medium text-gray-700">
            {value.secret ? "Secret Question" : "Public Question"}
          </span>
        </div>
        <div className="flex items-center space-x-1 text-sm text-gray-500">
          <LucideReact.Calendar size={16} aria-label="Question Date" />
      ...
      <div className="flex flex-wrap items-center text-sm text-gray-500 gap-x-6 gap-y-2 mb-4">
        ...
        {hasAttachments && (
          <div className="flex items-center space-x-1">
            <LucideReact.FileText size={16} aria-label="Attachments" />
            <span>
      ...
      {hasAnswer && latestAnswerSnapshot && (
        <div className="border-t pt-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-1 text-sm text-gray-500">
              <LucideReact.Calendar size={16} aria-label="Answer Date" />
