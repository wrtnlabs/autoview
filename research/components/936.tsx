...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const ownerName = value.owner.login;
  ...
  const lastUsedAt = React.useMemo(
    () => new Date(value.last_used_at).toLocaleString(),
    [value.last_used_at],
  );
  const retentionDate = React.useMemo(
    () =>
  ...
  type StateParams = { icon: JSX.Element; colorClass: string };
  const getStateParams = (state: AutoViewInput["state"]): StateParams => {
    switch (state) {
      case "Available":
        return {
          icon: (
      ...
      default:
        return {
          icon: <LucideReact.Clock className="text-gray-400" size={16} />,
          colorClass: "text-gray-600",
        };
    }
  };
  const { icon: StateIcon, colorClass: stateColorClass } = React.useMemo(
    () => getStateParams(value.state),
    [value.state],
  );
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-xl mx-auto">
      ...
      <div className="flex items-center mb-4">
        ...
        <div className="ml-3">
          ...
          <p className="text-sm text-gray-500 truncate">Owner: {ownerName}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
        ...
        <div className="flex items-center text-gray-600 truncate">
          <LucideReact.Code2 className="text-gray-400" size={16} />
          <span className="ml-2 truncate">{value.repository.full_name}</span>
        </div>

        {/* State */}
        <div className={`flex items-center ${stateColorClass}`}>
          {StateIcon}
          <span className="ml-2 truncate">{value.state}</span>
        </div>

        {/* Location */}
        <div className="flex items-center text-gray-600 truncate">
          <LucideReact.MapPin className="text-gray-400" size={16} />
          <span className="ml-2">{value.location}</span>
        </div>

        {/* Last Used */}
        <div className="flex items-center text-gray-600 truncate">
          <LucideReact.Clock className="text-gray-400" size={16} />
          <span className="ml-2">Last used: {lastUsedAt}</span>
        </div>
        ...
        {/* Machine */}
        <div className="flex items-center text-gray-600 truncate">
          <LucideReact.Cpu className="text-gray-400" size={16} />
          <span className="ml-2">
            {value.machine?.display_name ?? "No machine assigned"}
          </span>
        </div>
        ...
        <div className="flex items-center text-gray-600 truncate">
          ...
          <span className="ml-2">
            Retention expires:{" "}
            {retentionDate ?? <span className="italic">N/A</span>}
          </span>
        </div>
      </div>
      ...
      {value.pending_operation && (
        <div className="mt-4 flex items-center text-red-600 text-sm">
          <LucideReact.AlertTriangle size={16} className="mr-2" />
          <span className="truncate">
            {value.pending_operation_disabled_reason ||
              "Pending operation underway"}
          </span>
        </div>
      )}
    </div>
