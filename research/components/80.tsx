...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalItems = value.goods.reduce((sum, g) => sum + g.volume, 0);
  const formattedTotalPrice = value.price.real.toLocaleString(undefined, {
    style: 'currency',
    currency: 'USD',
  });
  const publish = value.publish;
  const paymentIcon = publish?.paid_at ? (
    <LucideReact.CheckCircle className="text-green-500" size={16} />
  ) : (
    <LucideReact.Clock className="text-amber-500" size={16} />
  );
  const paymentLabel = publish?.paid_at ? 'Paid' : 'Pending Payment';

  const delState = publish?.state;
  let deliveryIcon: React.ReactNode;
  let deliveryLabel: string;
  switch (delState) {
    case 'preparing':
      deliveryIcon = <LucideReact.Package className="text-blue-500" size={16} />;
      deliveryLabel = 'Preparing';
      break;
    case 'underway':
      deliveryIcon = <LucideReact.Truck className="text-blue-500" size={16} />;
      deliveryLabel = 'Underway';
      break;
    case 'manufacturing':
      deliveryIcon = <LucideReact.Settings className="text-blue-500" size={16} />;
      deliveryLabel = 'Manufacturing';
      break;
    case 'shipping':
      deliveryIcon = <LucideReact.Truck className="text-blue-500" size={16} />;
      ...
      break;
    case 'delivering':
      deliveryIcon = <LucideReact.MapPin className="text-blue-500" size={16} />;
      deliveryLabel = 'Delivering';
      break;
    case 'arrived':
      deliveryIcon = <LucideReact.CheckCircle className="text-green-500" size={16} />;
      deliveryLabel = 'Arrived';
      break;
    default:
      deliveryIcon = <LucideReact.Clock className="text-gray-500" size={16} />;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      <div className="flex flex-col sm:flex-row sm:justify-between">
        ...
        <div className="mt-4 sm:mt-0 flex space-x-4">
          <div className="flex items-center text-sm text-gray-600">
            {paymentIcon}
            <span className="ml-1">{paymentLabel}</span>
          </div>
          <div className="flex items-center text-sm text-gray-600">
            {deliveryIcon}
      ...
      <div className="mt-4 space-y-4">
        {value.goods.map((g) => {
          const thumbnail =
            g.commodity.sale.content.thumbnails[0]?.url ||
            'https://placehold.co/64x64/e2e8f0/1e293b?text=No+Image';
          const title = g.commodity.sale.content.title;
          const itemPrice = g.price.real;
          const formattedItemPrice = itemPrice.toLocaleString(undefined, {
            style: 'currency',
            currency: 'USD',
          });
            ...
            <div key={g.id} className="flex items-center space-x-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 flex items-center text-sm text-gray-600 space-x-2">
                  <span>Qty: {g.volume}</span>
                  <span>Price: {formattedItemPrice}</span>
                </div>
              </div>
            </div>
          );
