...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalItems = value.goods.length;
  const { nominal, real } = value.price;
  const nominalFormatted = nominal.toLocaleString();
  const realFormatted = real.toLocaleString();
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500">Ordered on {createdDate}</p>
      </div>

      {/* Summary Grid */}
      <div className="grid grid-cols-2 gap-4 mb-4 text-sm text-gray-700">
        <div>
          <p className="font-medium">Customer Channel</p>
          <p>{customerChannel}</p>
        </div>
        <div>
        ...
        <div>
          <p className="font-medium">Total (Nominal)</p>
          <p>{nominalFormatted}</p>
        </div>
        <div>
      ...
      <ul className="space-y-4">
        {value.goods.map((good) => {
          ...
          const volume = good.volume;
          const priceReal = good.price.real.toLocaleString();
          const status = stateLabels[good.state ?? "none"] || "N/A";

          return (
            <li key={good.id} className="flex items-center p-3 bg-gray-50 rounded-md">
              {thumbUrl && (
                <img
                  src={thumbUrl}
                  alt={title}
