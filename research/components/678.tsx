...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const cfg = statusMap[value.status];
  ...
  const succeededCount = scanned.filter(
    (r) => r.analysis_status === "succeeded",
  ).length;
  const failedCount = scanned.filter(
    (r) => r.analysis_status === "failed",
  ).length;
  const pendingCount = scanned.filter(
    (r) => r.analysis_status === "pending",
  ).length;

  const skippedGroups = value.skipped_repositories
    ? (Object.values(value.skipped_repositories) as Array<{
        repository_count?: number;
      }>)
    : [];
  const skippedTotal = skippedGroups.reduce(
    (sum, grp) => sum + (grp.repository_count ?? 0),
    0,
  );
    ...
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4 max-w-md mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center text-sm">
            {cfg.icon}
            <span className={`ml-1 ${cfg.textColor}`}>{cfg.label}</span>
          </div>
        </div>
        <span className="px-2 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded">
          {value.query_language.toUpperCase()}
        </span>
      </div>
      ...
      <div className="flex items-center space-x-4">
        <div className="flex items-center">
          ...
          />
          <span className="ml-2 text-sm font-medium">{value.actor.login}</span>
        </div>
      ...
      {/* Summary of scanned repositories */}
      <div className="text-sm text-gray-700">
        <span className="font-medium">Scanned Repositories:</span>{" "}
        {scannedCount} total —
        <span className="ml-1 text-green-600">{succeededCount} succeeded</span>,
        <span className="ml-1 text-red-600">{failedCount} failed</span>,
      ...
      {skippedTotal > 0 && (
        <div className="text-sm text-gray-700">
          <span className="font-medium">Skipped:</span> {skippedTotal}
        </div>
      )}

      {/* Failure reason */}
      {value.status === "failed" && value.failure_reason && (
        <div className="flex items-center text-sm text-red-600">
          <LucideReact.AlertTriangle size={16} className="inline-block" />
          <span className="ml-1">Reason: {value.failure_reason}</span>
        </div>
      )}

      {/* Query pack URL */}
      <div className="flex items-center text-sm text-blue-600 break-all">
        <LucideReact.Link size={16} className="text-gray-400" />
        <span className="ml-1 truncate">{value.query_pack_url}</span>
      </div>
    </div>
  );
}
