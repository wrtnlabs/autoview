...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const enforcementIcon = (() => {
    switch (enforcement) {
      case "active":
        return <LucideReact.CheckCircle className="text-green-500" size={18} />;
      case "evaluate":
        return <LucideReact.Clock className="text-amber-500" size={18} />;
      default:
        return <LucideReact.XCircle className="text-gray-400" size={18} />;
    }
  })();
  const target = value.target ?? "repository";
  const sourceType = value.source_type ?? "Repository";
  const ruleCount = value.rules?.length ?? 0;
  const bypassCount = value.bypass_actors?.length ?? 0;

  const refIncludes = value.conditions?.ref_name?.include?.length ?? 0;
  const refExcludes = value.conditions?.ref_name?.exclude?.length ?? 0;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md w-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-1 text-sm font-medium">
          {enforcementIcon}
          <span className="uppercase">{enforcement}</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-gray-600">
        <div className="flex items-center">
          <LucideReact.Tag size={16} className="text-gray-500" />
          <span className="ml-1 capitalize">{target}</span>
        </div>
        <div className="flex items-center">
        ...
        <div className="flex items-center">
          ...
          <span className="ml-1">{ruleCount} Rules</span>
        </div>
        {bypassCount > 0 && (
          <div className="col-span-2 flex items-center">
            <LucideReact.UserCheck size={16} className="text-gray-500" />
            <span className="ml-1">{bypassCount} Bypass Actors</span>
          </div>
        )}
        {(refIncludes > 0 || refExcludes > 0) && (
          <div className="col-span-2">
            <div className="text-sm font-medium text-gray-700 mb-1">
              Conditions
            </div>
            <ul className="list-disc list-inside space-y-0.5 text-gray-600 text-sm">
              {refIncludes > 0 && <li>Ref include: {refIncludes}</li>}
              {refExcludes > 0 && <li>Ref exclude: {refExcludes}</li>}
            </ul>
          </div>
        )}
      </div>
      <div className="mt-4 flex text-xs text-gray-500 space-x-4">
        {createdAt && (
          <div className="flex items-center">
