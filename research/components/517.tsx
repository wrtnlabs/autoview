...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const ruleTypes = value.rules?.map((rule) => rule.type) ?? [];
  const displayedRules = ruleTypes.slice(0, 5);
  const moreRulesCount = ruleTypes.length - displayedRules.length;

  const cond = value.conditions;
  const condItems: string[] = [];
  if (cond && "ref_name" in cond && cond.ref_name) {
    const inc = cond.ref_name.include?.join(", ");
    const exc = cond.ref_name.exclude?.join(", ");
    if (inc) condItems.push(`Refs include: ${inc}`);
    if (exc) condItems.push(`Refs exclude: ${exc}`);
  }
  if (cond && "repository_name" in cond) {
    const inc = cond.repository_name.include?.join(", ");
    const exc = cond.repository_name.exclude?.join(", ");
    if (inc) condItems.push(`Repos include: ${inc}`);
    if (exc) condItems.push(`Repos exclude: ${exc}`);
  }

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-md w-full bg-white p-4 rounded-lg shadow-md text-gray-800">
      {/* Header */}
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-semibold truncate">{value.name}</h2>
        <div className="flex items-center gap-1">
          {enforcementIcon}
          <span className="text-sm capitalize">{value.enforcement}</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap text-sm text-gray-600 gap-4 mb-3">
        <div className="flex items-center gap-1">
          ...
          <span>{value.target ?? "all targets"}</span>
        </div>
        <div className="flex items-center gap-1">
      ...
      {ruleTypes.length > 0 && (
        <div className="mb-3">
          <h3 className="text-sm font-medium text-gray-700 mb-1">Rules:</h3>
          <div className="flex flex-wrap gap-2">
            {displayedRules.map((type) => (
              <span
                key={type}
