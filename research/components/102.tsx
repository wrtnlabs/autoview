...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const snapshots = Array.isArray(value.snapshots) && value.snapshots.length > 0
    ? value.snapshots
    : [];
  ...
  // Pick the latest snapshot by created_at
  const latestSnapshot = snapshots.reduce((prev, curr) =>
    new Date(curr.created_at) > new Date(prev.created_at) ? curr : prev
  , snapshots[0]);

  // Format dates
  const reviewDate = new Date(value.created_at).toLocaleDateString(undefined, {
    year: 'numeric', month: 'long', day: 'numeric'
  });
  const snapshotDate = new Date(latestSnapshot.created_at).toLocaleDateString(undefined, {
    year: 'numeric', month: 'short', day: 'numeric'
  });

  // Stars for rating
  const score = Math.max(0, Math.min(5, Math.floor(latestSnapshot.score)));
  const stars = Array.from({ length: 5 }, (_, i) =>
    <span key={i} className={ i < score ? 'text-yellow-500' : 'text-gray-300' }>★</span>
  );

  // Body preview
  const bodyPreview = latestSnapshot.body;

  // Attachment count
  const fileCount = Array.isArray(latestSnapshot.files) ? latestSnapshot.files.length : 0;

  // Customer channel name
  const channelName = value.customer?.channel?.name ?? 'Unknown channel';

  // Read status badge
  const statusBadge = value.read_by_seller
    ? <span className="px-2 py-1 text-xs font-medium bg-green-100 text-green-800 rounded-full">Read</span>
    : <span className="px-2 py-1 text-xs font-medium bg-red-100 text-red-800 rounded-full">Unread</span>;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="flex justify-between items-center">
        {statusBadge}
        {value.answer && typeof value.answer === 'string' && (
          <div className="ml-4 flex-1">
            <h3 className="text-sm font-medium text-gray-800">Seller Answer:</h3>
