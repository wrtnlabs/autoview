...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  let latestAnswerSnapshot:
    | AutoViewInputSubTypes.IShoppingSaleInquiryAnswer["snapshots"][0]
    | null = null;
  ...
  if (value.answer) {
    ...
    latestAnswerSnapshot =
      sortedAnswerSnapshots[sortedAnswerSnapshots.length - 1];
    formattedAnswerDate = new Date(
      latestAnswerSnapshot.created_at,
    ).toLocaleDateString("default", {
      year: "numeric",
      month: "long",
      day: "numeric",
  ...
  // Helper to render body based on format
  const renderBody = (text: string, format: "html" | "md" | "txt") =>
    format === "html" ? (
      <div
        className="mt-1 text-gray-700 text-sm line-clamp-3"
        dangerouslySetInnerHTML={{ __html: text }}
      />
    ) : (
      <p className="mt-1 text-gray-700 text-sm line-clamp-3">{text}</p>
    );

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border border-gray-200">
      {/* Header: score and read status */}
        ...
        <div className="flex items-center space-x-1 text-sm">
          {value.read_by_seller ? (
            <LucideReact.CheckCircle
              ...
              size={16}
              aria-label="Viewed by seller"
            />
          ) : (
            <LucideReact.Clock
              className="text-amber-500"
              size={16}
              aria-label="Not viewed by seller"
            />
          )}
          <span>{value.read_by_seller ? "Viewed" : "Not viewed"}</span>
        </div>
      </div>
      ...
      {renderBody(latestSnapshot.body, latestSnapshot.format)}

      {/* Meta: review date and channel */}
      <div className="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between text-gray-500 text-xs">
        <div className="flex items-center space-x-1">
          <LucideReact.Calendar
            ...
            aria-hidden="true"
          />
          <span>Reviewed on {formattedReviewDate}</span>
        </div>
      ...
      {/* Seller's answer section */}
      {value.answer && latestAnswerSnapshot && (
        <div className="mt-4 p-4 bg-gray-50 rounded-md border border-gray-200">
          <h3 className="flex items-center text-sm font-medium text-gray-800 space-x-2">
            <LucideReact.MessageCircle size={16} aria-hidden="true" />
            <span>Seller’s Answer</span>
          </h3>
          ...
          {renderBody(latestAnswerSnapshot.body, latestAnswerSnapshot.format)}
        </div>
      )}
    </div>
  );
