...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const discountValueText = isAmount
  ...
  const limitText =
    discount.limit != null
      ? `Max: $${discount.limit.toLocaleString()}`
      : null;

  const openedDate = value.opened_at
    ? new Date(value.opened_at).toLocaleDateString()
    : "Any time";
  const closedDate = value.closed_at
  ...
  const criteriaTypes = Array.from(
    new Set(value.criterias.map((c) => c.type))
  );
  const criteriaLabels = criteriaTypes.map((type) => {
    switch (type) {
      case "section":
        return "Section";
      case "seller":
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4 max-w-md mx-auto">
      ...
      <div className="flex items-center space-x-2">
        <LucideReact.Tag size={20} className="text-indigo-500 flex-shrink-0" />
        <h2
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        {thresholdText && (
          <div className="flex items-center text-gray-500 text-sm">
            ...
            />
            <span className="ml-1">{thresholdText}</span>
          </div>
        )}
        {limitText && (
          <div className="flex items-center text-gray-500 text-sm">
            <LucideReact.ArrowDownCircle
              ...
              className="text-gray-400 flex-shrink-0"
            />
            <span className="ml-1">{limitText}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-4 text-sm">
        ...
        <div className="flex items-center text-gray-500">
          ...
          <span className="ml-1">
            {value.restriction.exclusive ? "Exclusive" : "Combinable"}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm text-gray-500">
        <div className="flex items-center">
          <LucideReact.Package size={16} className="flex-shrink-0" />
          <span className="ml-1">Stock: {inventoryVolume}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.User size={16} className="flex-shrink-0" />
          <span className="ml-1">Per User: {inventoryPerCitizen}</span>
        </div>
      </div>
