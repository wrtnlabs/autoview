...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const discountText =
    discount.unit === "amount"
      ? `$${discount.value.toLocaleString()}${discount.multiplicative ? " each" : ""}`
      : `${discount.value}%`;
  const thresholdText =
    discount.threshold !== null
      ? `Min. $${discount.threshold.toLocaleString()}`
      : null;
  const limitText =
    discount.limit !== null ? `Max. $${discount.limit.toLocaleString()}` : null;

  const openedAt = value.opened_at
    ? new Date(value.opened_at).toLocaleDateString(undefined, {
        year: "numeric",
        month: "short",
        day: "numeric",
      })
  ...
  const invVolume =
    value.inventory.volume !== null ? value.inventory.volume : "∞";
  const invPerCitizen =
    value.inventory.volume_per_citizen !== null
      ? value.inventory.volume_per_citizen
      : "∞";

  // summarize criteria by type and direction
  type Agg = Record<string, number>;
  const criteriaCounts = (value.criterias || []).reduce((acc: Agg, c) => {
    const key = `${c.type}-${c.direction}`;
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});
  const criteriaLines = Object.entries(criteriaCounts).map(([key, cnt]) => {
    const [type, dir] = key.split("-");
    const typeLabel = type.charAt(0).toUpperCase() + type.slice(1);
    const dirLabel = dir === "include" ? "Included" : "Excluded";
    return `${dirLabel} ${typeLabel}: ${cnt}`;
  });
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4 max-w-md mx-auto">
      ...
      <div className="flex items-center gap-2 text-gray-700">
        {discount.unit === "amount" ? (
          <LucideReact.DollarSign size={16} className="text-green-600" />
        ) : (
          <LucideReact.Percent size={16} className="text-blue-600" />
        )}
        <span className="font-medium">{discountText}</span>
        {thresholdText && (
          <span className="text-sm text-gray-500">({thresholdText})</span>
        )}
        {limitText && (
          <span className="text-sm text-gray-500">({limitText})</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-700">
        ...
        <span>
          Inventory: {invVolume} total / {invPerCitizen} per person
        </span>
      </div>
