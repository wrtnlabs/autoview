...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Derive computed values and formatting helpers
  ...
  // Enforcement status mapping to icon, label, and badge style
  const enforcementConfig = {
    ...
    disabled: {
      icon: <LucideReact.XCircle size={20} className="text-red-500" />,
      label: "Disabled",
  ...
  const {
    ...
    badgeClasses,
  } = enforcementConfig[value.enforcement] || enforcementConfig.disabled;

  // Summarize conditions succinctly
  let conditionsSummary = "";
  if (value.conditions) {
    const parts: string[] = [];
    const c = value.conditions as any;
    if (c.ref_name) {
      const inc = c.ref_name.include?.length ?? 0;
      const exc = c.ref_name.exclude?.length ?? 0;
      if (inc) parts.push(`${inc} ref${inc > 1 ? "s" : ""} include`);
      if (exc) parts.push(`${exc} ref${exc > 1 ? "s" : ""} exclude`);
    }
    if ("repository_name" in c && c.repository_name) {
      const inc = c.repository_name.include?.length ?? 0;
      if (inc) parts.push(`${inc} repo name${inc > 1 ? "s" : ""}`);
    }
    if ("repository_id" in c && c.repository_id) {
      const count = c.repository_id.repository_ids?.length ?? 0;
      if (count) parts.push(`${count} repo ID${count > 1 ? "s" : ""}`);
    }
    if ("repository_property" in c && c.repository_property) {
      const inc = c.repository_property.include?.length ?? 0;
      if (inc) parts.push(`${inc} prop spec${inc > 1 ? "s" : ""}`);
    }
    conditionsSummary = parts.join(", ");
  }

  // Icons for the 'target' property
  const targetIcons: Record<string, JSX.Element> = {
    branch: <LucideReact.GitBranch size={16} className="text-gray-400" />,
    tag: <LucideReact.Tag size={16} className="text-gray-400" />,
    push: <LucideReact.ArrowUp size={16} className="text-gray-400" />,
    repository: <LucideReact.Package size={16} className="text-gray-400" />,
  };
  const targetIcon = value.target ? targetIcons[value.target] : null;
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm max-w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          {enforcementIcon}
          <span
            className={`text-xs font-medium px-2 py-0.5 rounded ${badgeClasses}`}
          >
            {enforcementLabel}
          </span>
        </div>
      ...
      <div className="mt-2 flex flex-wrap text-sm text-gray-600 gap-x-4 gap-y-1">
        {value.target && (
          <div className="flex items-center space-x-1">
            {targetIcon}
            <span>{capitalize(value.target)}</span>
          </div>
        )}
        {value.source && (
          <div className="flex items-center space-x-1">
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-gray-700">
        ...
        <div className="flex items-center space-x-2">
          <LucideReact.Users size={16} className="text-gray-400" />
          <span>Bypass Actors: {bypassCount}</span>
        </div>
        <div className="flex items-center space-x-2">
          <LucideReact.Filter size={16} className="text-gray-400" />
          <span>Conditions: {conditionsSummary || "None"}</span>
        </div>
        <div className="flex items-center space-x-2">
