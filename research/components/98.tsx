...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Data derivations
  // Determine writer display name from various writer subtypes
  function getWriterName(
    writer: AutoViewInputSubTypes.IShoppingSaleInquiryComment["writer"],
  ): string {
    // Seller has explicit type
    if ("type" in writer && writer.type === "seller") {
      return writer.member.nickname;
    }
    // Customer has explicit type
    if ("type" in writer && writer.type === "customer") {
      return writer.member?.nickname ?? writer.citizen?.name ?? "Customer";
    }
    // Administrator invert (no type field)
    return writer.member.nickname;
  }

  const writerName = getWriterName(value.writer);
  const isReply = value.parent_id !== null;
  // Use the latest snapshot for display
  const latestSnapshot = value.snapshots[value.snapshots.length - 1];
  const { body, format, created_at, files } = latestSnapshot;
  ...
  const displayBody = shouldTruncate ? body.slice(0, 300) + "…" : body;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md space-y-3">
      {/* Header: Writer and Date */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <LucideReact.User className="text-gray-600" size={20} />
          <span className="font-semibold text-gray-800">{writerName}</span>
        </div>
