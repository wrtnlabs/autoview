...
export namespace AutoViewInputSubTypes {
    export type IShoppingCartDiscountable = {
        deposit: number;
        mileage: number;
        combinations: AutoViewInputSubTypes.IShoppingCartDiscountable.ICombination[];
    };
    export namespace IShoppingCartDiscountable {
        export type ICombination = {
            ...
            entries: AutoViewInputSubTypes.IShoppingCartDiscountable.IEntry[];
            amount: number;
        };
        export type IEntry = {
...
export type AutoViewInput = AutoViewInputSubTypes.IShoppingCartDiscountable;
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalCombinations = value.combinations.length;
  const totalAmount = value.combinations.reduce((sum, combo) => sum + combo.amount, 0);
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gray-50 p-3 rounded-lg">
          ...
          <div className="mt-1 text-lg font-semibold text-gray-800">
            {formatNumber(value.deposit)}
          </div>
        </div>
        <div className="bg-gray-50 p-3 rounded-lg">
          <div className="text-sm text-gray-500">Mileage</div>
          <div className="mt-1 text-lg font-semibold text-gray-800">
            {formatNumber(value.mileage)}
          </div>
        </div>
      </div>
      ...
      <div className="text-gray-700 text-sm">
        <span className="font-medium">Combinations:</span> {totalCombinations}
        <span className="ml-2">
          (Total Amount: <span className="font-medium">{formatNumber(totalAmount)}</span>)
        </span>
      </div>

      {/* Detailed List of Combinations */}
      <ul className="space-y-4">
        {value.combinations.map((combo, idx) => (
          <li
            key={idx}
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm text-gray-600">
                Amount: {formatNumber(combo.amount)}
              </span>
            </div>
            <div className="flex space-x-4 text-sm text-gray-600">
              <div>Entries: {combo.entries.length}</div>
              <div>Coupons: {combo.coupons.length}</div>
              <div>Tickets: {combo.tickets.length}</div>
            </div>

            {/* Show up to 2 entries with a "more" indicator */}
            {combo.entries.length > 0 && (
              <div className="pt-2 border-t border-gray-100">
                <ul className="space-y-1">
                  {combo.entries.slice(0, 2).map((entry, eIdx) => (
                    <li
                      key={eIdx}
                      className="flex justify-between text-sm text-gray-700"
                    >
                      <span className="truncate">{truncateId(entry.commodity_id)}</span>
                      <div className="flex items-center space-x-2">
                        <span>{formatNumber(entry.amount)}</span>
                        {entry.pseudo && (
                          <span className="px-1 text-xs text-blue-600 bg-blue-100 rounded">
                            Pseudo
                          </span>
                        )}
                      </div>
                  ...
                  {combo.entries.length > 2 && (
                    <li className="text-sm text-gray-500">
                      +{combo.entries.length - 2} more entries
                    </li>
                  )}
                </ul>
              </div>
