...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const getCustomerName = (
    ...
    if (customer.member?.nickname) return customer.member.nickname;
    if (customer.citizen?.name) return customer.citizen.name;
    if (customer.external_user?.nickname)
      return customer.external_user.nickname;
    if (customer.channel?.name) return customer.channel.name;
    return "Customer";
  };
          ...
          return (
            <li key={review.id} className="p-4 bg-gray-50 rounded-lg shadow-sm">
              ...
              <div className="flex flex-wrap items-center text-sm text-gray-500 mt-2 gap-4">
                ...
                <div className="flex items-center">
                  {review.answer ? (
                    <LucideReact.Reply className="text-green-500" size={16} />
                  ) : (
                    <LucideReact.Reply className="text-gray-300" size={16} />
                  )}
                  <span className="ml-1">
                    {review.answer ? "Answered" : "No Answer"}
                  </span>
                </div>
              </div>
              ...
              {review.files.length > 0 && (
                <div className="flex items-center text-sm text-gray-500 mt-2">
                  ...
                  <span className="ml-1">
                    {review.files.length} attachment
