...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const writerType = writer.type; // "administrator" | "customer" | "seller"
  const writerName =
    writer.citizen?.name ||
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-full">
      {/* Header: Writer info and timestamp */}
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          {writerType === "administrator" && (
            <LucideReact.Shield className="text-gray-600" size={20} />
          )}
          {writerType === "seller" && (
            <LucideReact.ShoppingCart className="text-gray-600" size={20} />
          )}
          {writerType === "customer" && (
          ...
          <span
            className={`text-xs uppercase font-medium px-2 py-0.5 rounded-full ${
              writerType === "administrator"
      ...
      {value.parent_id && (
        <div className="flex items-center text-gray-500 text-sm mt-2">
          <LucideReact.MessageSquare size={14} className="mr-1" />
          <span>In reply</span>
        </div>
      )}

      {/* Comment content */}
      <p className="mt-2 text-gray-700 text-sm line-clamp-3 whitespace-pre-line">
        {contentBody}
      </p>
            ...
            {attachments.map((file) => {
              ...
              return (
                <li key={file.url}>
                  ...
                  >
                    <LucideReact.FileText size={16} className="mr-2" />
                    <span>{fileName}</span>
                  </a>
                </li>
              );
