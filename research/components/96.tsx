...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Determine writer type and name
  const writer = value.writer as any;
  const writerType = typeof writer.type === "string" ? writer.type : "unknown";
  // Extract a human-readable name if available
  let writerName: string;
  if (writer.citizen && typeof writer.citizen.name === "string") {
    writerName = writer.citizen.name;
  } else if (writer.member && writer.member.nickname) {
    writerName = writer.member.nickname;
  } else {
    writerName = writerType.charAt(0).toUpperCase() + writerType.slice(1);
  }
  // Choose an icon based on writer type
  const writerIcon = (() => {
    switch (writerType) {
      case "administrator":
        return (
  ...
  // Find the latest snapshot by creation date
  const latestSnapshot = [...value.snapshots].sort(
    (a, b) =>
