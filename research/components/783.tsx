...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const formattedDate = new Date(value.created_at).toLocaleDateString("en-US", {
    year: "numeric",
    month: "short",
    day: "numeric",
  });
  const author = value.user?.login ?? "Unknown";
  const bodyText = value.body?.trim() ?? "";
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md flex flex-col space-y-4">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center text-sm font-medium text-gray-600 ml-4">
          {value.state === "open" ? (
            <LucideReact.Circle
              size={14}
      ...
      {/* Metadata: Number, Author, Date, Comments */}
      ...
      {/* Body Preview */}
      {bodyText && (
        <p className="text-gray-700 text-sm line-clamp-3">{bodyText}</p>
      )}
