...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { customer, created_at, value: amount, publish } = value;
  // Derive a friendly customer name
  let customerName = "Guest";
  if (customer.member?.nickname) customerName = customer.member.nickname;
  else if (customer.citizen?.name) customerName = customer.citizen.name;
  else if (customer.external_user?.nickname)
    customerName = customer.external_user.nickname;
  // Determine the payment status
  let status: "pending" | "paid" | "cancelled" = "pending";
  if (publish) {
    if (publish.cancelled_at) status = "cancelled";
    else if (publish.paid_at) status = "paid";
  }
  ...
  const formattedPaidAt = publish?.paid_at
    ? new Date(publish.paid_at).toLocaleString()
    : null;
  const formattedCancelledAt = publish?.cancelled_at
    ? new Date(publish.cancelled_at).toLocaleString()
    : null;
  // Extract channel and IP
  const channelName = customer.channel.name;
  const customerIp = customer.ip;
    ...
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <LucideReact.CreditCard className="text-gray-600" size={20} />
      ...
      <div className="mb-4">
        ...
        {formattedPaidAt && (
          <div className="flex items-center text-gray-500 text-sm mt-1">
            ...
            />
            <span>Paid at: {formattedPaidAt}</span>
          </div>
        )}
        {formattedCancelledAt && (
          <div className="flex items-center text-gray-500 text-sm mt-1">
            <LucideReact.XCircle size={16} className="mr-1 text-red-500" />
            <span>Cancelled at: {formattedCancelledAt}</span>
          </div>
        )}
      </div>
      ...
      <div className="border-t pt-4">
        <div className="text-sm text-gray-600 font-medium mb-2">Customer</div>
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-gray-800">{channelName}</span>
        </div>
        <div className="flex items-center gap-2">
          <LucideReact.Globe size={16} className="text-gray-500" />
          <span className="text-gray-800">{customerIp}</span>
        </div>
      </div>
    </div>
  );
