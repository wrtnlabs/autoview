...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const pageInfo = `Page ${pagination.current} of ${pagination.pages} · ${pagination.records.toLocaleString()} records · ${pagination.limit} per page`;
  ...
  return (
    <div className="space-y-6">
      {/* Page summary */}
      <div className="px-4 py-3 bg-white rounded-lg shadow-sm text-sm text-gray-700">
      ...
      <ul className="space-y-4">
        {coupons.map((coupon) => {
          const disc = coupon.discount;
          const isAmount = disc.unit === "amount";
          const discountLabel = isAmount
            ? `$${disc.value.toLocaleString()}`
            : `${disc.value.toLocaleString()}%`;
          return (
            <li
              key={coupon.id}
              className="bg-white rounded-lg shadow-sm p-4 flex flex-col space-y-3"
            >
              {/* Title */}
              ...
              {/* Discount */}
              <div className="flex items-center flex-wrap text-gray-600 text-sm gap-2">
                {isAmount ? (
                  ...
                  <LucideReact.Percent size={16} className="text-blue-500" />
                )}
                <span className="font-medium">{discountLabel}</span>
                {disc.threshold != null && (
                  <span>Min {disc.threshold.toLocaleString()}</span>
                )}
                {disc.limit != null && (
                  <span>Max {disc.limit.toLocaleString()}</span>
                )}
                {"multiplicative" in disc && disc.multiplicative && (
                  <span className="px-1 bg-gray-100 rounded text-xs">
                    × per unit
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center text-gray-600 text-sm space-x-2">
                ...
                <span>{formatDateTime(coupon.opened_at)}</span>
                <span className="mx-1">–</span>
                <span>{formatDateTime(coupon.closed_at)}</span>
              </div>
              ...
              <div className="flex items-center text-gray-600 text-sm space-x-2">
                ...
                <span>
                  {coupon.inventory.volume != null
                    ? coupon.inventory.volume.toLocaleString()
                    : "∞"}{" "}
                  total
                </span>
                <span className="mx-1">/</span>
                <span>
                  {coupon.inventory.volume_per_citizen != null
                    ? coupon.inventory.volume_per_citizen.toLocaleString()
                    : "∞"}{" "}
                  per user
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center text-gray-600 text-sm space-x-4">
                <div className="flex items-center space-x-1">
                  ...
                  <span>{coupon.restriction.access}</span>
                </div>
                <div className="flex items-center space-x-1">
                  ...
                  <span>
                    {coupon.restriction.exclusive ? "Exclusive" : "Stackable"}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center text-gray-600 text-sm space-x-1">
                <LucideReact.Filter size={16} />
                <span>{coupon.criterias.length} criteria</span>
              </div>
