...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // Utility: format ISO date string into readable form
  ...
  // Map enforcement levels to badge styles
  const enforcementStyles: Record<string, string> = {
    disabled: "bg-gray-200 text-gray-600",
    active: "bg-green-100 text-green-800",
    evaluate: "bg-amber-100 text-amber-800",
  };

  // Choose icon based on target type
  const getTargetIcon = (
    target?: AutoViewInputSubTypes.repository_ruleset["target"],
  ): JSX.Element => {
    switch (target) {
      case "branch":
        return <LucideReact.GitBranch className="text-blue-500" size={20} />;
      case "tag":
        return <LucideReact.Tag className="text-indigo-500" size={20} />;
      case "push":
        return (
          <LucideReact.ArrowUpCircle className="text-purple-500" size={20} />
        );
      case "repository":
      default:
        return <LucideReact.Layers className="text-gray-500" size={20} />;
    }
  };
  ...
  if (!Array.isArray(value) || value.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-gray-500">
        ...
        <span className="text-lg">No repository rulesets available</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {value.map((rs) => (
        ...
        >
          {/* Header: Icon, Name, Enforcement */}
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              {getTargetIcon(rs.target)}
              <h2
            ...
            <span
              className={`px-2 py-1 rounded-full text-sm ${enforcementStyles[rs.enforcement] || enforcementStyles.disabled}`}
            >
