...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const enforcementMap = {
    disabled: { icon: LucideReact.XCircle, label: 'Disabled', color: 'text-gray-400' },
    active: { icon: LucideReact.CheckCircle, label: 'Active', color: 'text-green-500' },
    evaluate: { icon: LucideReact.Clock, label: 'Evaluate', color: 'text-amber-500' },
  } as const;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {value.map((rs) => {
        const { icon: StatusIcon, label: statusLabel, color: statusColor } =
          enforcementMap[rs.enforcement];
        return (
          <div key={rs.id} className="bg-white p-4 rounded-lg shadow">
            {/* Header */}
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-semibold text-gray-800 truncate">{rs.name}</h3>
              <div className="flex items-center">
                <StatusIcon
                  size={18}
                  className={statusColor}
                  aria-label={`Enforcement: ${statusLabel}`}
                />
                <span className={`ml-1 text-sm font-medium ${statusColor}`}>{statusLabel}</span>
              </div>
            </div>
            {/* Metadata */}
            <div className="flex flex-wrap items-center text-sm text-gray-600 gap-4 mb-2">
              {rs.target && (
                <span className="capitalize">
                  Target: <span className="font-medium">{rs.target}</span>
                </span>
              )}
              <span className="capitalize">
                Source: <span className="font-medium">{rs.source_type ?? 'Repository'}</span>
                <span className="ml-1">({rs.source})</span>
              </span>
              <span>
                <LucideReact.List size={16} className="inline-block text-gray-500 mr-1" />
                <span className="align-middle">{rs.rules?.length ?? 0} rules</span>
              </span>
              {rs.bypass_actors && rs.bypass_actors.length > 0 && (
                <span>
                  <LucideReact.UserCheck
                    size={16}
                    className="inline-block text-gray-500 mr-1"
                  />
                  <span className="align-middle">{rs.bypass_actors.length} bypass</span>
                </span>
              )}
            </div>
            {/* Dates */}
            <div className="flex items-center text-sm text-gray-500 space-x-6">
              <div className="flex items-center">
                ...
                <span>{formatDate(rs.created_at)}</span>
              </div>
              <div className="flex items-center">
                <LucideReact.Edit2 size={16} className="mr-1" />
                <span>{formatDate(rs.updated_at)}</span>
              </div>
            </div>
          </div>
        );
