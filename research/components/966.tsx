...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const ownerName = value.owner?.login ?? "No owner";
  ...
  const sampleRepos = value.repositories.slice(0, 3).map((r) => r.name);
  const moreCount = repoCount - sampleRepos.length;
          ...
          {repoCount > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {sampleRepos.map((name) => (
                <span
                  key={name}
                  className="px-2 py-0.5 bg-gray-100 rounded-full text-xs text-gray-700 truncate"
                >
                  {name}
                </span>
              ))}
