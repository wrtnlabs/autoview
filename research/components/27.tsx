...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const answerSnapshots = answer?.snapshots || [];
  const latestAnswerSnapshot = answerSnapshots[answerSnapshots.length - 1];

  // 2. Compose the visual structure using JSX and Tailwind CSS
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      {/* Header: question type, secret indicator, date */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-800">
            {value.secret ? "Secret Question" : "Question"}
          </span>
        </div>
      ...
      <div className="mt-2 flex flex-wrap items-center text-sm text-gray-600 gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <span>
            {value.read_by_seller ? "Read by Seller" : "Unread by Seller"}
          </span>
        </div>
      </div>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          {latestAnswerSnapshot.files.length > 0 && (
            <div className="mt-2 flex items-center text-sm text-gray-600 gap-1">
              <LucideReact.FileText size={16} />
              <span>
                {latestAnswerSnapshot.files.length} attachment
                {latestAnswerSnapshot.files.length > 1 ? "s" : ""}
              </span>
            </div>
          )}
        </div>
