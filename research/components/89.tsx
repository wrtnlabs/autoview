...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { writer, snapshots } = value;
  const latestSnapshot = snapshots[snapshots.length - 1];

  // Format timestamp
  const dateObj = new Date(latestSnapshot.created_at);
  const formattedDate =
  ...
  // Determine how to render body
  const isHTML = latestSnapshot.format === "html";
  const bodyContent = latestSnapshot.body;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-700">{writerName}</span>
        </div>
      ...
      {/* Comment Body */}
      <div className="text-gray-800 text-sm mb-2 break-words whitespace-pre-wrap line-clamp-6">
        {isHTML ? (
          <div
