...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const writerName =
    ...
    writer.citizen?.name ??
    ("external_user" in writer ? writer.external_user?.nickname : undefined) ??
    writer.id;
  const writerRole = writer.type
  ...
  const editCount = snapshots.length - 1;
  const maxBodyLength = 300;
  const displayBody =
    latest.body.length > maxBodyLength
      ? latest.body.slice(0, maxBodyLength) + "…"
      : latest.body;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md w-full max-w-md mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-gray-500">{writerRole}</span>
          </div>
        </div>
      ...
      {editCount > 0 && (
        <div className="mt-1 flex items-center text-xs text-gray-400">
          <LucideReact.Edit3 size={12} className="mr-1" />
          Edited {editCount} {editCount > 1 ? "times" : "time"} on{" "}
