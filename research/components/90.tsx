    ...
    <section className="p-4 bg-gray-50 rounded-lg">
      <header className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
          <LucideReact.MessageCircle size={20} className="text-blue-500" aria-label="Comments" />
          Comments
        </h2>
      ...
      <ul className="space-y-4">
        {data.map((comment) => {
          ...
          // Derive writer name
          let writerName = comment.writer.id;
          if (comment.writer.member && 'nickname' in comment.writer.member) {
            writerName = comment.writer.member.nickname;
          } else if (comment.writer.citizen) {
            writerName = comment.writer.citizen.name;
          }

          // Choose icon based on writer type
          const roleIcon =
            comment.writer.type === 'administrator' ? (
              <LucideReact.UserCheck size={16} className="text-blue-500" aria-label="Administrator" />
            ) : comment.writer.type === 'seller' ? (
              <LucideReact.ShoppingBag size={16} className="text-green-500" aria-label="Seller" />
            ) : (
              <LucideReact.User size={16} className="text-gray-500" aria-label="Customer" />
            );
            ...
            <li key={comment.id} className="p-4 bg-white rounded-lg shadow">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-gray-700">
                  {roleIcon}
                  <span className="font-medium truncate">{writerName}</span>
                </div>
                <div className="flex items-center gap-1 text-xs text-gray-500">
                  <LucideReact.Calendar size={14} className="text-gray-400" aria-label="Date" />
                  <time dateTime={comment.created_at}>{formatDate(comment.created_at)}</time>
                </div>
              </div>
              <div className="mt-2 text-gray-800 text-sm line-clamp-2">
                {latest.body}
              </div>
              <div className="mt-3 flex items-center text-xs text-gray-500 space-x-4">
                <div className="flex items-center gap-1">
                  <LucideReact.Edit2 size={14} className="text-gray-400" aria-label="Edits" />
                  <span>
                    {snapshots.length} {snapshots.length > 1 ? 'edits' : 'edit'}
                  </span>
                </div>
                {latest.files.length > 0 && (
                  <div className="flex items-center gap-1">
                    <LucideReact.FileText size={14} className="text-gray-400" aria-label="Attachments" />
                    <span>
                      {latest.files.length} {latest.files.length > 1 ? 'attachments' : 'attachment'}
                    </span>
                  </div>
                )}
              </div>
