...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Utility: derive a human-readable name for various writer types
  function getWriterName(
    writer: AutoViewInputSubTypes.IShoppingSaleInquiryComment["writer"],
  ): string {
    // Administrator or Seller invert always have member.nickname
    if (
      "type" in writer &&
      (writer.type === "administrator" || writer.type === "seller")
    ) {
      return writer.member.nickname;
    }
    // Customer may have member.nickname or fallback to "Customer"
    if ("type" in writer && writer.type === "customer") {
      return writer.member?.nickname ?? "Customer";
    }
    // Fallback
    return "Unknown";
  }

  // Format ISO date strings into medium date + short time
            ...
            >
              ...
              {snapshots.length > 1 && (
                <div className="mt-2 flex items-center text-gray-500 text-sm">
                  ...
                  <span>{snapshots.length} edits</span>
                </div>
              )}
            </div>
          );
