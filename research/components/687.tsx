...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const activeStates = new Set<AutoViewInput["state"]>(["Available"]);
  const pendingStates = new Set<AutoViewInput["state"]>([
    "Unknown",
    "Created",
    "Queued",
    "Provisioning",
  ...
  const { icon: statusIcon, label: statusLabel } = (() => {
    ...
    if (pendingStates.has(value.state)) {
      return {
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      {/* Repository */}
      <div className="flex items-center mb-4 text-sm text-gray-600 space-x-1">
      ...
      {/* Location, Idle Timeout, Retention */}
      <div className="flex flex-wrap items-center gap-4 mb-4 text-sm text-gray-600">
        ...
        {value.idle_timeout_minutes != null && (
          <div className="flex items-center space-x-1">
            ...
            <span>Idle timeout: {value.idle_timeout_minutes}m</span>
          </div>
        )}
        {retentionExpires && (
          <div className="flex items-center space-x-1">
            <LucideReact.Calendar className="text-gray-400" size={16} />
            <span>Retain until: {retentionExpires}</span>
          </div>
        )}
      </div>
      ...
      {value.pending_operation && (
        <div className="flex items-start bg-amber-50 border-l-4 border-amber-400 p-3 rounded">
          <LucideReact.AlertTriangle className="text-amber-600" size={20} />
          <p className="ml-2 text-sm text-amber-800">
            {value.pending_operation_disabled_reason ||
              "Operation pending, codespace unavailable."}
          </p>
        </div>
      )}
    </div>
