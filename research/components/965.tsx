...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const ownerName = value.owner?.login ?? "Unknown";
  const avatarFallback = `https://ui-avatars.com/api/?name=${encodeURIComponent(
    ownerName,
  )}&background=0D8ABC&color=fff`;
  const avatarUrl = value.owner?.avatar_url ?? avatarFallback;

  const stateKey = value.state.toLowerCase();
  let statusIcon: JSX.Element;
  const statusText = value.state.charAt(0).toUpperCase() + value.state.slice(1);
  switch (stateKey) {
    case "pending":
    ...
    case "exporting":
      statusIcon = (
        <LucideReact.Loader size={16} className="animate-spin text-gray-500" />
      );
      break;
    case "completed":
      statusIcon = (
  ...
  const firstRepos = repos.slice(0, 3).map((repo) => repo.name);
  const restCount = repos.length - firstRepos.length;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-full">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center">
          ...
          />
          <span className="ml-2 font-semibold text-gray-800">{ownerName}</span>
        </div>
        <div className="flex items-center text-sm">
          {statusIcon}
          <span className="ml-1 font-medium capitalize text-gray-700">
            {statusText}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600">
        ...
        <div>
          ...
          <div className="ml-5">{createdDate}</div>
        </div>
        ...
        <div>
          ...
          <div className="ml-5">{updatedDate}</div>
        </div>
        ...
        <div className="sm:col-span-2">
          <div className="flex items-center">
            ...
            <span className="ml-1">Migration URL:</span>
          </div>
          <div className="ml-5 break-all text-blue-600">{value.url}</div>
        </div>
        ...
        <div className="sm:col-span-2">
          <div className="flex items-center">
            ...
            <span className="ml-1">Repositories ({repos.length}):</span>
          </div>
          <ul className="ml-5 list-disc list-inside">
            ...
            {restCount > 0 && (
              <li className="text-gray-500">+{restCount} more</li>
            )}
          </ul>
        </div>

        {/* Excluded Data Badges */}
        {value.exclude && value.exclude.length > 0 && (
          <div className="sm:col-span-2">
            <div className="flex items-center">
              <LucideReact.Tag size={16} className="text-gray-500" />
              <span className="ml-1">Excluded Data:</span>
            </div>
