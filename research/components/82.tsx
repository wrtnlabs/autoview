...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const hasDiscount = value.nominal > value.real;
  const savings = value.nominal - value.real;
  const couponCount = value.ticket_payments.length;
  const couponNames = value.ticket_payments
    .map((tp) => tp.ticket.coupon.name)
    .filter((name) => name);
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="mb-4 flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-2">
        ...
        {hasDiscount && (
          <span className="text-sm font-medium text-green-600">
            You saved {formatNumber(savings)}
          </span>
        )}
      </div>
      ...
      <ul className="space-y-3 text-gray-700">
        <li className="flex items-center">
          ...
          />
          <span>Cash: {formatNumber(value.cash)}</span>
        </li>
        <li className="flex items-center">
          <LucideReact.PiggyBank
            size={16}
            className="text-gray-400 mr-2 flex-shrink-0"
          />
          <span>Deposit: {formatNumber(value.deposit)}</span>
        </li>
        <li className="flex items-center">
          <LucideReact.Star
            size={16}
            className="text-gray-400 mr-2 flex-shrink-0"
          />
          <span>Mileage: {formatNumber(value.mileage)}</span>
        </li>
        <li className="flex items-center">
          <LucideReact.Tag
            size={16}
            className="text-gray-400 mr-2 flex-shrink-0"
          />
          <span>Coupon Discount: –{formatNumber(value.ticket)}</span>
        </li>
      </ul>
      ...
      {couponCount > 0 && (
        <div className="mt-6">
          <h3 className="flex items-center text-sm font-medium text-gray-800 mb-2">
            <LucideReact.Tag
              size={16}
              className="text-gray-500 mr-2 flex-shrink-0"
            />
            {couponCount} Coupon{couponCount > 1 ? "s" : ""} Applied
          </h3>
