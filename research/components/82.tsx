...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  return (
    <div className="bg-white max-w-md mx-auto p-6 rounded-lg shadow-md">
      <h2 className="text-xl font-semibold text-gray-800 mb-4">Order Price Summary</h2>

      <div className="mb-6">
        {discount > 0 ? (
          <div className="flex flex-col sm:flex-row sm:items-baseline sm:space-x-4">
            <span className="text-gray-500 line-through text-lg">${formatAmount(nominal)}</span>
            <span className="text-2xl font-bold text-red-600">${formatAmount(real)}</span>
            <span className="mt-2 sm:mt-0 text-sm text-green-600">
              You saved ${formatAmount(discount)}
            </span>
          </div>
        ) : (
          <div className="text-2xl font-bold text-gray-800">${formatAmount(real)}</div>
        )}
      </div>

      <div className="grid grid-cols-2 gap-4 mb-6 text-center">
        <div>
          <span className="block text-sm text-gray-500">Cash</span>
          <span className="font-medium text-gray-800">${formatAmount(value.cash)}</span>
        </div>
        <div>
          <span className="block text-sm text-gray-500">Deposit</span>
          <span className="font-medium text-gray-800">${formatAmount(value.deposit)}</span>
        </div>
        <div>
          <span className="block text-sm text-gray-500">Mileage</span>
          <span className="font-medium text-gray-800">${formatAmount(value.mileage)}</span>
        </div>
        <div>
          <span className="block text-sm text-gray-500">Coupon Tickets</span>
          <span className="font-medium text-gray-800">
            ${formatAmount(value.ticket)} ({couponCount})
          </span>
        </div>
      </div>
      ...
      {couponCount > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-gray-700 mb-3">Coupon Details</h3>
          <ul className="space-y-2">
            {value.ticket_payments.map((payment) => {
              const exp = payment.ticket.expired_at
                ? new Date(payment.ticket.expired_at)
                : null;
              const isExpired = exp ? exp < new Date() : false;
              const formattedExpiry = exp
                ...
                : 'No expiry';
              const couponName = payment.ticket.coupon.name;
              return (
                <li
                  key={payment.id}
                  className="flex items-center justify-between p-3 bg-gray-50 rounded"
                >
                  <span className="text-gray-800 truncate">{couponName}</span>
                  <span
                    className={`text-sm font-medium ${
                      isExpired ? 'text-red-500' : 'text-green-600'
                    }`}
                  >
                    {isExpired ? 'Expired' : 'Valid until'} {formattedExpiry}
                  </span>
                </li>
              );
            })}
