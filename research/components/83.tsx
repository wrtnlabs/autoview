...
export namespace AutoViewInputSubTypes {
    export interface IShoppingOrderDiscountable {
        deposit: number;
        mileage: number;
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalDiscount = value.combinations.reduce((sum, comb) => sum + comb.amount, 0);
  const totalCoupons = value.combinations.reduce((sum, comb) => sum + comb.coupons.length, 0);
  const totalTickets = value.combinations.reduce((sum, comb) => sum + comb.tickets.length, 0);
  const totalEntries = value.combinations.reduce((sum, comb) => sum + comb.entries.length, 0);
      ...
      <div className="mt-6">
        ...
        <div className="flex flex-wrap text-sm text-gray-600 gap-4 mb-4">
          <div className="flex items-center gap-1">
            ...
            <span>Total: {totalCombinations}</span>
          </div>
          <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-1">
            <LucideReact.Pocket size={16} className="text-green-600" />
            <span>Total Discount: {totalDiscount.toLocaleString()}</span>
          </div>
        </div>

        {/* Detailed List */}
        <ul className="space-y-3">
          {value.combinations.map((comb, idx) => {
            const { coupons, tickets, entries, amount } = comb;
            return (
              <li
                key={idx}
