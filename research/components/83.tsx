...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalDiscount = combinations.reduce(
    (sum, combo) => sum + combo.amount,
    0,
  );
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md max-w-full overflow-x-auto">
      <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0 mb-6">
        <div className="flex-1 flex items-center bg-gray-50 p-4 rounded-lg">
      ...
      <section className="mb-4">
        ...
        <p className="text-sm text-gray-500">
          Total Combos:{" "}
          <span className="font-semibold">{totalCombinations}</span>, Total
          Discount:{" "}
      ...
      {combinations.length > 0 ? (
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {combinations.map((combo, idx) => {
                const couponsCount = combo.coupons.length;
                const ticketsCount = combo.tickets.length;
                const entriesCount = combo.entries.length;
                return (
                  <tr
                    key={idx}
                  ...
                  >
                    ...
                    <td className="px-3 py-2">{couponsCount}</td>
                    <td className="px-3 py-2">{ticketsCount}</td>
                    <td className="px-3 py-2">{entriesCount}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="flex flex-col items-center py-6 text-gray-400">
          <LucideReact.AlertCircle size={32} aria-hidden="true" />
          <span className="mt-2">No combinations available</span>
        </div>
      )}
    </div>
  );
