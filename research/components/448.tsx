...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Count occurrences of each event type for a summary badge row.
  const typeCounts = value.reduce((acc: Record<string, number>, event) => {
    const key = event.type ?? "Unknown";
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});

  // Helper to format ISO date‐time strings into a readable format.
  ...
  return (
    <div className="max-w-full bg-white rounded-lg shadow-md p-4">
      ...
      <ul className="divide-y divide-gray-200">
        {value.map((event) => {
          const { id, actor, type, repo, payload, created_at } = event;
            ...
            <li key={id} className="py-4">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  {/* Primary Event Line */}
                  <div className="flex flex-wrap items-center text-sm text-gray-700 gap-x-1 gap-y-1">
                    ...
                    <LucideReact.Code
                      ...
                      aria-hidden="true"
                    />
                    <span className="text-gray-600 truncate">{repo.name}</span>
                  </div>
                  ...
                  {payload.action && (
                    <div className="mt-1 text-sm text-gray-600">
                      Action:{" "}
                      <span className="capitalize">{payload.action}</span>
                    </div>
                  )}
                  {payload.issue && payload.issue.title && (
                    <div className="mt-1 flex items-center text-sm text-gray-600 line-clamp-2">
                      <LucideReact.FileText
                        size={16}
                        className="text-gray-400 flex-shrink-0 mr-1"
                        aria-hidden="true"
                      />
                      <span>{payload.issue.title}</span>
                    </div>
                  )}
                  {payload.comment && (
                    <div className="mt-1 flex items-center text-sm text-gray-600 line-clamp-2">
                  ...
                  {payload.pages && payload.pages.length > 0 && (
                    <div className="mt-1 flex items-center text-sm text-gray-600">
                      <LucideReact.File
                        ...
                        aria-hidden="true"
                      />
                      <span>{payload.pages.length} page(s)</span>
                    </div>
                  )}
