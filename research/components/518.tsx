...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Enforcement status icon and color
  let EnforcementIcon = LucideReact.Clock;
  let enforcementColor = "text-amber-500";
  if (value.enforcement === "active") {
    EnforcementIcon = LucideReact.CheckCircle;
    enforcementColor = "text-green-500";
  } else if (value.enforcement === "disabled") {
    EnforcementIcon = LucideReact.XCircle;
    enforcementColor = "text-red-500";
  }

  // Target icon mapping
  const targetMap: Record<string, JSX.Element> = {
    branch: <LucideReact.GitBranch size={16} />,
    tag: <LucideReact.Tag size={16} />,
    push: <LucideReact.ArrowUpRight size={16} />,
    repository: <LucideReact.GitCommit size={16} />,
  };

  // Rules summary
  const ruleTypes = value.rules?.map((r) => r.type) ?? [];
  const displayedRuleTypes = ruleTypes.slice(0, 3);
  const extraRulesCount =
    ruleTypes.length > displayedRuleTypes.length
      ? ruleTypes.length - displayedRuleTypes.length
      : 0;

  // Conditions summary
  const cond = value.conditions;
  let conditionsSummary: string | undefined;
  if (cond) {
    const parts: string[] = [];
    if ("repository_name" in cond) {
      if (cond.repository_name.include)
        parts.push(`Repos: ${cond.repository_name.include.join(", ")}`);
      if (cond.repository_name.exclude)
        parts.push(`Except: ${cond.repository_name.exclude.join(", ")}`);
    } else if ("repository_id" in cond) {
      if (cond.repository_id.repository_ids)
        parts.push(`Repo IDs: ${cond.repository_id.repository_ids.join(", ")}`);
    } else if ("repository_property" in cond) {
      if (cond.repository_property.include) {
        const list = cond.repository_property.include.map(
          (p) => `${p.name}=${p.property_values.join("|")}`
        );
        parts.push(`Props: ${list.join(", ")}`);
      }
      if (cond.repository_property.exclude) {
        const list = cond.repository_property.exclude.map(
          (p) => `${p.name}=${p.property_values.join("|")}`
        );
        parts.push(`Not Props: ${list.join(", ")}`);
      }
    }
    if ("ref_name" in cond && cond.ref_name) {
      if (cond.ref_name.include)
        parts.push(`Branches: ${cond.ref_name.include.join(", ")}`);
      if (cond.ref_name.exclude)
        parts.push(`Except: ${cond.ref_name.exclude.join(", ")}`);
    }
    conditionsSummary = parts.join("; ");
  }
  ...
  return (
    <div className="max-w-md w-full bg-white shadow-md rounded-lg p-4 mx-auto">
      <div className="flex items-center justify-between">
        ...
        <EnforcementIcon
          size={20}
          className={enforcementColor}
          role="img"
          aria-label={`Enforcement: ${value.enforcement}`}
        />
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-4 text-sm text-gray-600">
        {value.target && (
          <div className="flex items-center gap-1">
            {targetMap[value.target] || <LucideReact.Hash size={16} />}
            <span className="capitalize">{value.target}</span>
          </div>
        )}
        {value.source_type && (
          <div className="flex items-center gap-1">
            <LucideReact.Building size={16} className="text-gray-500" />
            <span>{value.source_type}</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-1 flex flex-wrap gap-2">
          {displayedRuleTypes.map((type) => (
            <span
              key={type}
              className="inline-flex items-center px-2 py-0.5 bg-gray-200 text-gray-800 text-xs font-medium rounded"
            >
              {type.replace(/_/g, " ")}
            </span>
          ))}
      ...
      {conditionsSummary && (
        <div className="mt-4">
          ...
          <p className="mt-1 text-sm text-gray-600">{conditionsSummary}</p>
        </div>
      )}
      ...
      {bypassCount > 0 && (
        <div className="mt-4 flex items-center gap-1 text-sm text-gray-700">
          ...
          <span>
            {bypassCount} bypass actor{bypassCount > 1 ? "s" : ""}
          </span>
        </div>
      )}

      {(created || updated) && (
        <div className="mt-4 border-t pt-2 flex flex-col gap-2 text-xs text-gray-500">
          {created && (
            <div className="flex items-center gap-1">
