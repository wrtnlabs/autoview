...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const enforcementDetails =
    enforcementMap[value.enforcement] || enforcementMap.disabled;

  const targetIconMap = {
    branch: <LucideReact.GitBranch className="text-gray-400" size={16} />,
    tag: <LucideReact.Tag className="text-gray-400" size={16} />,
    push: <LucideReact.ArrowUp className="text-gray-400" size={16} />,
    repository: <LucideReact.Folder className="text-gray-400" size={16} />,
  } as const;
  const targetIcon = value.target
    ? targetIconMap[value.target]
    : targetIconMap.branch;

  const rulesCount = Array.isArray(value.rules) ? value.rules.length : 0;
  const bypassCount = Array.isArray(value.bypass_actors)
    ? value.bypass_actors.length
    : 0;

  const includes = value.conditions?.ref_name?.include || [];
  const excludes = value.conditions?.ref_name?.exclude || [];
  const condParts: string[] = [];
  if (includes.length) condParts.push(`Include: ${includes.join(", ")}`);
  if (excludes.length) condParts.push(`Exclude: ${excludes.join(", ")}`);
  if (!condParts.length) condParts.push("All refs");
    ...
    <div className="p-4 bg-white rounded-lg shadow-md w-full max-w-md mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
        ...
        <div className="flex items-center space-x-1">
          {targetIcon}
          <span>{value.target ? capitalize(value.target) : "All"}</span>
        </div>
        ...
        {/* Enforcement */}
        <div className="flex items-center space-x-1">
          {enforcementDetails.icon}
          <span>{enforcementDetails.label}</span>
        </div>

        {/* Rule Count */}
        <div className="flex items-center space-x-1">
          <LucideReact.List className="text-gray-400" size={16} />
          <span>{rulesCount} Rules</span>
        </div>

        {/* Conditions (spans two columns) */}
        <div className="flex items-center space-x-1 col-span-2">
          <LucideReact.GitCommit className="text-gray-400" size={16} />
          <span>Conditions: {condSummary}</span>
        </div>

        {/* Bypass Actors */}
        <div className="flex items-center space-x-1">
          <LucideReact.User className="text-gray-400" size={16} />
          <span>Bypass: {bypassCount}</span>
        </div>
