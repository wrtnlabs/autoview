...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Thumbnail selection with fallback
  const thumbnailUrl =
  ...
  const prices = value.units.flatMap((unit) =>
  ...
  const totalStock = value.units.reduce(
    (sum, unit) =>
      sum +
      unit.stocks.reduce(
        (stockSum, stock) =>
          stockSum + (stock.inventory.income - stock.inventory.outcome),
        0,
      ),
    0,
  );
  ...
  } else if (value.opened_at && new Date(value.opened_at).getTime() > now) {
    statusText = "Upcoming";
    StatusIcon = LucideReact.Clock;
    statusColor = "text-blue-500";
  } else if (value.closed_at && new Date(value.closed_at).getTime() < now) {
    statusText = "Closed";
  ...
  const rawDesc = value.content.body || "";
  const description =
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-2 flex items-center text-sm text-gray-600 space-x-4">
          <div className="flex items-center">
            <LucideReact.DollarSign size={16} className="text-gray-400 mr-1" />
            <span>{priceDisplay}</span>
          </div>
          <div className="flex items-center">
            <LucideReact.Package size={16} className="text-gray-400 mr-1" />
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {value.categories.map((cat) => (
            <span
              key={cat.id}
              className="flex items-center bg-gray-100 text-gray-700 text-xs px-2 py-0.5 rounded"
            >
              <LucideReact.Tag size={12} className="text-gray-400 mr-1" />
              {cat.name}
            </span>
          ))}
          {value.tags.map((tag) => (
        ...
        <div className="mt-4 text-xs text-gray-500 flex flex-wrap gap-3">
          ...
          <div className="flex items-center">
            <LucideReact.MapPin size={14} className="mr-1" />
            {sectionName}
          </div>
          <div className="flex items-center">
