...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Seller and section names
  const sellerName = value.seller.member.nickname;
  const sectionName = value.section.name;

  // Thumbnail image (first available)
  const thumbnailUrl = value.content.thumbnails[0]?.url ?? "";

  // Short description snippet (first 100 chars)
  const rawBody = value.content.body ?? "";
  const snippet =
    rawBody.length > 100 ? rawBody.slice(0, 100).trimEnd() + "…" : rawBody;
  ...
  const openDate = formatDate(value.opened_at);
  const closeDate =
    value.closed_at && value.closed_at !== value.opened_at
  ...
  else if (opened && now < opened) status = "Upcoming";
  else if (closed && now > closed) status = "Closed";
    ...
    <div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-4 flex flex-col space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`px-2 py-1 text-xs font-medium rounded ${badgeBg} ${badgeText}`}
          >
            {status}
          </span>
        </div>
        <p className="text-sm text-gray-600 truncate">
          {sectionName} • Seller: {sellerName}
        </p>
        ...
        <p className="text-gray-700 text-sm line-clamp-3">{snippet}</p>
