...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const title: string = value.content.title;
  const sellerName: string = value.seller.member.nickname;
  const sectionName: string = value.section.name;
  const categoryChips = value.categories.map(c => ({ id: c.id, name: c.name }));
  const tagsList = value.tags;
  // Remove basic markdown markers for preview
  const plainBody = value.content.body.replace(/[#_*~`>]/g, '').trim();
  const truncatedBody =
    plainBody.length > 120 ? plainBody.slice(0, 120).trimEnd() + '…' : plainBody;
  const mainImage: string | null =
    value.content.thumbnails.length > 0 ? value.content.thumbnails[0].url : null;
  // Aggregate all real prices across all stocks
  const allStocks = value.units.flatMap(u => u.stocks);
  const realPrices = allStocks.map(s => s.price.real);
  const hasPrices = realPrices.length > 0;
  const minPrice = hasPrices ? Math.min(...realPrices) : 0;
  const maxPrice = hasPrices ? Math.max(...realPrices) : 0;
  const formatNumber = (n: number) => new Intl.NumberFormat().format(n);
  // Derive status and icon
  let statusLabel = '';
  ...
  if (value.suspended_at) {
    ...
    statusLabel = 'Paused';
    statusIcon = <LucideReact.Pause className="text-amber-500" size={16} />;
    ...
    statusIcon = <LucideReact.Clock className="text-blue-500" size={16} />;
  }
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        {mainImage && (
          <img
            src={mainImage}
            alt={title}
            className="w-full sm:w-1/3 aspect-video object-cover rounded"
            onError={(e) => {
              (e.target as HTMLImageElement).src =
                'https://placehold.co/300x200/f8fafc/475569?text=No+Image';
            }}
          />
        )}
        <div className="flex-1 flex flex-col">
          ...
          <p className="text-gray-600 text-sm flex-1">{truncatedBody}</p>
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="text-gray-800 font-medium">
              {hasPrices
                ? minPrice === maxPrice
                  ? `Price: ${formatNumber(minPrice)}`
                  : `Price: ${formatNumber(minPrice)} – ${formatNumber(maxPrice)}`
                : 'Price: N/A'}
            </div>
            <div className="flex items-center text-sm">
              {statusIcon}
              <span className="ml-1 text-gray-600">{statusLabel}</span>
            </div>
          </div>
        </div>
      </div>
