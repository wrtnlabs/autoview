...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const reviewDate = new Date(value.created_at).toLocaleDateString("en-US", {
    year: "numeric",
    month: "short",
    day: "numeric",
  });
  ...
  if (answer) {
    const ansSnaps = [...answer.snapshots].sort(
      (a, b) =>
        new Date(a.created_at).getTime() - new Date(b.created_at).getTime(),
    );
    latestAnswerSnapshot = ansSnaps[ansSnaps.length - 1];
    answerDate = new Date(answer.created_at).toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
    ...
    <div className="p-4 bg-white rounded-lg shadow-md">
      {latestSnapshot ? (
        <>
          <div className="flex items-start justify-between">
            <div className="text-sm text-gray-600">
              ...
              <span className="ml-2">{reviewDate}</span>
            </div>
            {isRead ? (
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <div className="flex items-center text-sm text-gray-500">
              <LucideReact.Star className="text-amber-400" size={18} />
              <span>{latestSnapshot.score}</span>
            </div>
          </div>
        ...
        <div className="text-center text-gray-400">
          ...
          <p className="mt-2">No review data</p>
        </div>
      )}
    </div>
  );
