...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const variantCount = units.reduce((sum, u) => sum + u.stocks.length, 0);
  const priceList = units.flatMap((u) => u.stocks.map((s) => s.price.real));
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
    e.currentTarget.src = `https://placehold.co/300x300/f8fafc/475569?text=${encodeURIComponent(
      content.title,
    )}`;
  };
