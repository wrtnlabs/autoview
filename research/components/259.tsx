...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const rawText = message?.plainText ?? '';
  const snippet = rawText.length > 100 ? rawText.slice(0, 100) + '…' : rawText;
  ...
  function getTagClasses(variant?: string): string {
    switch (variant) {
      ...
      case 'cobalt': return 'bg-blue-100 text-blue-800';
      case 'navy': return 'bg-blue-200 text-blue-900';
      ...
      <div className="flex-1 ml-4 min-w-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            {isBookmarked && <span className="text-gray-400">📌</span>}
            {session?.unread ? (
        ...
        {snippet && (
          <p className="mt-1 text-sm text-gray-600 line-clamp-2">{snippet}</p>
        )}
        {chatTags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {chatTags.map((tag, idx) => (
              <span
                key={tag.key ?? tag.id ?? idx}
                className={`${getTagClasses(tag.colorVariant)} text-xs font-medium px-2 py-0.5 rounded`}
              >
                {tag.name}
              </span>
            ))}
