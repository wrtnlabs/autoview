...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  return (
    <div className="w-full p-4 bg-white rounded-lg shadow-md flex flex-col space-y-4">
      {/* Header with pagination info */}
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b pb-2">
      ...
      <ul className="flex flex-col divide-y">
        {comments.map((comment) => {
          const { id, snapshots, created_at, writer } = comment;
          const latest = snapshots[snapshots.length - 1];

          // Determine writer role and display name
          const role = writer.type;
          let displayName: string;
          if (writer.member) {
            displayName = writer.member.nickname;
          } else if ("citizen" in writer && writer.citizen) {
            displayName = writer.citizen.name;
          } else {
