...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Helper to extract writer info: role and display name
  function getWriterInfo(
    writer: AutoViewInputSubTypes.IShoppingSaleInquiryComment["writer"],
  ): { role: string; name: string } {
    if ("type" in writer) {
      if (writer.type === "seller") {
        return { role: "Seller", name: writer.member.nickname };
      }
      if (writer.type === "customer") {
        return {
          role: "Customer",
          name: writer.member?.nickname ?? writer.citizen?.name ?? writer.id,
        };
      }
    }
    // Administrator invert case (no discriminant)
    return {
      role: "Administrator",
  ...
  return (
    <section className="p-4 bg-gray-50">
      <div className="mb-4 text-sm text-gray-600">
        Showing {comments.length} of {pagination.records} comments (Page{" "}
      ...
      <div className="space-y-4">
        {comments.map((comment) => {
          ...
          const { role, name } = getWriterInfo(comment.writer);

          return (
            <div key={comment.id} className="bg-white p-4 rounded-lg shadow">
              <div className="flex items-center justify-between">
                <div className="flex items-center text-sm text-gray-800">
                  <LucideReact.User size={18} className="text-gray-500 mr-1" />
