...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const discountLabel =
    discount.unit === "amount" ? `$${discount.value}` : `${discount.value}%`;
  const thresholdLabel =
    discount.threshold !== null ? `Min. spend: $${discount.threshold}` : null;
  const limitLabel =
    "limit" in discount && discount.limit !== null
      ? `Max discount: $${discount.limit}`
      : null;
  const isMultiplicative =
    "multiplicative" in discount && discount.multiplicative;
  ...
  const validityStart = value.opened_at ? new Date(value.opened_at) : null;
  const validityEnd = value.closed_at ? new Date(value.closed_at) : null;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 max-w-md">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="inline-flex items-center bg-blue-100 text-blue-800 text-sm font-medium px-2 py-1 rounded">
          {discount.unit === "amount" ? (
            <LucideReact.DollarSign size={14} className="mr-1" />
          ) : (
            <LucideReact.Percent size={14} className="mr-1" />
          )}
          {discountLabel}
        </span>
      </div>
      ...
      <div className="text-sm text-gray-600 mb-4 space-y-1">
        ...
        {limitLabel && (
          <div className="flex items-center">
            <LucideReact.Maximize2 size={14} className="mr-1" />
            <span>{limitLabel}</span>
          </div>
        )}
        {isMultiplicative && (
          <div className="flex items-center">
            <LucideReact.Repeat size={14} className="mr-1" />
            <span>Multiplicative discount</span>
          </div>
        )}
      </div>

      {/* Inventory, Validity, Access */}
      <div className="grid grid-cols-2 gap-4 mb-4 text-sm text-gray-600">
        {value.inventory.volume !== null && (
          <div className="flex items-center">
            <LucideReact.Layers size={16} className="mr-2" />
            <span>Stock left: {value.inventory.volume}</span>
          </div>
        )}
        {value.inventory.volume_per_citizen !== null && (
          <div className="flex items-center">
            <LucideReact.User size={16} className="mr-2" />
            <span>Per person: {value.inventory.volume_per_citizen}</span>
          </div>
        )}
        <div className="flex items-center">
          <LucideReact.Calendar size={16} className="mr-2" />
          <span>
            {validityStart ? formatDate(validityStart) : "Any"} –{" "}
            {validityEnd ? formatDate(validityEnd) : "∞"}
          </span>
        </div>
        <div className="flex items-center">
          {value.restriction.access === "public" ? (
            <LucideReact.Unlock size={16} className="mr-2 text-gray-500" />
          ) : (
            <LucideReact.Lock size={16} className="mr-2 text-gray-500" />
          )}
      ...
      <div className="border-t pt-3 text-xs text-gray-500 space-y-1">
        <div className="flex items-center">
          ...
          <span>Designer ID: {value.designer.id}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.Calendar size={14} className="mr-1" />
