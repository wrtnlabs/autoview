  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-6">
      ...
      <div className="flex items-center text-gray-700">
        <LucideReact.Calendar size={20} strokeWidth={1.5} />
        <span className="ml-2 font-semibold">
          Connected on {formatDate(value.created_at)}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-600">
        <div className="flex items-center truncate">
          ...
          <span className="ml-2 truncate">{value.href}</span>
        </div>
        <div className="flex items-center truncate">
          <LucideReact.Link size={16} />
          <span className="ml-2 truncate">
            {value.referrer && value.referrer.length > 0
              ? value.referrer
              : "No Referrer"}
          </span>
        </div>
        <div className="flex items-center">
          ...
          <span className="ml-2">{value.ip}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.Tag size={16} />
          <span className="ml-2">{value.channel.name}</span>
        </div>
      </div>

      {/* Verified Citizen Section */}
      {value.citizen && (
        <div className="p-3 bg-gray-50 rounded space-y-1">
          <div className="flex items-center text-gray-700">
            <LucideReact.UserCheck size={16} />
            <span className="ml-2 font-medium">Verified Citizen</span>
          </div>
      ...
      {value.member && (
        <div className="p-3 bg-gray-50 rounded space-y-1">
          <div className="flex items-center text-gray-700">
            ...
            <span className="ml-2 font-medium">Member Account</span>
          </div>
          <div className="flex items-center text-gray-600">
            <LucideReact.User size={14} />
            <span className="ml-1">{value.member.nickname}</span>
          </div>
          <div className="flex items-center text-gray-600">
            <LucideReact.Mail size={14} />
            <span className="ml-1">
              {value.member.emails.map((e) => e.value).join(", ")}
            </span>
          </div>
        </div>
      )}
      ...
      {value.external_user && (
        <div className="p-3 bg-gray-50 rounded space-y-1">
          <div className="flex items-center text-gray-700">
            ...
            <span className="ml-2 font-medium">External User</span>
          </div>
          <div className="text-gray-600">
            ...
            <span>{value.external_user.uid}</span>
          </div>
          <div className="flex items-center text-gray-600">
            <LucideReact.User size={14} />
            <span className="ml-1">{value.external_user.nickname}</span>
          </div>
        </div>
      )}
    </div>
