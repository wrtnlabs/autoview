...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const memberSince = new Date(value.member.created_at).toLocaleString();
  const connectionTime = new Date(value.customer.created_at).toLocaleString();
  const verifiedAt = new Date(value.citizen.created_at).toLocaleString();

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-6 text-gray-800">
      ...
      {/* Administrator Joined Date */}
      <div className="flex items-center text-sm text-gray-500 space-x-2">
        <LucideReact.Calendar size={16} />
        <span>Joined: {adminSince}</span>
      </div>

      {/* Membership Section */}
      <section>
        <h3 className="text-md font-medium text-gray-700 mb-2">Membership</h3>
        <div className="space-y-2 text-sm">
          <div className="flex items-center space-x-2">
            <LucideReact.Users size={16} className="text-gray-500" />
            <span>Nickname: {value.member.nickname}</span>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <LucideReact.Mail size={16} className="text-gray-500" />
            {value.member.emails.length > 0 ? (
              value.member.emails.map((email) => (
                <span
                  key={email.id}
                  className="inline-block bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full truncate"
                >
                  {email.value}
                </span>
              ))
            ) : (
              <span className="text-gray-400">No email addresses</span>
            )}
          </div>
          <div className="flex items-center space-x-2">
      ...
      <section>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <span>Channel: {value.customer.channel.name}</span>
          </div>
          {value.customer.external_user && (
            <div className="flex items-center space-x-2">
              <LucideReact.Link size={16} className="text-gray-500" />
              <span>
                External ({value.customer.external_user.application}):
          ...
          {value.customer.referrer && (
            <div className="flex items-center space-x-2">
              <LucideReact.Link size={16} className="text-gray-500" />
              <span className="truncate" title={value.customer.referrer}>
                {value.customer.referrer}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center space-x-2">
            <LucideReact.Calendar size={16} className="text-gray-500" />
            <span>Connected: {connectionTime}</span>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-medium">{value.citizen.name}</span>
          </div>
          <div className="flex items-center space-x-2">
