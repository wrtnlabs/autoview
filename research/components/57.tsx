...
export namespace AutoViewInputSubTypes {
    ...
    */
    export interface IPageIShoppingCoupon {
        /**
         * Page information.
         *
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const startRecord = (pagination.current - 1) * pagination.limit + 1;
  const endRecord = Math.min(pagination.current * pagination.limit, pagination.records);
  ...
  return (
    <div className="p-4 bg-gray-50">
      <div className="mb-4 flex flex-col sm:flex-row sm:justify-between text-sm text-gray-600">
        ...
        <div className="flex items-center gap-1">
          ...
          <span>Showing {startRecord}-{endRecord} of {pagination.records}</span>
        </div>
      </div>
          ...
          <div key={coupon.id} className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-gray-600">
              {coupon.discount.unit === 'amount' ? (
                <div className="flex items-center gap-1">
                  <LucideReact.CreditCard className="text-indigo-500" size={16} />
                  <span>${coupon.discount.value.toFixed(2)}</span>
                </div>
              ) : (
                <div className="flex items-center gap-1">
                  <LucideReact.Percent className="text-green-500" size={16} />
                  <span>{coupon.discount.value}%</span>
                </div>
              )}
              {coupon.discount.threshold !== null && (
                <div className="flex items-center gap-1">
                  <LucideReact.Minus className="text-gray-400" size={16} />
                  <span>Min ${coupon.discount.threshold.toFixed(2)}</span>
                </div>
              )}
            </div>
            <div className="mt-3 space-y-2 text-sm text-gray-600">
              <div className="flex items-center gap-1">
                ...
                <span>
                  Inventory:&nbsp;
                  {coupon.inventory.volume === null ? '∞' : coupon.inventory.volume}
                  {' '}/ per customer&nbsp;
              ...
              <div className="flex items-center gap-1">
                ...
                <span>
                  Valid:&nbsp;{formatDate(coupon.opened_at)} – {formatDate(coupon.closed_at)}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <LucideReact.Shield className="text-gray-400" size={16} />
                <span>
                  Access:&nbsp;
                  {coupon.restriction.access === 'public' ? 'Public' : 'Private'}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <LucideReact.Lock className="text-gray-400" size={16} />
                <span>
                  {coupon.restriction.exclusive ? 'Exclusive' : 'Non-exclusive'}
                </span>
              </div>
            </div>
          </div>
