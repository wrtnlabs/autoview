...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { current, records, pages } = value.pagination;

  const formatDate = (dateStr: string | null, options?: Intl.DateTimeFormatOptions): string => {
    if (!dateStr) return 'N/A';
    return new Date(dateStr).toLocaleDateString(undefined, options || { year: 'numeric', month: 'short', day: 'numeric' });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-gray-500">
        Showing {value.data.length} of {records} coupons &nbsp;|&nbsp; Page {current} of {pages}
      </div>

      {/* Coupon list */}
      <div className="grid grid-cols-1 gap-4">
        {value.data.map((coupon) => (
          <div key={coupon.id} className="p-4 bg-white rounded-lg shadow-sm">
            {/* Coupon Title */}
            <h2 className="text-lg font-semibold text-gray-800 truncate">{coupon.name}</h2>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              <span
                className={`px-2 py-1 text-xs font-medium rounded ${
                  coupon.inventory.volume != null ? 'bg-blue-100 text-blue-800' : 'bg-blue-50 text-blue-800'
                }`}
              >
                {coupon.inventory.volume != null ? `Total: ${coupon.inventory.volume}` : 'Unlimited'}
              </span>

              {/* Per-Citizen Inventory */}
              <span
                className={`px-2 py-1 text-xs font-medium rounded ${
              ...
              >
                {coupon.inventory.volume_per_citizen != null
                  ? `Per Person: ${coupon.inventory.volume_per_citizen}`
                  : 'Unlimited/Person'}
              </span>
            </div>
            ...
            <p className="mt-2 text-sm text-gray-600">
              Valid: {formatDate(coupon.opened_at)} &ndash; {formatDate(coupon.closed_at)}
            </p>
          </div>
        ))}
      </div>
