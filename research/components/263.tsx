...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const name = user?.name || user?.memberId || "Unknown User";
  const initials = name
    .split(" ")
    .map((w) => w[0])
    .join("")
  ...
  const online = Boolean(value.userOnline);

  const chatState = value.userChat?.state;
  const chatStateInfo = (() => {
    const map: Record<string, { label: string; classes: string }> = {
      opened: { label: "Open", classes: "bg-green-100 text-green-800" },
      queued: { label: "Queued", classes: "bg-yellow-100 text-yellow-800" },
      snoozed: { label: "Snoozed", classes: "bg-orange-100 text-orange-800" },
      closed: { label: "Closed", classes: "bg-red-100 text-red-800" },
    };
    return chatState && map[chatState] ? map[chatState] : null;
  })();

  const message = value.message;
  const messageText = message?.plainText || "No recent message";
  ...
  if (value.campaign?.name) details.push(`Campaign: ${value.campaign.name}`);
  if (value.supportBot?.botName) details.push(`Bot: ${value.supportBot.botName}`);
  if (value.workflow?.name) details.push(`Workflow: ${value.workflow.name}`);
  if (value.appMessenger?.displayName)
    details.push(`Messenger: ${value.appMessenger.displayName}`);

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  // 3. Return the React element.
  return (
    <div className="p-4 bg-white rounded-lg shadow-md flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0">
      ...
      <div className="flex-1 flex flex-col justify-between">
        {/* Header: Name & Chat State */}
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900 truncate">{name}</h3>
          {chatStateInfo && (
            <span
              className={`px-2 py-0.5 text-xs font-medium rounded ${chatStateInfo.classes}`}
            >
              {chatStateInfo.label}
            </span>
          )}
        ...
        {messageTime && (
          <div className="mt-1 text-gray-400 text-xs">{messageTime}</div>
        )}

        {/* Tags & Details */}
        {(tags.length > 0 || details.length > 0) && (
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {tags.map((tag) => (
              <span
                key={tag.key}
                className={`px-2 py-0.5 text-xs font-medium rounded ${
                  tagColorClasses[tag.colorVariant || "gray"]
                }`}
              >
                {tag.name}
              </span>
