...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { nominal, real, cash, deposit, mileage, ticket, ticket_payments } = value;
  const discount = nominal - real;
  const couponCount = ticket_payments.length;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md w-full">
      <h2 className="text-xl font-semibold text-gray-900">Order Price Summary</h2>

      <dl className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
        <div className="flex flex-col">
          <dt className="text-sm text-gray-600">Nominal Price</dt>
          <dd className="text-base font-medium text-gray-800">{formatAmount(nominal)}</dd>
        </div>

        <div className="flex flex-col">
          <dt className="text-sm text-gray-600">Real Price</dt>
          <dd className="text-base font-medium text-gray-800">{formatAmount(real)}</dd>
        </div>

        {discount > 0 && (
          <div className="flex flex-col sm:col-span-2">
            <dt className="text-sm text-gray-600">Discount Given</dt>
            <dd className="text-base font-medium text-green-600">{formatAmount(discount)}</dd>
          </div>
        )}
      </dl>

      <h3 className="mt-6 text-lg font-semibold text-gray-900">Payment Breakdown</h3>
      <ul className="mt-2 space-y-2 text-gray-700">
        <li className="flex justify-between">
          <span>Cash</span>
          <span>{formatAmount(cash)}</span>
        </li>
        <li className="flex justify-between">
          <span>Deposit</span>
          <span>{formatAmount(deposit)}</span>
        </li>
        <li className="flex justify-between">
          <span>Mileage</span>
          <span>{formatAmount(mileage)}</span>
        </li>
        <li className="flex justify-between">
          <span>Coupon Tickets</span>
          <span>
            {couponCount} {couponCount === 1 ? 'ticket' : 'tickets'}{' '}
            <span className="text-gray-500">(-{formatAmount(ticket)})</span>
          </span>
        </li>
      </ul>
    </div>
