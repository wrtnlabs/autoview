...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Determine writer role
  let writerRole = "Administrator";
  const writer = value.writer as any;
  if ("type" in writer) {
    if (writer.type === "customer") writerRole = "Customer";
    else if (writer.type === "seller") writerRole = "Seller";
  }

  // Determine writer display name: prefer member.nickname, then citizen.name, fallback to ID
  const writerName =
    (writer.member?.nickname as string) ||
    (writer.citizen?.name as string) ||
    writer.id ||
    "Unknown";
  ...
  const commentDate = formatDate(value.created_at);
  const snapshotDate = latestSnapshot ? formatDate(latestSnapshot.created_at) : "";
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="flex items-center mb-3">
        ...
        <div className="ml-2">
          <p className="font-semibold text-gray-900 truncate">{writerName}</p>
          <p className="text-sm text-gray-500">
            {writerRole} · {commentDate}
          </p>
        </div>
      </div>
      ...
      {value.parent_id && (
        <div className="flex items-center text-sm text-gray-500 mb-3">
          <LucideReact.CornerDownRight size={16} />
          <span className="ml-1 truncate">{value.parent_id}</span>
        </div>
      )}
      ...
      {/* Footer: snapshot date & attachments */}
      {latestSnapshot && (
        <div className="mt-4 flex items-center justify-between text-sm text-gray-500">
          <div className="flex items-center">
            <LucideReact.Calendar size={16} />
            <span className="ml-1">{snapshotDate}</span>
          </div>
          {latestSnapshot.files.length > 0 && (
            <div className="flex items-center">
              <LucideReact.FileText size={16} />
              <span className="ml-1">
                {latestSnapshot.files.length} file
