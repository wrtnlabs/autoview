...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const migrations = Array.isArray(value) ? value : [];

  // Map migration state to icon and color
      ...
      {migrations.map((mig) => (
        ...
        >
          <header className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center" title={mig.state}>
              {getStateIcon(mig.state)}
              <span className="ml-1 text-sm text-gray-600 capitalize truncate">
                {mig.state}
              </span>
            </div>
          </header>
          ...
          <ul className="text-sm text-gray-600 space-y-1 mb-3">
            <li className="flex items-center">
              ...
              <span>Created: {formatDate(mig.created_at)}</span>
            </li>
            <li className="flex items-center">
              <LucideReact.Calendar size={16} className="text-gray-400 mr-1" />
              <span>Updated: {formatDate(mig.updated_at)}</span>
            </li>
            <li className="flex items-center">
              <LucideReact.Users size={16} className="text-gray-400 mr-1" />
              <span>Repos: {mig.repositories.length}</span>
            </li>
          </ul>
