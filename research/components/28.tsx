...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const revisionCount = value.snapshots.length;
  ...
  const displayName =
    ...
    writer.type.charAt(0).toUpperCase() + writer.type.slice(1);

  const writerIcon = (() => {
    switch (writer.type) {
      case 'customer':
        return <LucideReact.User size={20} className="text-blue-500" />;
      case 'seller':
        return <LucideReact.ShoppingCart size={20} className="text-green-500" />;
      case 'administrator':
        return <LucideReact.ShieldUser size={20} className="text-purple-500" />;
      default:
        return <LucideReact.User size={20} className="text-gray-500" />;
    }
  })();
  ...
  return (
    <div className="max-w-md w-full mx-auto bg-white rounded-lg shadow p-4">
      <div className="flex items-center">
        ...
        <div className="ml-3 text-sm">
          ...
          <div className="flex items-center text-gray-500 text-xs mt-0.5">
            <LucideReact.Calendar size={14} className="mr-1 text-gray-400" aria-hidden={true} />
            <time dateTime={value.created_at}>{formattedDate}</time>
            {revisionCount > 1 && (
              <span className="flex items-center ml-3">
                <LucideReact.Clock size={14} className="mr-1 text-gray-400" aria-hidden={true} />
      ...
      <div className="mt-3 text-gray-700 text-sm line-clamp-3">
        {latestSnapshot.body}
      </div>
