...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Determine writer role, icon, and display name
  const { icon: roleIcon, name: displayName } = (() => {
    if ("type" in writer) {
      if (writer.type === "seller") {
        return {
          icon: <LucideReact.Store size={16} className="text-blue-500" />,
          name: writer.member.nickname,
        };
      }
      if (writer.type === "customer") {
        const name =
    ...
    // Administrator invert fallback
    return {
      icon: <LucideReact.Shield size={16} className="text-purple-500" />,
      name: writer.member.nickname,
    };
  })();
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border border-gray-200 max-w-md mx-auto">
      ...
      {latestSnapshot && (
        <div className="mt-3 text-gray-700 text-sm line-clamp-3 whitespace-pre-wrap">
          {latestSnapshot.body}
        </div>
      )}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {latestSnapshot.files.map((file) => {
            ...
            return (
              <div
                key={file.url}
                className="flex items-center space-x-1 bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs"
              >
