...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Badge for chat status
  const chatStatusLabel = chat?.active ? 'Active' : 'Inactive';
  const chatStatusClasses = chat?.active
  ...
  // Fallback when no core data
  const hasContent =
    chat || bot || managers.length > 0 || message || thread;
    ...
    <div className="bg-white shadow-md rounded-lg p-4 max-w-md mx-auto">
      ...
      {chat?.channelId && (
        <div className="flex items-center justify-between mb-3">
          ...
          <span
            className={`px-2 py-0.5 text-xs font-semibold rounded ${chatStatusClasses}`}
          >
            {chatStatusLabel}
          </span>
        </div>
