...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const stateLabel = value.state
    ...
    : "Unknown";
  const dateSource = value.published_at ?? value.created_at ?? value.updated_at;
  const formattedDate = dateSource
    ? new Date(dateSource).toLocaleString(undefined, {
        year: "numeric",
        month: "short",
        day: "numeric",
        hour: "2-digit",
    ...
    : "N/A";
  const vulnerabilitiesCount = value.vulnerabilities?.length ?? 0;
  const cvssScore = value.cvss?.score;
  const authorName = value.author?.login ?? value.author?.name ?? "Unknown";
  const publisherName =
    value.publisher?.login ?? value.publisher?.name ?? "Unknown";
  const cweList = value.cwe_ids ?? [];

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-xl mx-auto p-4 bg-white rounded-lg shadow-md space-y-4">
      {/* Header: IDs and Status */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
        <div className="flex items-center space-x-2">
          <LucideReact.Shield size={20} className="text-gray-600" />
        ...
        <div className="flex flex-wrap gap-2 mt-2 sm:mt-0">
          <span
            className={`px-2 py-0.5 rounded-full text-xs font-medium ${
              severityColorMap[value.severity ?? "null"]
            }`}
          >
            {severityLabel}
          </span>
      ...
      {/* Summary */}
      <p className="text-gray-700 line-clamp-3">{value.summary}</p>
      ...
      <div className="flex flex-wrap text-sm text-gray-500 gap-4">
        {dateSource && (
          <div className="flex items-center">
            ...
            <span className="ml-1">Published: {formattedDate}</span>
          </div>
        )}
        <div className="flex items-center">
          <LucideReact.Bug size={16} />
          <span className="ml-1">
            {vulnerabilitiesCount} Vulnerabilit
            {vulnerabilitiesCount === 1 ? "y" : "ies"}
          </span>
        </div>
      ...
      <div className="flex flex-wrap text-sm text-gray-500 gap-4">
        <div className="flex items-center">
          ...
          <span className="ml-1">Author: {authorName}</span>
        </div>
        <div className="flex items-center">
          <LucideReact.UserCheck size={16} />
          <span className="ml-1">Publisher: {publisherName}</span>
        </div>
      </div>
