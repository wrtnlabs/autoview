...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const rawState = value.publish?.state ?? 'none';
  const statusLabel =
    rawState === 'none'
      ? 'Pending Payment'
    ...
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md flex flex-col space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Order ID: {value.id}</p>
        </div>
      ...
      <div className="text-sm text-gray-500 flex items-center space-x-1">
        ...
        <span>
          {value.goods.length} item{value.goods.length !== 1 && 's'}
        </span>
      </div>
      ...
      <ul className="divide-y divide-gray-100">
        {value.goods.map((good) => {
          ...
          const thumbUrl = good.commodity.sale.content.thumbnails[0]?.url;
          return (
            <li key={good.id} className="flex items-center py-3">
              <img
                src={thumbUrl}
                alt={title}
                className="w-12 h-12 bg-gray-100 object-cover rounded"
                onError={(e) => {
              ...
              <div className="ml-4 flex-1">
                ...
                <p className="text-xs text-gray-500">Qty: {good.volume}</p>
              </div>
      ...
      {/* Price summary */}
      <div className="border-t border-gray-100 pt-4 flex flex-col space-y-1 text-sm">
        <div className="flex justify-between">
          <span>Subtotal</span>
          <span>${formatNumber(totalNominal)}</span>
        </div>
        {totalNominal !== totalReal && (
          <div className="flex justify-between text-gray-700">
            <span>Discounted Total</span>
            <span>${formatNumber(totalReal)}</span>
          </div>
        )}
      </div>
    </div>
