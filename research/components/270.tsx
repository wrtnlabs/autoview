...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const userName = value.user?.name?.trim() || 'Unknown User';
  const avatarInitial = userName.charAt(0).toUpperCase();

  // Map chat state to human-readable labels
  ...
  const chatStateLabel = stateMap[rawState] || 'Unknown';

  // Determine badge styles for different states
  ...
  const lastMessageTime = value.message?.createdAt
  ...
  // Recursively extract a text preview from message blocks
  function extractText(
    blocks: AutoViewInputSubTypes.message.Block[] | undefined
  ): string | undefined {
    if (!blocks) return undefined;
    for (const blk of blocks) {
      if (blk.value) return blk.value;
      const nested = extractText(blk.blocks);
  ...
  let rawPreview = value.message?.plainText
    || extractText(value.message?.blocks)
    || '';
  rawPreview = rawPreview.trim().replace(/\s+/g, ' ');
  const preview =
  ...
  return (
    <div className="flex items-start p-4 bg-white rounded-lg shadow-sm space-x-4">
      {/* Avatar or Initial */}
      ...
      <div className="flex-1 min-w-0">
        {/* Header: Name, State badge, Time, Unread */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span
              className={`px-2 py-0.5 text-xs font-medium rounded-full ${stateBadgeClass}`}
            >
              {chatStateLabel}
            </span>
          </div>
        ...
        {/* Message preview */}
        {preview && (
          <p className="mt-2 text-gray-600 text-sm line-clamp-2">{preview}</p>
        )}
          ...
          <div className="mt-3 flex flex-wrap gap-1">
            {tags.map((tag) => (
              <span
                ...
                className="inline-flex items-center px-2 py-0.5 bg-indigo-100 text-indigo-800 text-xs font-medium rounded"
              >
                {tag.name}
              </span>
            ))}
