...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  type Writer = AutoViewInputSubTypes.IShoppingSaleInquiryComment["writer"];

  const renderWriter = (writer: Writer): JSX.Element => {
    // Determine display name
    const name =
      // Member nickname if available
      ("member" in writer && writer.member?.nickname) ||
      // Citizen name if available
      ("citizen" in writer && writer.citizen?.name) ||
      // Fallback to generic
      "Unknown";

    // Choose icon by writer type
    let icon = <LucideReact.User size={16} className="text-gray-500" />;
    if ("type" in writer) {
      if (writer.type === "administrator") {
        icon = <LucideReact.UserCheck size={16} className="text-blue-500" />;
      } else if (writer.type === "seller") {
        icon = <LucideReact.ShoppingCart size={16} className="text-purple-500" />;
      } else if (writer.type === "customer") {
        icon = <LucideReact.User size={16} className="text-green-500" />;
      }
    }
    ...
    <div className="space-y-4 p-4">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-lg shadow-sm">
        <div className="flex items-center text-sm text-gray-600 gap-1">
          <LucideReact.ChevronsLeft size={16} />
          <span>
        ...
        <div className="flex items-center text-sm text-gray-600 gap-1">
          ...
          <span>{pagination.records} comments</span>
        </div>
      </div>
        ...
        {data.map((comment) => {
          ...
          return (
            ...
            >
              ...
              {/* Comment Body */}
              <div className="text-sm text-gray-700 mb-2 line-clamp-3 whitespace-pre-wrap">
                {body}
              </div>

              {/* Footer: Format & Attachments */}
