...
export type AutoViewInput =
  AutoViewInputSubTypes.IApiReposActionsRuns.GetResponse;
...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Map each run to a formatted object for display
  const formattedRuns = workflow_runs.map((run) => {
    // Determine display name
    const runName = run.name ?? run.display_title ?? `Run #${run.run_number}`;

    // Format date
    const dateSource = run.run_started_at ?? run.created_at;
    const formattedDate = new Date(dateSource).toLocaleString(undefined, {
      dateStyle: "medium",
      timeStyle: "short",
    });
    ...
    let statusIcon: React.ReactNode = (
      <LucideReact.Circle size={16} className="text-gray-400" />
    );
    if (run.status === "queued") {
      ...
      if (run.conclusion === "success") {
        ...
        const conclusionLabel = run.conclusion
          ? run.conclusion.charAt(0).toUpperCase() + run.conclusion.slice(1)
          : "Failed";
        statusText = conclusionLabel;
        statusIcon = (
    ...
    return { ...run, runName, formattedDate, statusText, statusIcon };
  });
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md space-y-4">
      ...
      {formattedRuns.length === 0 ? (
        <div className="flex items-center justify-center py-8 text-gray-500 space-x-2">
          ...
          {formattedRuns.map((run) => (
            ...
            >
              ...
              <div className="flex items-center space-x-2 mt-3 sm:mt-0">
                <span className="text-sm text-gray-700">{run.statusText}</span>
                {run.statusIcon}
              </div>
            </li>
          ))}
