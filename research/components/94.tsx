...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  // 1. Data aggregation / derived constants
  const latestSnapshot = value.snapshots[value.snapshots.length - 1];
  const questionDate = new Date(latestSnapshot.created_at).toLocaleString();
  const isRead = value.read_by_seller;
  const secret = value.secret;
  const channelName = value.customer.channel?.name || "Unknown";
  const memberStatus = value.customer.member ? "Member" : "Guest";
  const answer = value.answer;
  const latestAnswerSnapshot =
    answer && answer.snapshots.length > 0
      ? answer.snapshots[answer.snapshots.length - 1]
      : null;
  const answerDate = latestAnswerSnapshot
    ? new Date(latestAnswerSnapshot.created_at).toLocaleString()
    : "";
  ...
  return (
    <div
      className={`p-4 bg-white rounded-lg shadow-md border-l-4 ${
        secret ? "border-yellow-400" : "border-blue-400"
      } space-y-4`}
    >
      {/* Header: type, secret icon, date, read status */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
        <div className="flex items-center space-x-2">
          {secret ? (
            <LucideReact.Lock className="text-yellow-500" size={16} />
          ) : (
            <LucideReact.Unlock className="text-blue-500" size={16} />
          )}
          <span className="text-sm font-medium text-gray-600">
            {secret ? "Secret Question" : "Question"}
          </span>
        </div>
        <div className="flex items-center space-x-4 mt-2 sm:mt-0">
          ...
          <div className="flex items-center text-sm text-gray-500">
            {isRead ? (
              ...
              <LucideReact.EyeOff size={16} className="mr-1 text-red-500" />
            )}
            <span>{isRead ? "Read by Seller" : "Unread"}</span>
          </div>
        </div>
      </div>
      ...
      <article>
        ...
        <p className="mt-1 text-gray-700 overflow-hidden line-clamp-3">
          {latestSnapshot.body}
        </p>
      </article>
      ...
      {latestSnapshot.files.length > 0 && (
        <div className="flex items-center space-x-2">
          <LucideReact.FileText className="text-indigo-500" size={16} />
          <span className="text-sm text-indigo-600">
            {latestSnapshot.files.length} attachment
            {latestSnapshot.files.length > 1 ? "s" : ""}
          </span>
        </div>
      )}
      ...
      <div className="text-sm text-gray-500">
        Asked via <span className="font-medium">{channelName}</span> as{" "}
        <span className="font-medium">{memberStatus}</span>
      </div>

      {/* Seller's Answer */}
      {answer && latestAnswerSnapshot && (
        <section className="pt-4 border-t border-gray-200 space-y-2">
          <div className="flex items-center space-x-2">
            <LucideReact.MessageSquare size={18} className="text-green-500" />
          ...
          <h5 className="mt-1 text-gray-800 font-medium truncate">
            {latestAnswerSnapshot.title}
          </h5>
