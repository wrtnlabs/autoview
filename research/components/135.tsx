...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const categories = value.categories.map((c) => c.name);
  const tags = value.tags;

  const unitsCount = value.units.length;
  const totalVariants = value.units.reduce(
    (sum, u) => sum + (u.stocks?.length || 0),
    0,
  );

  const allPrices = value.units.flatMap((u) =>
    u.stocks?.map((s) => s.price.real) ?? [],
  );
  ...
  if (value.suspended_at) {
    ...
    value.opened_at &&
    new Date(value.opened_at) > new Date()
  ) {
    statusLabel = "Upcoming";
    statusColor = "bg-blue-100 text-blue-800";
  } else if (
    value.closed_at &&
    new Date(value.closed_at) <= new Date()
  ) {
    statusLabel = "Closed";
  ...
  // Version badge
  const versionLabel = value.latest
    ? "Latest version"
    : "Archived version";
  const versionColor = value.latest
    ...
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
      <div className="p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2 items-center">
          <span
            className={`inline-block ${statusColor} text-xs font-medium px-2.5 py-0.5 rounded`}>
            {statusLabel}
          </span>
          <span
            className={`inline-block ${versionColor} text-xs font-medium px-2.5 py-0.5 rounded`}>
            {versionLabel}
          </span>
        ...
        <p className="text-sm text-gray-600">
          Units: {unitsCount}, Variants: {totalVariants}
        </p>
