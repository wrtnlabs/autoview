...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const isSuspended = !!value.suspended_at;
  const isPaused = !!value.paused_at && !isSuspended;
  ...
  if (isSuspended) {
  ...
  } else if (openedAt && openedAt > now) {
    statusText = "Upcoming";
    statusIcon = <LucideReact.Clock size={16} className="text-amber-500" />;
  } else if ((!openedAt || openedAt <= now) && (!closedAt || closedAt > now)) {
    statusText = "On Sale";
    statusIcon = (
    ...
    statusText = "Closed";
    statusIcon = <LucideReact.XCircle size={16} className="text-gray-500" />;
  }
  ...
  const firstThumbnail = value.content.thumbnails[0]?.url;
  const placeholder = `https://placehold.co/400x300/f1f5f9/64748b?text=${encodeURIComponent(
    value.content.title,
  )}`;
  ...
  return (
    <div className="max-w-sm mx-auto bg-white rounded-lg shadow-md overflow-hidden">
      <div className="relative w-full aspect-[4/3] bg-gray-100">
        <img
          src={firstThumbnail || placeholder}
        ...
        />
        <div className="absolute top-2 right-2 flex items-center space-x-1 bg-white bg-opacity-75 text-xs px-2 py-1 rounded-full">
          {statusIcon}
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex flex-wrap text-sm text-gray-500 gap-4">
