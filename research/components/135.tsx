...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Price range across all units' stocks
  ...
  // Sale status determination
  let status = "Ongoing";
  let StatusIcon = <LucideReact.Calendar size={16} className="text-gray-500" />;
  ...
  if (value.suspended_at) {
    ...
    status = "Paused";
    StatusIcon = <LucideReact.PauseCircle size={16} className="text-amber-500" />;
  ...
  const openedAt = value.opened_at ? formatDate(value.opened_at) : null;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="flex flex-col md:flex-row bg-white rounded-lg shadow-md overflow-hidden">
      {/* Thumbnail */}
      <div className="w-full md:w-40 h-48 md:h-auto flex-shrink-0">
        <img
          src={thumbnailUrl}
          alt={title}
      ...
      <div className="flex-1 p-4 flex flex-col justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center space-x-2">
            ...
            {openedAt && (
              <span className="text-sm text-gray-400">Since {openedAt}</span>
            )}
          </div>
