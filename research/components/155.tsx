...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const categoryPaths: string[] = value.categories.map((cat) => {
    const path: string[] = [];
  ...
  const unitsSummary: Array<{
    ...
    const available = unit.stocks.reduce(
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden md:flex">
      {/* Thumbnail */}
      <div className="md:w-1/3 bg-gray-100 aspect-square">
        <img
          src={thumbnailUrl}
      ...
      <div className="p-4 md:w-2/3 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {value.tags.map((tag) => (
            <span
              key={tag}
              className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs truncate"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="pt-2 border-t border-gray-200">
          <h3 className="text-md font-medium text-gray-700 mb-2">Units</h3>
          <div className="space-y-2">
            {unitsSummary.map((unit) => (
              <div
                key={unit.id}
                className="p-2 bg-gray-50 rounded border border-gray-200 space-y-1"
              >
                {/* Unit Name & Primary Indicator */}
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 text-sm text-gray-600">
                  ...
                  <span>
                    {unit.minPrice === unit.maxPrice
                      ...
                      : `${formatPrice(unit.minPrice)} - ${formatPrice(
                          unit.maxPrice,
                        )}`}
                  </span>
                </div>
