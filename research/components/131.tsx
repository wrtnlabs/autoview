...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const totalPriceReal = value.price.real;
  const totalPriceNominal = value.price.nominal;
  const hasDiscount = totalPriceNominal > totalPriceReal;
  const totalGoods = value.goods.length;

  const paymentMethods = [
    { label: 'Cash', amount: value.price.cash },
    { label: 'Deposit', amount: value.price.deposit },
    { label: 'Mileage', amount: value.price.mileage },
    { label: 'Coupon', amount: value.price.ticket },
  ].filter(pm => pm.amount > 0);

  const formatNumber = (num: number) =>
    new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(num);

  const goodsToShow = value.goods.slice(0, 3);
  const moreCount = totalGoods - goodsToShow.length;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md max-w-md mx-auto">
      ...
      <div className="space-y-2 mb-4">
        {goodsToShow.map((good) => {
          ...
          const thumbnail = good.commodity.sale.content.thumbnails?.[0]?.url;
          const quantity = good.volume;
          const price = good.price.real;

          return (
            <div key={good.id} className="flex items-center space-x-3">
        ...
        {moreCount > 0 && (
          <div className="text-xs text-gray-500 text-center">+{moreCount} more</div>
        )}
      </div>
      ...
      <div className="border-t pt-3">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-gray-700">Total ({totalGoods} items)</span>
          <div className="flex items-baseline space-x-2">
            {hasDiscount && (
              <span className="text-xs text-gray-500 line-through">
                ${formatNumber(totalPriceNominal)}
              </span>
            )}
            <span className="text-lg font-semibold text-gray-900">
              ${formatNumber(totalPriceReal)}
            </span>
          </div>
        </div>
        {paymentMethods.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {paymentMethods.map(pm => (
              ...
              >
                {pm.label}: ${formatNumber(pm.amount)}
              </span>
            ))}
          </div>
        )}
