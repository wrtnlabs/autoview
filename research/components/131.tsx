...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const paidDate =
    publish?.paid_at != null ? formatDate(publish.paid_at) : null;

  // Preview up to two goods images
  const goodsPreview = goods.slice(0, 2);
  const moreCount = goods.length - goodsPreview.length;

  // Map delivery status to icons and labels
  const statusKey = publish?.state ?? "none";
  ...
  const {
    ...
    text: statusText,
    color: statusColor,
  } = statusMap[statusKey] || statusMap.none;

  // 2. Compose the visual structure using JSX and Tailwind CSS.
  return (
    <div className="max-w-md mx-auto bg-white shadow-md rounded-lg overflow-hidden p-6 space-y-4">
      {/* Header: Order name and date */}
      ...
      <div className="flex -space-x-2">
        {goodsPreview.map((good) => {
          const content = good.commodity.sale.content;
          const thumbUrl =
            content.thumbnails?.[0]?.url ||
            `https://placehold.co/40x40/f8fafc/475569?text=Item`;
          const title = content.title ?? "Item";
          return (
            <img
              key={good.id}
              src={thumbUrl}
              ...
              title={`${title} ×${good.volume}`}
              className="w-10 h-10 rounded-md object-cover border-2 border-white"
            />
          );
        })}
        {moreCount > 0 && (
          <div className="w-10 h-10 bg-gray-200 rounded-md flex items-center justify-center text-xs text-gray-700 border-2 border-white">
            +{moreCount}
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center text-sm">
          <StatusIcon size={16} className={`${statusColor} mr-1`} />
      ...
      {paidDate && (
        <div className="flex items-center text-sm text-gray-600">
          <LucideReact.CreditCard size={16} className="mr-1" />
          <span>Paid on {paidDate}</span>
        </div>
      )}
      ...
      {publish?.address && (
        <div className="space-y-1 text-sm text-gray-700">
          <div className="flex items-center">
            <LucideReact.MapPin size={16} className="mr-1 text-gray-400" />
            <span className="truncate">
              {publish.address.name}, {publish.address.city},{" "}
              {publish.address.country}
            </span>
          </div>
          <div className="flex items-center">
            <LucideReact.Phone size={16} className="mr-1 text-gray-400" />
            <span>{publish.address.mobile}</span>
          </div>
        </div>
      )}
    </div>
