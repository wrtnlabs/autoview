...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const paymentDate = isPaid
    ? new Date(value.publish!.paid_at!).toLocaleDateString()
    : '';
    
  const formatNumber = (num: number) =>
    new Intl.NumberFormat(undefined, { minimumFractionDigits: 2 }).format(num);

  const isDiscounted = value.price.nominal > value.price.real;

  function getDeliveryIcon(
    state: AutoViewInputSubTypes.IShoppingOrderGood['state']
  ): JSX.Element {
    switch (state) {
      case 'arrived':
      ...
      case 'shipping':
      case 'underway':
        return <LucideReact.Truck className="text-amber-500" size={16} />;
      default:
    ...
    <div className="w-full max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <ul className="divide-y divide-gray-200">
        {value.goods.map((good) => {
          const title =
            good.commodity.sale.content.title || 'Product Item';
          const thumbUrl =
            good.commodity.sale.content.thumbnails[0]?.url ||
            'https://placehold.co/80x80/f8fafc/475569?text=Image';
          return (
            <li key={good.id} className="flex items-center py-2">
              ...
              <div className="ml-4 flex-1 overflow-hidden">
                ...
                <div className="flex items-center text-sm text-gray-500 mt-1">
                  ...
                  <span className="ml-2">
                    ${formatNumber(good.price.real)}
                  </span>
                </div>
              </div>
              <div className="ml-2">{getDeliveryIcon(good.state)}</div>
            </li>
          );
        })}
      </ul>
      ...
      <div className="mt-4 border-t border-gray-200 pt-4">
        <div className="flex justify-between text-gray-700">
          <span>Subtotal:</span>
          <span>${formatNumber(value.price.nominal)}</span>
        </div>
        {isDiscounted && (
          <div className="flex justify-between text-gray-700 mt-1">
            <span>Discount:</span>
            <span>
              -${formatNumber(value.price.nominal - value.price.real)}
            </span>
          </div>
        )}
        <div className="flex justify-between font-semibold text-gray-900 mt-2">
          <span>Total:</span>
          <span>${formatNumber(value.price.real)}</span>
        </div>
      </div>
    </div>
  );
