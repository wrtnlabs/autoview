...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  function formatDate(dateTime: string | null): string | null {
  ...
  function isCouponActive(
    ...
    const start = opened ? new Date(opened) : null;
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-lg">
      {/* Pagination summary */}
      <div className="mb-4 flex items-center text-sm text-gray-600 gap-2">
      ...
      {coupons.length === 0 ? (
        <div className="flex flex-col items-center py-12 text-gray-400">
          ...
          <span className="mt-2">No coupons found</span>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {coupons.map((coupon) => {
            const active = isCouponActive(coupon.opened_at, coupon.closed_at);
            const openLabel = formatDate(coupon.opened_at) || "No start";
            const closeLabel = coupon.closed_at
            ...
            const minLabel =
              coupon.discount.threshold != null && coupon.discount.threshold > 0
                ? `Min $${coupon.discount.threshold}`
                : null;
            ...
            return (
              ...
              >
                ...
                {/* Discount info */}
                <div className="flex items-center text-sm text-gray-700 gap-1 mb-2">
                  {coupon.discount.unit === "amount" ? (
                    <LucideReact.DollarSign size={16} />
                  ) : (
                    <LucideReact.Percent size={16} />
                  )}
                  <span className="font-medium">{discountLabel}</span>
                  {minLabel && (
                    <span className="ml-2 text-gray-500">{minLabel}</span>
                  )}
                </div>
