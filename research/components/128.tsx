...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const { current, records, pages } = pagination;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <div className="text-sm text-gray-500 mt-2 sm:mt-0">
          Page {current} of {pages} &bull; {records.toLocaleString()} total
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {coupons.map((coupon) => (
          <div
            key={coupon.id}
            className="flex flex-col justify-between border border-gray-200 rounded-lg p-4 hover:shadow-lg transition-shadow"
          >
            {/* Name */}
            ...
            <div className="flex items-center text-sm text-gray-700 mt-3">
              ...
              <span className="ml-1 font-medium text-green-700">
                {coupon.discount.unit === "amount"
                  ? `$${coupon.discount.value.toLocaleString()}`
                  : `${coupon.discount.value}%`}
              </span>
              {coupon.discount.threshold != null && (
                <span className="ml-2 text-gray-500">
                  Min {`$${coupon.discount.threshold.toLocaleString()}`}
                </span>
              )}
              {coupon.discount.limit != null &&
                coupon.discount.unit === "amount" && (
                  <span className="ml-2 text-gray-500">
                    Max {`$${coupon.discount.limit.toLocaleString()}`}
                  </span>
                )}
            </div>
            ...
            <div className="flex items-center text-sm text-gray-600 mt-3">
              ...
              />
              <span className="ml-1">
                {formatDate(coupon.opened_at)} –{" "}
                {coupon.closed_at ? formatDate(coupon.closed_at) : "∞"}
              </span>
            </div>
            ...
            <div className="flex items-center text-sm text-gray-600 mt-3">
              ...
              {coupon.inventory.volume_per_citizen != null && (
                <span className="ml-2">
                  / person {coupon.inventory.volume_per_citizen}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center mt-4">
              ...
              >
                {coupon.restriction.access.charAt(0).toUpperCase() +
                  coupon.restriction.access.slice(1)}
              </span>
