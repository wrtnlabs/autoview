...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  // Pull request count
  const prCount = pull_requests?.length ?? 0;
  ...
  if (status === "completed") {
    if (conclusion === "success") {
      ...
      statusText =
        conclusion
          ?.replace("_", " ")
          .replace(/\b\w/g, (c) => c.toUpperCase()) || "Completed";
    }
  } else if (status === "in_progress" || status === "queued") {
    StatusIcon = LucideReact.Clock;
    statusColor = status === "queued" ? "text-gray-500" : "text-amber-500";
  }

  // Run attempt text
  const attemptText =
    run_attempt && run_attempt > 1 ? `Attempt ${run_attempt}` : "";

  // Actor avatar fallback
  const avatarSrc = actor?.avatar_url;
  const avatarFallback = actor
    ? `https://ui-avatars.com/api/?name=${encodeURIComponent(actor.login)}&background=E2E8F0&color=64748B`
    : "";
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md w-full max-w-md mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 mt-2">
        ...
        {/* Event */}
        <div className="flex items-center gap-1">
          <LucideReact.Mic size={16} className="text-gray-400" />
        ...
        {head_branch && (
          <div className="flex items-center gap-1">
            ...
            <span className="truncate max-w-[6rem]">{head_branch}</span>
          </div>
        )}
        {/* SHA */}
        {shortSha && (
      ...
      <div className="flex items-center mt-3 text-sm">
        ...
        <span className="ml-1 font-medium text-gray-700">{statusText}</span>
        {attemptText && (
          <span className="ml-2 text-gray-500">{attemptText}</span>
        )}
      </div>
      ...
      <div className="flex items-center mt-3 text-sm text-gray-500">
        ...
        {actor && (
          <span className="ml-2 truncate max-w-[6rem]">{actor.login}</span>
        )}
        <LucideReact.Calendar size={16} className="ml-4 text-gray-400" />
        <time dateTime={created_at} className="ml-1">
          {createdDate}
        </time>
        {duration && <span className="ml-auto text-gray-500">{duration}</span>}
      </div>
    </div>
  );
}
