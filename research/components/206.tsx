...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const avatarSrc = value.user?.avatarUrl || fallbackAvatar;

  // Flatten message text from blocks if plainText is unavailable
  function extractTextFromBlocks(
    ...
    return blocks
      .map((b) => {
        ...
        if (b.blocks) return extractTextFromBlocks(b.blocks);
        return "";
      })
      .filter(Boolean)
      .join(" ");
  ...
  const rawMessage =
    ...
    extractTextFromBlocks(value.message?.blocks) ||
    "No messages yet";
  const snippet =
    rawMessage.length > 60
      ? rawMessage.slice(0, 57).trim() + "..."
      : rawMessage;
  ...
  return (
    <div className="flex items-center justify-between p-4 hover:bg-gray-50">
        ...
        {tags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {tags.map((tag) => (
              <span
                key={tag.key}
                className="flex items-center text-xs text-blue-600 bg-blue-100 rounded px-1.5 py-0.5"
              >
                <LucideReact.Tag size={12} className="mr-0.5" />
                {tag.name}
              </span>
            ))}
          </div>
      ...
      <div className="flex flex-col items-end space-y-1">
        ...
        {unreadCount > 0 && (
          <div className="flex items-center text-sm text-white bg-blue-500 rounded-full px-2 py-0.5">
