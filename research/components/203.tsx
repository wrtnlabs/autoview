...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const displayName = value.user?.name?.trim() || "Unknown User";
  ...
  const lastMsgDate = value.message?.createdAt
  ...
  const formattedTime = lastMsgDate ? formatTime(lastMsgDate) : "";
  ...
  const unread = value.session?.unread ?? value.userSession?.unread ?? 0;
  ...
  return (
    <div className="flex items-start p-4 bg-white rounded-lg shadow hover:bg-gray-50 transition">
      <div className="relative flex-shrink-0">
        ...
        {unread > 0 && (
          <span className="absolute -top-1 -right-1 inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold text-white bg-red-500 rounded-full">
            {unread}
          </span>
        )}
      </div>
      <div className="flex flex-col flex-grow ml-4 min-w-0">
        ...
        {tags.length > 0 && (
          <div className="flex items-center space-x-2 mt-1 overflow-hidden">
            {tags.slice(0, 3).map((tag, i) => {
              const variant = tag.colorVariant || "green";
              const classes = tagColors[variant] || "bg-gray-100 text-gray-800";
              return (
                <span
                  key={i}
                  className={`px-2 py-0.5 text-xs rounded-full ${classes} truncate`}
                >
                  {tag.name}
                </span>
              );
        ...
        <p className="mt-2 text-sm text-gray-600 line-clamp-2">{lastMsg}</p>
      </div>
    </div>
  );
}
