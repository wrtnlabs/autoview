...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const userName = value.user?.name || value.user?.memberId || "Unknown User";
  const avatarUrl =
  ...
  const stateInfo =
    (chatState && stateMap[chatState]) || {
      icon: <LucideReact.AlertCircle />,
      label: chatState || "Unknown",
      colorClass: "text-gray-500",
    };

  const timestamp =
    value.message?.createdAt ||
    value.userChat?.createdAt ||
    ...
    <div className="max-w-md mx-auto p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-gray-900 font-medium truncate">{userName}</p>
          <div className="flex items-center text-sm mt-0.5">
            {React.cloneElement(stateInfo.icon, {
              size: 16,
              className: `mr-1 ${stateInfo.colorClass}`,
            })}
            <span className="text-gray-500">{stateInfo.label}</span>
          </div>
        </div>
      </div>
      ...
      {/* Message preview */}
      <div className="mt-4 text-gray-800 leading-relaxed line-clamp-3">
        {textContent}
      </div>
