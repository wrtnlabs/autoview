...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const statusMap: Record<string, { label: string; classes: string }> = {
    ...
    snoozed: { label: "Snoozed", classes: "bg-yellow-100 text-yellow-800" },
    queued: { label: "Queued", classes: "bg-blue-100 text-blue-800" },
  };
  const statusInfo = chatState ? statusMap[chatState] : undefined;

  const rawSnippet =
    value.message?.plainText ??
    value.message?.blocks?.find((b) => typeof b.value === "string")?.value ??
    "";
  const snippet =
    rawSnippet.length > 60 ? rawSnippet.slice(0, 60).trim() + "…" : rawSnippet;
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-lg shadow-md p-4 flex flex-col space-y-3">
      <div className="flex items-start">
        ...
        <div className="flex-1 flex flex-col ml-3">
          <div className="flex items-center">
            <h3 className="flex-1 text-gray-900 font-medium truncate">
              {userName}
            </h3>
            {statusInfo && (
              <span
                className={`ml-2 px-2 py-0.5 text-xs font-semibold rounded ${statusInfo.classes}`}
              >
                {statusInfo.label}
              </span>
            )}
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {tags.map((tag, idx) => {
            const clr = tag.colorVariant
              ? tagColorMap[tag.colorVariant] || "bg-gray-100 text-gray-800"
              : "bg-gray-100 text-gray-800";
            return (
              <span
                key={idx}
                className={`text-xs font-medium px-2 py-0.5 rounded ${clr}`}
              >
                {tag.name}
              </span>
            );
