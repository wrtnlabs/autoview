...
export default function VisualComponent(value: AutoViewInput): React.ReactNode {
  ...
  const getStateClasses = (
    state: AutoViewInputSubTypes.repository_advisory["state"]
  ): string => {
    switch (state) {
      case "published":
  ...
  if (!value || value.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-gray-500">
        ...
        <span className="mt-2">No advisories available.</span>
      </div>
    );
  }
        ...
        >
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <span
                className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded ${getSeverityClasses(
                  advisory.severity
                )}`}
              >
              ...
              <span
                className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded ${getStateClasses(
                  advisory.state
                )}`}
              >
                {advisory.state}
            ...
            <div className="mt-4 flex flex-wrap items-center text-sm text-gray-500 space-x-4">
              <div className="flex items-center">
                ...
                {formatDate(advisory.published_at)}
              </div>
              {advisory.author && (
                <div className="flex items-center">
