...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const featureMap: [keyof Schema.code_security_configuration, string][] = [
  ...
  const cards: IAutoView.IAutoViewVerticalCardProps[] = input.map((item) => {
    ...
    const chips: IAutoView.IAutoViewChipProps[] = featureMap.reduce(
      (acc, [key, label]) => {
        const value = (item as any)[key] as string | undefined | null;
        if (value != null) {
          acc.push({
            type: "Chip",
            label: `${label}: ${value}`,
    ...
    return {
      ...
      childrenProps: [
        ...
        {
          type: "CardHeader",
          title: item.name ?? "Unnamed Configuration",
