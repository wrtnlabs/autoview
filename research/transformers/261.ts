...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const makeText = (
    content: string,
    variant: IAutoView.IAutoViewTextProps["variant"] = "body1",
  ...
  const title = user?.name ?? "Unknown User";
  // Description: latest message plain text (truncated) or fallback
  const description = input.message?.plainText
  ...
  // Session alert count
  if (input.session?.alert !== undefined) {
    items.push({
      type: "DataListItem",
  ...
  if (input.chatTags && input.chatTags.length > 0) {
    const chips: IAutoView.IAutoViewChipProps[] = input.chatTags.map((tag) => ({
      type: "Chip",
      label: tag.name,
      color: tag.colorVariant as IAutoView.IAutoViewChipProps["color"] || "gray",
      size: "small",
      variant: "outlined",
    }));
    items.push({
