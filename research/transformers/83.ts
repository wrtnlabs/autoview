...
namespace Schema {
    export type IShoppingOrderDiscountable = {
        deposit: number;
        mileage: number;
        combinations: Schema.IShoppingOrderDiscountable.ICombination[];
    };
    export namespace IShoppingOrderDiscountable {
        export type ICombination = {
            ...
            entries: Schema.IShoppingOrderDiscountable.IEntry[];
            amount: number;
        };
        export type IEntry = {
...
type IAutoViewTransformerInputType = Schema.IShoppingOrderDiscountable;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
    ...
    const listItems: IAutoView.IAutoViewDataListItemProps[] = combinations.map((comb, idx) => {
        const couponsCount = comb.coupons?.length ?? 0;
        const ticketsCount = comb.tickets?.length ?? 0;
        const entriesCount = comb.entries?.length ?? 0;
