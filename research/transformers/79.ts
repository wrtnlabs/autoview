...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
    ...
    const items: IAutoView.IAutoViewDataListItemProps[] = input.data.map((order) => {
        ...
        const goodsMarkdown = order.goods.map((good) => {
            ...
            const saleId = good.commodity?.sale?.id ?? "Unknown";
            const volume = good.volume ?? 0;
        ...
        const content = [
            ...
            "",
            `### 💰 Summary`,
            `- **Nominal**: $${(order.price.nominal ?? 0).toFixed(2)}`,
            `- **Real**: $${(order.price.real ?? 0).toFixed(2)}`
        ].join("\n");
