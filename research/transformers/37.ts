...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
    ...
    const createChips = (labels: string[]): IAutoView.IAutoViewChipGroupProps => ({
        type: "ChipGroup",
    ...
    const formatPriceRange = (stocks: Schema.IShoppingSaleUnitStock[]): string => {
        if (!stocks.length) return "No price info";
    ...
    if (input.categories.length > 0) {
        const categoryLabels = input.categories.map((cat) => (cat as any).name || String((cat as any).id));
