...
type IShoppingOrderPublish = any;
type IAutoViewTransformerInputType = IShoppingOrder;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  // Create a card header to display order-level information.
  const cardHeader: IAutoView.IAutoViewCardHeaderProps = {
    type: "CardHeader",
    title: input.name,
    // We combine order id and creation time (formatted with locale string) into the description.
    description: `Order ID: ${input.id}\nCreated: ${new Date(input.created_at).toLocaleString()}`,
    // Use an icon as startElement to visually represent the order.
    startElement: {
      type: "Icon",
      id: "shopping-cart", // using "shopping-cart" icon (assumes proper icon library mapping)
      color: "blue",
      size: 24,
    },
    // endElement is deliberately left out, but could be used for status badges etc.
  };

  // Transform the list of goods (each order good) into data list items.
  const goodsListItems: IAutoView.IAutoViewDataListItemProps[] = input.goods.map((good) => {
    // Extract sale content from the commodity.
    const saleContent = good.commodity.sale.content;
    // If thumbnails exist, pick the first one to showcase the product image.
    const thumbnail = saleContent.thumbnails && saleContent.thumbnails.length > 0 ? saleContent.thumbnails[0] : null;

    // Label component: Prefer an image if available; otherwise fallback to a product icon.
    let labelComponent: IAutoView.IAutoViewPresentationComponentProps;
    if (thumbnail) {
      labelComponent = {
        type: "Image",
        src: thumbnail.url,
        alt: thumbnail.name || "Product Image",
      } as IAutoView.IAutoViewImageProps;
    } else {
      labelComponent = {
        type: "Icon",
        id: "box", // using a box icon to represent a product if image is missing
        color: "gray",
        size: 24,
      } as IAutoView.IAutoViewIconProps;
    }

    // Compose a markdown summary that includes volume, state, and price details.
    const markdownContent = `**Volume:** ${good.volume}\n\n**State:** ${good.state || "N/A"}\n\n**Price:** Nominal ${good.price.nominal}, Real ${good.price.real}`;
    const valueComponent: IAutoView.IAutoViewMarkdownProps = {
      type: "Markdown",
      content: markdownContent,
    };

    // Return the data list item composed of a label (visual element) and a value (markdown summary).
    return {
      type: "DataListItem",
  ...
  // Wrap the list of goods into a DataList component.
  const goodsDataList: IAutoView.IAutoViewDataListProps = {
    type: "DataList",
    childrenProps: goodsListItems,
  };

  // Create a markdown component to display customer details.
  // Using markdown ensures that even essential text is rendered in a more engaging format.
  const customerInfoMarkdown = `**Customer Information**\n\n` +
    `ID: ${input.customer.id}\n\n` +
    `Channel: ${input.customer.channel.name || input.customer.channel.code}\n\n` +
    `Connected from: ${input.customer.href}`;
  const customerInfo: IAutoView.IAutoViewMarkdownProps = {
    type: "Markdown",
    content: customerInfoMarkdown,
  };

  // Compose the card content to include both customer details and the list of ordered goods.
  ...
  // Finally, assemble a Vertical Card to structure the UI.
  // VerticalCard.childrenProps can be an array of components including header and content.
  ...
  // Return the composed component which adheres to IAutoView.IAutoViewComponentProps.
  return verticalCard;
}
