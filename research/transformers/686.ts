...
type IAutoViewTransformerInputType = Schema.IApiReposCodespaces.GetResponse;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const getStateColor = (state: string): IAutoView.IAutoViewChipProps["color"] => {
    switch (state) {
      case "Available":
        return "success";
      case "Provisioning":
  ...
  const items: IAutoView.IAutoViewListItemProps[] = input.codespaces.map((cs) => {
    ...
    > = [stateChip];
    if (lastUsed) endElements.push(lastUsed);
