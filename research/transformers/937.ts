...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const machineName = input.machine?.display_name ?? "N/A";

  // Normalize Git status values
  const ahead = input.git_status?.ahead ?? 0;
  const behind = input.git_status?.behind ?? 0;
  ...
  const gitChipColor: IAutoView.IAutoViewChipProps["color"] =
    ahead > 0 || behind > 0 ? "warning" : "success";
