...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  for (const lbl of input.labels ?? []) {
    if (typeof lbl === "string") labelNames.push(lbl);
    else if (lbl && typeof lbl.name === "string") labelNames.push(lbl.name);
  }
  if (labelNames.length > 0) {
    // Create a chip group for labels
    const chips = labelNames.map<IAutoView.IAutoViewChipProps>((name) => ({
      type: "Chip",
      label: name,
      size: "small",
      variant: "outlined"
