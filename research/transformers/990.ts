...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const pickIconId = (evt: Schema.event): string => {
    if (evt.payload.comment) return "comment";
    if (evt.payload.pages && evt.payload.pages.length > 0) return "file-alt";
    if (evt.payload.issue) return "exclamation-circle";
    // fallback to a generic activity icon
    return "github";
  };

  // Helper: build a human‐readable action description
  const buildDescription = (evt: Schema.event): string => {
    ...
    if (comment) {
      ...
      const url = (comment.html_url as string) || evt.payload.issue?.html_url || evt.repo.url;
  ...
  const items: IAutoView.IAutoViewListItemProps[] = input.map((evt) => {
