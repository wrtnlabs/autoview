...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  // In this transformation, we will compose a vertical card that displays key order information.
  // The card is divided into three parts: CardHeader, CardContent, and CardFooter.
  // - The CardHeader uses an icon (a shopping cart) to represent the order visually
  //   along with the order name and order id.
  // - The CardContent uses a Markdown component to render order details in a friendly format.
  // - The CardFooter includes a button to "View Details" of the order, linking to a details page.
  
  // Compose the CardHeader using the IAutoViewCardHeaderProps type:
  const cardHeader: IAutoView.IAutoViewCardHeaderProps = {
    type: "CardHeader",
    title: input.name,
    // Combine order id and creation date to form a descriptive subtitle.
    description: `Order ID: ${input.id} · Created: ${input.created_at}`,
    // Use an icon to visually represent the order. Using "shopping-cart" as a common icon.
    startElement: {
      type: "Icon",
      id: "shopping-cart", // Icon name in kebab-case with no set prefix
      color: "blue",
      size: 24
    }
  };

  // Construct Markdown content to summarize order details.
  // We include customer information, goods count, and payment summary.
  const markdownContent = `
**Order Details:**

- **Customer ID:** ${input.customer.id}
- **Number of Goods:** ${input.goods && Array.isArray(input.goods) ? input.goods.length : 0}
- **Payment Summary:**
   - **Cash:** ${input.price.cash}
   - **Deposit:** ${input.price.deposit}
   - **Mileage:** ${input.price.mileage}
   - **Ticket:** ${input.price.ticket}
   - **Nominal Price:** ${input.price.nominal}
   - **Real Price:** ${input.price.real}
`;

  // Compose the CardContent utilizing the IAutoViewMarkdownProps component inside of CardContent.
  ...
  // Compose the CardFooter with a Button element to act as a call-to-action.
  // The button links to a details page URL that is dynamically constructed.
  const cardFooter: IAutoView.IAutoViewCardFooterProps = {
    ...
    childrenProps: {
      ...
      variant: "contained",
      color: "primary",
      // Construct a URL based on the order id.
      href: `/orders/${input.id}`
    }
  };

  // Wrap the header, content, and footer inside a VerticalCard component.
  // This vertical card serves as a visual container that is responsive by design.
