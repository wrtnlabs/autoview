...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
   ...
   */
  function chooseScoreColor(score: number): IAutoView.IAutoViewChipProps['color'] {
  ...
  const listChildren: IAutoView.IAutoViewDataListItemProps[] = 
    input.snapshots && input.snapshots.length > 0
      ? input.snapshots.map((snap) => {
          // Format file attachments as buttons linking to the URL.
          const attachmentButtons: IAutoView.IAutoViewButtonProps[] = (snap.files || []).map((file) => ({
            type: 'Button',
