...
namespace Schema {
    export namespace IApiSearchCommits {
        export type GetResponse = {
            ...
            items: Schema.commit_search_result_item[];
        };
    }
    /**
...
type IAutoViewTransformerInputType = Schema.IApiSearchCommits.GetResponse;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const listItems: IAutoView.IAutoViewDataListItemProps[] = input.items.map((item) => {
    ...
    const authorLogin = author?.login ?? item.commit.author.name;
    ...
    const messageMd: IAutoView.IAutoViewMarkdownProps = {
      type: "Markdown",
