    ...
    export namespace IShoppingDelivery {
        ...
        */
        export type IInvert = {
            ...
            */
            orders: Schema.IShoppingOrder.IInvertFromDelivery[];
            /**
             * Primary Key.
             *
             * @title Primary Key
...
type IAutoViewTransformerInputType = Schema.IShoppingDelivery.IInvert;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
    ...
    const mapStateColor = (state: string): IAutoView.IAutoViewChipProps["color"] => {
        switch (state) {
            case "none":
            case "preparing":
            case "manufacturing":
    ...
    const journeyItems: IAutoView.IAutoViewDataListItemProps[] = input.journeys.map((j) => {
    ...
    const journeysSection: IAutoView.IAutoViewPresentationComponentProps = {
        type: "DataList",
        childrenProps: journeyItems,
    };

    // 3. Orders: list each order with ID and summary (item count + total)
    const orderItems: IAutoView.IAutoViewDataListItemProps[] = input.orders.map((o) => {
    ...
    const ordersSection: IAutoView.IAutoViewPresentationComponentProps = {
        type: "DataList",
        childrenProps: orderItems,
    };

    // 4. Shippers: show each shipper as a small Chip
    const shipperChips: IAutoView.IAutoViewChipProps[] = input.shippers.map((s) => ({
        type: "Chip",
    ...
    const shippersSection: IAutoView.IAutoViewChipGroupProps = {
        type: "ChipGroup",
