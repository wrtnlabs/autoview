...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const priceItems: IAutoView.IAutoViewDataListItemProps[] = [];
  ...
  const priceList: IAutoView.IAutoViewDataListProps = {
    type: "DataList",
    childrenProps: priceItems
  };

  // If there are coupon ticket payments, render a separate section
  const children: IAutoView.IAutoViewPresentationComponentProps[] = [priceList];
  ...
  if (input.ticket_payments && input.ticket_payments.length > 0) {
    ...
    const couponItems: IAutoView.IAutoViewDataListItemProps[] = input.ticket_payments.map(payment => ({
      type: "DataListItem",
      label: [
