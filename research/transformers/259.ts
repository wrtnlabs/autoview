...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  // 1. User information
  if (input.user) {
    const name = input.user.name ?? input.user.id ?? "Unknown User";
  ...
  if (input.chatTags && input.chatTags.length > 0) {
    ...
    const chips: IAutoView.IAutoViewChipProps[] = input.chatTags.map((tag) => ({
      ...
      label: tag.name,
      variant: "outlined",
      color: colorMap[tag.colorVariant ?? ""] ?? "gray",
      size: "small",
    }));
    items.push({
      type: "DataListItem",
  ...
  const header: IAutoView.IAutoViewCardHeaderProps = {
    type: "CardHeader",
    title: input.user?.name ?? input.user?.id ?? "Chat Overview",
    description:
      input.user?.email ??
      (input.session ? `Chat ID: ${input.session.chatId}` : undefined),
