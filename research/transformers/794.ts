...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const stateBadge: IAutoView.IAutoViewBadgeProps = {
    type: "Badge",
  ...
  const dataListItems: IAutoView.IAutoViewDataListItemProps[] = [
    ...
    },
    {
      ...
      value: {
        type: "Text",
        content: String(input.reactions?.total_count ?? 0),
        variant: "body2"
      }
    }
  ];
    ...
    childrenProps: [
      {
        type: "CardHeader",
        title: input.title,
        description: `#${input.number} opened by ${authorLogin}`,
        // Show author avatar if available
        ...(authorAvatar
          ? {
              startElement: {
                type: "Avatar",
