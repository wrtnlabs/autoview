...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
    ...
    const avatar: IAutoView.IAutoViewAvatarProps | undefined = user?.avatarUrl
        ? {
            type: "Avatar",
            src: user.avatarUrl,
    ...
    if (session) {
        ...
        sessionLines.push(`- **Unread**: ${session.unread ?? 0}`);
    ...
    let messageMarkdown: IAutoView.IAutoViewMarkdownProps | undefined;
    ...
    const chips: IAutoView.IAutoViewChipProps[] = (chatTags ?? []).map((tag) => {
        // Map legacy colorVariant to our chip color scale
        const mapColor: Record<string, IAutoView.IAutoViewChipProps["color"]> = {
            red: "red",
            orange: "orange",
            yellow: "yellow",
            olive: "lime",
        ...
        const color = mapColor[tag.colorVariant ?? "green"] ?? "gray";
        return {
            type: "Chip",
            label: tag.name,
            color,
    ...
    let campaignMarkdown: IAutoView.IAutoViewMarkdownProps | undefined;
    if (campaign) {
        campaignMarkdown = {
            type: "Markdown",
            content: [
                ...
                `- **Viewed**: ${campaign.view ?? 0}`,
                `- **Clicked**: ${campaign.click ?? 0}`,
            ].join("\n"),
        };
    }

    // Assemble card content section
    const contentChildren: IAutoView.IAutoViewPresentationComponentProps[] = [sessionMarkdown];
    if (messageMarkdown) contentChildren.push(messageMarkdown);
    if (campaignMarkdown) contentChildren.push(campaignMarkdown);
