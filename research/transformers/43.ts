...
namespace IPageIShoppingDelivery {
    /**
     * A page.
     *
...
type IAutoViewTransformerInputType = IPageIShoppingDelivery.IInvert;
...
// The function below aggregates and transforms the input data from the shopping delivery page
// into a UI structure that makes effective use of visual elements such as icons and markdown.
// In this example, we build a vertical card that contains overall delivery summary information in the header,
// and a list of individual deliveries in a data list nested within card content.
// Each delivery item is represented using a markdown component for detailed text and an icon
// (based on the delivery state) placed in the value slot of the data list item.

function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  // Map each delivery record to a DataListItem component.
  // We use a markdown component as the label to format details nicely.
  // We also use an icon to represent the delivery state visually.
  const dataListItems: IAutoView.IAutoViewDataListItemProps[] = input.data.map(delivery => {
    // Compose a markdown string with key information about the delivery.
    // Two spaces at the end of a line in markdown force a line break.
    const markdownContent = [
      `**Delivery ID:** ${delivery.id}`,
      `**State:** ${delivery.state}`,
      `**Orders:** ${delivery.orders.length}`,
      `**Journeys:** ${delivery.journeys.length}`,
      `**Pieces:** ${delivery.pieces.length}`,
      `**Shippers:** ${delivery.shippers.length}`,
      `**Created At:** ${delivery.created_at}`
    ].join("  \n");

    // Create a markdown component to serve as the label for this delivery item.
    const labelComponent: IAutoView.IAutoViewMarkdownProps = {
      type: "Markdown",
      content: markdownContent
    };

    // Create an icon component to visually represent the delivery state.
    // Here we choose the icon id based on the state and use a blue color by default.
    // Adjust the icon id and color as needed for your UI.
    const stateIcon: IAutoView.IAutoViewIconProps = {
      type: "Icon",
      id: delivery.state === "shipping" ? "shipping-fast" : "info",
      color: "blue",
      size: 16
    };

    // Return a DataListItem component with a title and the visual elements we composed.
    // The title shows the seller's identifier.
    return {
      type: "DataListItem",
      title: `Seller ID: ${delivery.seller.id}`,
      // Use the markdown component to display detailed info.
      label: labelComponent,
      // Use the icon component to give an immediate visual cue of the state.
      value: stateIcon
    };
  });

  // Compose a DataList component containing all the delivery items.
  const dataList: IAutoView.IAutoViewDataListProps = {
    type: "DataList",
    childrenProps: dataListItems
  };

  // Create a CardHeader component to display an overview for the current page.
  // The header includes an icon (using the truck image) and a description with pagination details.
  const cardHeader: IAutoView.IAutoViewCardHeaderProps = {
    type: "CardHeader",
    title: "Delivery Overview",
    description: `Page ${input.pagination.current} of ${input.pagination.pages} (Total records: ${input.pagination.records})`,
    startElement: {
      type: "Icon",
      id: "truck",  // Use an appropriate icon id representing delivery/trucking.
      size: 24,
      color: "blue"
    }
  };

  // Create a CardContent component that nests our data list.
  // Since markdown and datalists are more visually engaging, this component helps group the delivery records.
  const cardContent: IAutoView.IAutoViewCardContentProps = {
    type: "CardContent",
    childrenProps: dataList  // DataList is allowed as a child of CardContent.
  };

  // Finally, embed the header and content in a VerticalCard.
  // The VerticalCard serves as the overall container and ensures responsiveness and consistency in the layout.
  const verticalCard: IAutoView.IAutoViewVerticalCardProps = {
    type: "VerticalCard",
    childrenProps: [cardHeader, cardContent]
  };

  // Return the composed UI component tree.
  return verticalCard;
}
