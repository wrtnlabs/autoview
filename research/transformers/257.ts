...
type IAutoViewTransformerInputType = Schema.desk.SessionBasedUserChatsView;
...
function visualizeData(input: IAutoViewTransformerInputType): IAutoView.IAutoViewComponentProps {
  ...
  const items: IAutoView.IAutoViewDataListItemProps[] = input.sessions.map((session) => {
    // Try to find the corresponding user by personId; fallback to a generic chat label.
    const user = input.users?.find((u) => u.id === session.personId);
    ...
    const nameText: IAutoView.IAutoViewTextProps = {
      type: "Text",
      content: user?.name ?? session.chatId ?? session.chatKey ?? "Chat",
      variant: "body1"
    };

    // Text component for the unread‐message summary.
    const unreadText: IAutoView.IAutoViewTextProps = {
      type: "Text",
