...
You will make a viewer component of ${{ type }} type.

Note that, never import below type from somewhere external file. The types must be embedded in.
...
function chatBot() {
  const result = streamText({
    model: openai("gpt-4o"),
    prompt:
      "You are a helpful assistant. User: How can I improve my productivity?",
    onChunk: ({ chunk }) => {
